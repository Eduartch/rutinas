...
MySQL - 8.0.39 : Database - syscom_bdbateyer
*********************************************************************
*/
...
/*!50003 CREATE FUNCTION `FunIngresaDPedidos`(ncoda varchar(15),ncant float,nprec float,nidauto integer) RETURNS int
BEGIN
declare id integer default 0;
...
/* Function  structure for function  `FunIngresaGuiasTransportista` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaGuiasTransportista` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaGuiasTransportista`(dfecha datetime,cptop varchar(100),cptoll varchar(150),nidauto integer,
dfechat datetime,nidus integer,cdeta varchar(150),nidtr integer,cndoc varchar(12),nidtda integer,nidrt integer,niddt integer,nidtra1 integer) RETURNS int
BEGIN
declare id integer;
insert into fe_guiatr(guia_fech,guia_ptop,guia_ptoll,guia_idau,guia_fect,guia_idus,guia_fope,guia_deta,guia_idtr,
guia_ndoc,guia_moti,guia_codt,guia_idrt,guia_iddt,guia_idtr1)
values(dfecha,cptop,cptoll,nidauto,dfechat,nidus,localtime,cdeta,nidtr,cndoc,'',nidtda,nidrt,niddt,nidtra1);
select last_insert_id() into id from  fe_guiatr group by last_insert_id();
return id;
END */$$
DELIMITER ;
...
dfechat datetime,nidus integer,cdeta varchar(150),nidtr integer,cndoc varchar(12),nidt integer) RETURNS int
BEGIN
declare id integer;
insert into fe_guias(guia_fech,guia_ptop,guia_ptoll,guia_idau,guia_fect,guia_idus,guia_fope,guia_deta,guia_idtr,guia_ndoc,guia_moti,guia_codt)
values(dfecha,cptop,cptoll,nidauto,dfechat,nidus,localtime,cdeta,nidtr,cndoc,'D',nidt);
select last_insert_id() into id from  fe_guias group by last_insert_id();
return id;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProActualizadetalleGuiasTransportista` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizadetalleGuiasTransportista` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizadetalleGuiasTransportista`(nidguia INTEGER)
BEGIN
UPDATE fe_detaguiar SET detv_acti='I' where detv_idgu=nidguia;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProActualizaGuiasTransportista` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizaGuiasTransportista` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizaGuiasTransportista`(dfecha datetime,cptop varchar(100),cptoll varchar(150),nidauto integer,
dfechat datetime,nidus integer,cdeta varchar(150),nidtr integer,cndoc varchar(12),nidtda integer,
nidrt integer,niddt integer,nidtra1 integer,nidguia integer)
BEGIN
update fe_guiatr set guia_fech=dfecha,guia_ptop=cptop,guia_ptoll=cptoll,guia_fect=dfechat,
guia_idua=nidus,guia_deta=cdeta,guia_idtr=nidtr,guia_ndoc=cndoc,guia_codt=nidtda,guia_idrt=nidrt,guia_iddt=niddt,guia_idtr1=nidtra1
where guia_idgui=nidguia;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProIngresaDetalleguiaTransportista` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProIngresaDetalleguiaTransportista` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProIngresaDetalleguiaTransportista`(cdesc VARCHAR(80),nitem INTEGER,nitem1 INTEGER,nitem2 INTEGER,
nidauto INTEGER,npeso decimal(10,2),ncant DECIMAL(12,4),cunid VARCHAR(4))
BEGIN
   INSERT INTO fe_detaguiar(detv_desc,detv_item,detv_ite1,detv_ite2,detv_idgu,detv_cant,detv_unid,detv_peso)
   VALUES(cdesc,nitem,nitem1,nitem2,nidauto,ncant,cunid,npeso);
END */$$
DELIMITER ;
...
nprecio DECIMAL(14,7),ncant DECIMAL(12,4),cunid VARCHAR(4))
BEGIN
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vguiasventas` AS select `b`.`guia_idgui` AS `idguia`,`a`.`idart` AS `coda`,`a`.`descri` AS `descri`,`a`.`unid` AS `unid`,`b`.`guia_ndoc` AS `ndoc`,`b`.`guia_fech` AS `fech`,`b`.`guia_fect` AS `fect`,`b`.`guia_ptoll` AS `ptoll`,`b`.`guia_deta` AS `detalle`,`x`.`entr_cant` AS `cant`,`y`.`placa` AS `placa`,`y`.`razon` AS `Transportista`,`y`.`ructr` AS `ructr`,`y`.`nombr` AS `Chofer`,`y`.`breve` AS `Brevete`,`y`.`cons` AS `Constancia`,`y`.`marca` AS `marca`,`y`.`dirtr` AS `Direccion`,`p`.`nomb` AS `usuario`,`d`.`razo` AS `cliente`,`d`.`idclie` AS `idcliente`,`c`.`ndoc` AS `refe`,`c`.`tdoc` AS `tdoc`,`b`.`guia_arch` AS `guia_arch`,`d`.`clie_corr` AS `clie_corr` from (((((((`fe_guias` `b` join `fe_ent` `x` on((`x`.`entr_idgu` = `b`.`guia_idgui`))) left join `fe_tra` `y` on((`y`.`idtra` = `b`.`guia_idtr`))) join `fe_kar` `s` on((`s`.`idkar` = `x`.`entr_idkar`))) join `fe_art` `a` on((`a`.`idart` = `s`.`idart`))) join `fe_usua` `p` on((`p`.`idusua` = `b`.`guia_idus`))) join `fe_rcom` `c` on((`c`.`idauto` = `b`.`guia_idau`))) join `fe_clie` `d` on((`d`.`idclie` = `c`.`idcliente`))) where (`b`.`guia_acti` <> 'I') */;
