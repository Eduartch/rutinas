...
MySQL - 8.0.42 : Database - sysven_bdalpharma
*********************************************************************
*/
...
ncosto decimal(10,2),nmargen decimal(8,2),cmoneda char(1),cestilo char(1),ncomi decimal(6,3),lista integer) RETURNS int
BEGIN
declare idep integer default 0;
insert into fe_epta(epta_idar,epta_pres,epta_prec,epta_cant,epta_cost,epta_marg,epta_mone,epta_esti,epta_comi,epta_list)
values(niart,idp,nprec,ncant,ncosto,nmargen,cmoneda,cestilo,ncomi,lista);
...
equi1 float,equi2 float,nigv decimal(6,4),nalma integer,ccodigo varchar(15)) RETURNS int
BEGIN
declare nid integer default 0;
...
cidpc,ncome,ncomc,nidusua,nsmin,nsmax,nidcosto,ndolar,cuni1,equi1,equi2,nigv,nalma,ccodigo);
...
/*!50003 CREATE FUNCTION `FunCreaProveedor`(cruc varchar(11),crazo varchar(100),cdire varchar(150),cciud varchar(150),
...
ctdoc varchar(2),cform char,cndoc varchar(12),dfecha date,dfechar date,cdetalle varchar(180),
nv float,nigv float,nt float,cndo2 varchar(12),cm char,
ndolar float,ni float,ctg char,ccodp integer,cmvto char,nus integer,opt integer,nidcodt integer,
n1 integer,n2 integer,n3 integer,nitem decimal(12,2),npvta decimal(12,2)) RETURNS int
BEGIN
declare nid,nidctaper integer;
declare ctipo char;
set nid=0;
if opt=0 then
   ...
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,rcom_otro,pimpo)
...
n1 INTEGER,n2 INTEGER,n3 INTEGER,ngrati DECIMAL(12,2),npvta DECIMAL(12,2),nidven INTEGER) RETURNS int
BEGIN
DECLARE nid,ntdoc,idve,nidctaper INTEGER;
DECLARE ctipo CHAR;
SET ntdoc=0;
SET nid=0;
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,rcom_otro,rcom_dsct,rcom_vend,rcom_porc)
   VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,LOCALTIME,nus,nidcodt,ngrati,npvta,nidven,nporcentaje);
   SELECT LAST_INSERT_ID() INTO nid FROM fe_rcom GROUP BY LAST_INSERT_ID();
   IF n1>0 AND n2>0 AND n3>0 THEN
...
/* Function  structure for function  `FunIngresaCabeceraCVtasinafectas` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaCabeceraCVtasinafectas` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaCabeceraCVtasinafectas`(
ctdoc VARCHAR(2),cform CHAR,cndoc VARCHAR(12),dfecha DATE,dfechar DATE,cdetalle VARCHAR(180),
nv FLOAT,nigv FLOAT,nt FLOAT,cndo2 VARCHAR(12),cm CHAR,
ndolar FLOAT,ni FLOAT,ctg CHAR,ccodp INTEGER,cmvto CHAR,nus INTEGER,nporcentaje DECIMAL(5,2),nidcodt INTEGER,
n1 INTEGER,n2 INTEGER,n3 INTEGER,ngrati DECIMAL(12,2),npvta DECIMAL(12,2),nidven INTEGER,tinafecta decimal(12,2)) RETURNS int
BEGIN
DECLARE nid,ntdoc,idve,nidctaper INTEGER;
DECLARE ctipo CHAR;
SET ntdoc=0;
SET nid=0;
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,rcom_otro,rcom_dsct,rcom_vend,
   rcom_porc,rcom_inaf)
   VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,
   LOCALTIME,nus,nidcodt,ngrati,npvta,nidven,nporcentaje,tinafecta);
   SELECT LAST_INSERT_ID() INTO nid FROM fe_rcom GROUP BY LAST_INSERT_ID();
   IF n1>0 AND n2>0 AND n3>0 THEN
      CALL IngresaCuentasV(tinafecta,nigv,nt,n1,n2,n3,"H","H","D",nid,0,0);
   END IF;
RETURN nid;
END */$$
DELIMITER ;
...
DECLARE id INTEGER;
INSERT INTO fe_cbancos(cban_idba,cban_nume,cban_fech,cban_idmp,cban_deta,cban_idpr,cban_idcl,cban_ndoc,cban_idct,
cban_debe,cban_haber,cban_orde,cban_clpr,cban_tran,cban_ttra,cban_dola,lcaj_form,lcaj_fope)VALUES
(idb,nop,dfecha,idmp,cdeta,idpr,idcl,cndoc,idcta,debe,haber,norden,nidclpr,'T','T',ndola,'E',localtime);
SELECT LAST_INSERT_ID() INTO id FROM fe_cbancos GROUP BY LAST_INSERT_ID();
RETURN id;
END */$$
DELIMITER ;
...
/*!50003 CREATE FUNCTION `FunIngresaDatosLcajaE12`(dfecha date,cndoc varchar(10),cdeta varchar(100),idcta integer,sdeudor decimal(12,2),
sacreedor decimal(12,2),cmone char,ndolar decimal(5,3),nidus integer,nidt integer) RETURNS int
begin
declare id integer;
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
lcaj_idus,lcaj_codt,lcaj_idau,lcaj_fope,lcaj_form)values
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidt,0,localtime,'E');
select last_insert_id() into id from fe_lcaja group by last_insert_id();
return id;
end */$$
DELIMITER ;
...
/*!50003 CREATE FUNCTION `FunIngresaDatosLcajaECreditos`(dfecha date,cndoc varchar(10),cdeta varchar(200),idcta integer,sdeudor decimal(12,2),
...
cunid varchar(20),nequi decimal(8,2),npos integer,idepta integer,comi decimal(6,3)) RETURNS int
BEGIN
declare id integer default 0;
INSERT INTO fe_ped(idart,cant,prec,idautop,dped_unid,dped_equi,dped_posi,dped_idep,dped_comi)
VALUES(ncoda,ncant,nprec,nidauto,cunid,nequi,npos,idepta,comi);
select last_insert_id() into id from fe_ped group by last_insert_id();
return id;
END */$$
DELIMITER ;
...
dfechat DATETIME,nidus INTEGER,cdeta VARCHAR(150),nidtr INTEGER,cndoc VARCHAR(12),nidtda integer,cubigeo varchar(10)) RETURNS int
BEGIN
DECLARE id INTEGER;
...
/* Function  structure for function  `FunIngresaGuias1` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaGuias1` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaGuias1`(dfecha DATETIME,cptop VARCHAR(100),cptoll VARCHAR(150),nidclie INTEGER,
dfechat DATETIME,nidus INTEGER,cdeta VARCHAR(150),nidtr INTEGER,cndoc VARCHAR(12),nidtda integer,cubigeo varchar(10)) RETURNS int
BEGIN
DECLARE id INTEGER;
INSERT INTO fe_guias(guia_fech,guia_ptop,guia_ptoll,guia_idcl,guia_fect,guia_idus,guia_fope,guia_deta,guia_idtr,guia_ndoc,guia_moti,guia_codt,guia_ubig)
VALUES(dfecha,cptop,cptoll,nidclie,dfechat,nidus,LOCALTIME,cdeta,nidtr,cndoc,'v',nidtda,cubigeo);
SELECT LAST_INSERT_ID() INTO id FROM  fe_guias GROUP BY LAST_INSERT_ID();
RETURN id;
END */$$
DELIMITER ;
...
/*!50003 CREATE FUNCTION `FunIngresaGuiasCons`(dfecha DATE,cptop VARCHAR(200),cptoll VARCHAR(200),nidauto INTEGER,
dfechat DATE,nidus INTEGER,cdeta VARCHAR(150),nidtr INTEGER,cndoc VARCHAR(12),cmoti CHAR,nidcl INTEGER,cubigeo VARCHAR(10),ncodt INTEGER) RETURNS int
BEGIN
DECLARE id INTEGER;
INSERT INTO fe_guias(guia_fech,guia_ptop,guia_ptoll,guia_idau,guia_fect,guia_idus,guia_fope,
guia_deta,guia_idtr,guia_ndoc,guia_moti,guia_idcl,guia_ubig,guia_codt)
VALUES(dfecha,cptop,cptoll,nidauto,dfechat,nidus,LOCALTIME,cdeta,nidtr,cndoc,cmoti,nidcl,cubigeo,ncodt);
SELECT LAST_INSERT_ID() INTO id FROM  fe_guias GROUP BY LAST_INSERT_ID();
RETURN id;
END */$$
DELIMITER ;
...
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 integer,vcom float,nequi decimal(12,8),
cuni varchar(60),epta integer,npos integer,ncosto decimal(12,4),nigv decimal(6,4),fvto date,nlote varchar(15)) RETURNS int
BEGIN
declare nidk integer default 0;
if ct='C' then
  INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_idco,codv,kar_alma1,kar_equi,kar_unid,kar_epta,kar_posi,kar_fvto,kar_lote)
  VALUES (nid,cc,ct,npr,nct,tmvto,cincl,calma,nidcosto1,ccodv,calma,nequi,cuni,epta,npos,fvto,nlote);
  SELECT LAST_INSERT_ID() INTO nidk FROM fe_kar GROUP BY LAST_INSERT_ID();
 else
  INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_idco,codv,kar_alma1,kar_comi,
  kar_equi,kar_unid,kar_epta,kar_posi,kar_tigv,kar_cost,kar_fvto,kar_lote)
  VALUES (nid,cc,ct,npr,nct,tmvto,cincl,calma,nidcosto1,ccodv,0,vcom,nequi,cuni,epta,npos,nigv,ncosto,fvto,nlote);
 SELECT LAST_INSERT_ID() INTO nidk FROM fe_kar GROUP BY LAST_INSERT_ID();
end if;
return nidk;
END */$$
...
/* Function  structure for function  `FunIngresaPagosCreditosCV` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaPagosCreditosCV` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaPagosCreditosCV`(
cndoc varchar(12),nacta float,cesta char,cmone char,cb1 varchar(100),dfech date,
dfevto date,ctipo char,nctrl integer,cnrou varchar(40),nidrc integer,cpc varchar(45),
idusua integer,nidv integer) RETURNS int
BEGIN
declare nid integer;
set nid=0;
INSERT INTO fe_cred(fech,fevto,acta,ndoc,estd,mone,banc,tipo,cred_idrc,cred_idus,cred_fope,ncontrol,nrou,cred_idpc,cred_codv)
values(dfech,dfevto,nacta,cndoc,cesta,cmone,cb1,ctipo,nidrc,idusua,localtime,nctrl,cnrou,cpc,nidv);
select last_insert_id() into nid from fe_cred group by last_insert_id();
return nid;
END */$$
DELIMITER ;
...
nv decimal(12,2),nigv decimal(12,2),nt decimal(12,2),cndo2 varchar(10),cm char,
ndolar float,ni float,ctg char,ccodp integer,cmvto char,nus integer,nidv integer,nidcodt integer,
...
set nid=0;
   ...
   idprov,tipom,fusua,idusua,codt,rcom_nitem,rcom_exon,rcom_otro,rcom_vend)
   VALUES (ctdoc,cform,cndoc,dfecha,dfechar,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,
   localtime,nus,nidcodt,nitem,nexon,notros,nidv);
   select last_insert_id() into nid from fe_rcom group by last_insert_id();
   if n1>0 and n2>0 and n3>0 then
      call IngresaCuentas(nv,0,0,0,nigv,0,0,nt,n1,0,0,0,n2,0,0,n3,"D","","","","D","","","H",nid);
   end if;
...
begin
declare id integer;
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,
lcaj_acre,lcaj_mone,lcaj_dola,lcaj_idus,lcaj_clpr,lcaj_tran,lcaj_fope,lcaj_form)values
...
select a.idart,a.tcompras,a.tventas,a.alma,IF(p.prod_equi1>0,p.prod_equi1,1) AS equi1
...
inner join fe_rcom as yy on yy.idauto=b.idauto
where b.acti='A' and yy.acti='A' group by idart,alma) as a
inner join fe_art as p on p.idart=a.idart  where (a.tcompras-a.tventas)<>0  order by a.idart,alma ;
...
if nidc=0 then
  ...
  select idart,dcat,cast(sum(impo) as decimal(12,2)) as impo,nomv,codv from (
...
SELECT
  GROUP_CONCAT(DISTINCT
    CONCAT('sum(if(trim(vv.nomv)=''',trim(nomv),''',vv.impo,0)) AS ',left(nomv,4)))
    INTO @sql
    FROM vvtas;
SET @sql = CONCAT('select idart as Codigo,dcat as producto, ', @sql, '
...
if opt=0 then
   ...
   update fe_rcom set tdoc=ctdoc,form=cform,ndoc=cndoc,fech=dfecha,fecr=dfechar,deta=cdetalle,valor=nv,igv=nigv,impo=nt,ndo2=cndo2,
   mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,rcom_arch="" where idauto=nidauto;
   call ProDesactivaLcajaE(nidauto);
   if n1>0 and n2>0 and n3>0 then
...
ndolar FLOAT,ni FLOAT,ctg CHAR,ccodp INTEGER,cmvto CHAR,nus INTEGER,nporcentaje DECIMAL(5,2),nidcodt INTEGER,
n1 INTEGER,n2 INTEGER,n3 INTEGER,ndscto DECIMAL(12,2),npvta DECIMAL(12,2),nidauto INTEGER,nidven INTEGER)
BEGIN
DECLARE idve,nidctaper INTEGER;
    UPDATE fe_rcom SET tdoc=ctdoc,form=cform,ndoc=cndoc,fech=dfecha,fecr=dfechar,deta=cdetalle,valor=nv,igv=nigv,impo=nt,ndo2=cndo2,
   mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,rcom_porc=nporcentaje,
   rcom_vend=nidven,rcom_arch="",rcom_otro=npvta,rcom_dsct=ndscto WHERE idauto=nidauto;
 CALL ProDesactivaLcajaE(nidauto);
   IF n1>0 AND n2>0 AND n3>0 THEN
...
nr integer,idepta integer,equi decimal(8,4),ctipoa char,npos integer)
BEGIN
if ctipoa='A' then
     UPDATE fe_ped SET acti='I' WHERE idped=nr;
   else
     UPDATE fe_ped SET idart=ncoda,cant=ncant,prec=nprec,
     dped_equi=equi,dped_idep=idepta,dped_posi=npos WHERE idped=nr;
end if;
END */$$
DELIMITER ;
...
cestilo char(1),ncomi decimal(6,3),lista integer)
BEGIN
if opt=0 then
   ...
   epta_marg=nmargen,epta_mone=cmoneda,epta_esti=cestilo,epta_comi=ncomi,epta_list=lista where epta_idep=idepta;
end if;
END */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProActualizaGuiasCons`(dfecha date,cptop varchar(200),cptoll varchar(200),nidauto integer,
...
nigv decimal(6,4),fvto date,nlte varchar(15))
BEGIN
if op=0 then
  Update fe_kar set Acti='I' where idkar=nidkar;
 else
  ...
  idauto=nid,idart=cc,tipo=ct,prec=npr,cant=nct,ttip=tmvto,incl=cincl,
  alma=calma,kar_idco=nidcosto1,codv=ccodv,kar_comi=xcom,
  kar_equi=nequi,kar_unid=cunid,kar_epta=nidepta,kar_posi=npos,kar_cost=ncosto,kar_tigv=nigv,kar_fvto=fvto,kar_lote=nlte where idkar=nidkar;
end if;
END */$$
DELIMITER ;
...
BEGIN
update fe_art set prod_alma=nid,prod_uact=nidusua,prod_fact=localtime,tmon=cmone,prec=nprecio where idart=nidart;
end */$$
DELIMITER ;
...
cc integer,dfe date,npr float,cnd integer,idp integer,cmda char,ni decimal(5,3),ndolar float,nidusua integer)
BEGIN
declare costor,nprecio float;
declare cmoneda char;
SELECT prec into nprecio FROM fe_art WHERE idart=cc;
SELECT tmon into cmoneda FROM fe_art WHERE idart=cc;
   ...
   prod_uact=nidusua,prod_dola=ndolar,prod_tigv=ni WHERE idart=cc;
...
npeso float,ccat integer,cmar integer,ctipro char,nflete integer,cm char,cidpc varchar(45),
ncome float,ncomc float,nidusua integer,nsmin float,nsmax float,nidcosto integer,ndolar decimal(6,4),cuni1 varchar(15),
equi1 decimal(12,2),equi2 decimal(12,2),nigv decimal(6,4),ncoda integer,cesta char,nalma integer,ccodigo varchar(15))
BEGIN
...
prod_tigv=nigv,prod_acti=cesta,prod_alma=nalma,prod_cod1=ccodigo WHERE idart=ncoda;
END */$$
DELIMITER ;
...
BEGIN
   ...
   rcom_vend=nidv,rcom_exon=nexon,rcom_otro=notros where idauto=nidauto;
   call ProDesactivaLcajaE(nidauto);
END */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProactualizaResumenBoletas`(cticket VARCHAR(25),cmensaje VARCHAR(80),cdrxml longblob)
BEGIN
...
/* Procedure structure for procedure `ProDetalleguiasventas` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProDetalleguiasventas` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProDetalleguiasventas`(nidk integer,ncant decimal(8,2),nidg integer,ncoda integer)
BEGIN
...
case
   when nalma=1 then
      select uno as stock from fe_art where idart=nidart;
   when nalma=2 then
      select dos as stock from fe_art where idart=nidart;
   when nalma=1 then
      select tre as stock from fe_art where idart=nidart;
   else
      select cua  as stock from fe_art where idart=nidart;
end case;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProIngresaPagosCreditosCbVendedor` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProIngresaPagosCreditosCbVendedor` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProIngresaPagosCreditosCbVendedor`(
cndoc varchar(10),nacta float,cesta char,cmone char,cb1 varchar(100),dfech date,
dfevto date,ctipo char,nctrl integer,cnrou varchar(40),nidrc float,cpc varchar(45),
idusua integer,idcb integer,nidv integer)
BEGIN
INSERT INTO fe_cred(fech,fevto,acta,ndoc,estd,mone,banc,tipo,cred_idrc,cred_idus,cred_fope,ncontrol,nrou,cred_idpc,cred_idcb,cred_codv)
values(dfech,dfevto,nacta,cndoc,cesta,cmone,cb1,ctipo,nidrc,idusua,localtime,nctrl,cnrou,cpc,idcb,nidv);
END */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProMostrarMenu1`(nid varchar(5),ct char,nidus integer)
BEGIN
SELECT a.Menu_idme as iKey,a.Menu_text as Texto,a.menu_enla as Parent,a.menu_clav as clave from fe_menus a
inner join fe_opt b on b.opti_idme=a.menu_idme where menu_tipo=ct and b.opti_idus=nidus and b.opti_acti=1;
END */$$
DELIMITER ;
...
begin
select a.cban_nume,a.cban_fech,b.pago_codi,b.pago_deta,a.cban_deta,if(a.cban_debe>0,m.razo,n.razo) as razon,a.cban_idba,cban_idco1,cban_dola,
...
BEGIN
SELECT a.pres_desc,b.epta_cost,b.epta_marg,b.epta_prec as epta_prec,round(epta_comi*100,3) as epta_comi,epta_list,
if(b.epta_mone='S',round(b.epta_cost/((100-vv.pmvtas)/100),2),round((b.epta_cost*nd)/((100-vv.pmvtas)/100),2)) as precio1,
...
FROM fe_epta as b  inner JOIN fe_presentaciones as a  ON b.epta_pres=a.pres_idpr, fe_gene as vv
...
BEGIN
SELECT a.pres_desc,b.epta_cost,b.epta_marg,b.epta_prec as epta_prec,round(epta_prec*((vv.gene_cred/100)+1),2) as preciocredito,
round(epta_comi*100,3) as epta_comi,epta_list,
if(b.epta_mone='S',round(b.epta_cost/((100-vv.pmvtas)/100),2),round((b.epta_cost*nd)/((100-vv.pmvtas)/100),2)) as precio1,
if(b.epta_mone='S',b.epta_cost,round(b.epta_cost*nd,2)) as costo,
b.epta_cant,b.epta_pres,b.epta_idar,b.epta_idep,b.epta_mone,b.epta_esti
FROM fe_epta  as b inner join fe_presentaciones as a
on a.pres_idpr=b.epta_pres,fe_gene as vv where b.epta_acti='A' and b.epta_idar=nidart order by b.epta_cant;
END */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProMuestraProductos1`(abuscar varchar(80),nd decimal(5,2),opt integer,nid integer)
BEGIN
declare cbuscar varchar(80);
if opt=1 then
  set cbuscar=concat('%',trim(abuscar),+'%');
  ...
  peso,a.prec,tipro,idmar,a.idcat,cost,tmon,a.idflete,prod_uti1,prod_uti2,prod_uti3,prod_idus,prod_equi1,prod_equi2,
  prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,ifnull(o.razo,'') as proveedor,
  ifnull(yy.ndoc,'') as ndoc,ifnull(yy.fech,'') as fech,prod_idpc,prod_idpm,prod_cod1,prod_acti,prod_alma
  FROM fe_art  as a 
...
/*!50001 CREATE TABLE  `vmuestraventas`(
 ...
 `rcom_porc` decimal(8,2) ,
 `rcom_otro` decimal(12,2) ,
...
/*Table structure for table `vpdtesventas` */

DROP TABLE IF EXISTS `vpdtesventas`;

/*!50001 DROP VIEW IF EXISTS `vpdtesventas` */;
/*!50001 DROP TABLE IF EXISTS `vpdtesventas` */;

/*!50001 CREATE TABLE  `vpdtesventas`(
 `saldo` decimal(12,2) ,
 `ncontrol` int ,
 `fevto` date ,
 `rcre_idcl` int ,
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vguiasdevolucion` AS select `b`.`guia_idgui` AS `idguia`,`a`.`idart` AS `coda`,`a`.`descri` AS `descri`,`a`.`unid` AS `unid`,`b`.`guia_ndoc` AS `ndoc`,`b`.`guia_fech` AS `fech`,`b`.`guia_fect` AS `fect`,`b`.`guia_ptoll` AS `ptoll`,`b`.`guia_deta` AS `detalle`,`x`.`entr_cant` AS `cant`,`y`.`placa` AS `placa`,`y`.`razon` AS `Transportista`,`y`.`ructr` AS `ructr`,`y`.`nombr` AS `Chofer`,`y`.`breve` AS `Brevete`,`y`.`cons` AS `Constancia`,`y`.`marca` AS `marca`,`y`.`dirtr` AS `Direccion`,`p`.`nomb` AS `usuario`,`d`.`razo` AS `cliente`,`d`.`idprov` AS `idprov`,`c`.`ndoc` AS `refe`,`c`.`tdoc` AS `tdoc`,`b`.`guia_mens` AS `guia_mens`,`b`.`guia_arch` AS `guia_arch`,`d`.`email` AS `email`,`b`.`guia_hash` AS `guia_hash`,`b`.`guia_feen` AS `guia_feen`,`b`.`guia_codt` AS `guia_codt`,`b`.`guia_tick` AS `guia_tick` from ((((((`fe_guias` `b` join `fe_ent` `x` on((`x`.`entr_idgu` = `b`.`guia_idgui`))) join `fe_tra` `y` on((`y`.`idtra` = `b`.`guia_idtr`))) join `fe_art` `a` on((`a`.`idart` = `x`.`entr_idar`))) join `fe_usua` `p` on((`p`.`idusua` = `b`.`guia_idus`))) join `fe_rcom` `c` on((`c`.`idauto` = `b`.`guia_idau`))) join `fe_prov` `d` on((`d`.`idprov` = `c`.`idprov`))) where ((`b`.`guia_acti` <> 'I') and (`b`.`guia_moti` = 'D') and (`x`.`entr_acti` = 'A')) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vmuestracompras` AS select `a`.`idauto` AS `idauto`,`a`.`alma` AS `alma`,`a`.`idkar` AS `idkar`,`a`.`kar_equi` AS `kar_equi`,`a`.`kar_fvto` AS `kar_fvto`,`a`.`kar_lote` AS `kar_lote`,`b`.`descri` AS `descri`,`b`.`peso` AS `peso`,`b`.`prod_idco` AS `prod_idco`,`a`.`kar_unid` AS `unid`,`b`.`tipro` AS `tipro`,`a`.`idart` AS `idart`,`a`.`incl` AS `incl`,`c`.`ndoc` AS `ndoc`,`c`.`valor` AS `valor`,`c`.`igv` AS `igv`,`c`.`impo` AS `impo`,`c`.`pimpo` AS `pimpo`,`a`.`cant` AS `cant`,`a`.`prec` AS `prec`,`c`.`fech` AS `fech`,`c`.`fecr` AS `fecr`,`c`.`form` AS `form`,`c`.`exon` AS `exon`,`c`.`ndo2` AS `ndo2`,`c`.`vigv` AS `vigv`,`c`.`idprov` AS `idprov`,`a`.`tipo` AS `tipo`,`c`.`tdoc` AS `tdoc`,`c`.`dolar` AS `dolar`,`c`.`mone` AS `mone`,`p`.`razo` AS `razo`,`p`.`dire` AS `dire`,`p`.`ciud` AS `ciud`,`p`.`nruc` AS `nruc`,`a`.`kar_posi` AS `kar_posi`,`a`.`kar_epta` AS `kar_epta`,ifnull(`x`.`idcaja`,0) AS `Idcaja`,`c`.`codt` AS `codt`,`c`.`fusua` AS `fusua`,`w`.`nomb` AS `Usuario`,`a`.`kar_idkar` AS `kar_idkar`,`c`.`idusua` AS `idusua` from (((((`fe_rcom` `c` left join `fe_kar` `a` on((`c`.`idauto` = `a`.`idauto`))) left join `fe_art` `b` on((`b`.`idart` = `a`.`idart`))) join `fe_prov` `p` on((`p`.`idprov` = `c`.`idprov`))) left join `fe_caja` `x` on((`x`.`idauto` = `c`.`idauto`))) join `fe_usua` `w` on((`w`.`idusua` = `c`.`idusua`))) where ((`c`.`acti` <> 'I') and (`a`.`acti` <> 'I')) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vmuestraventas` AS select `c`.`idusua` AS `idusua`,`b`.`tipro` AS `tipro`,`c`.`rcom_idtr` AS `rcom_idtr`,`a`.`codv` AS `codv`,`a`.`idauto` AS `idauto`,`a`.`kar_cost` AS `kar_cost`,`a`.`kar_tigv` AS `kar_tigv`,`a`.`kar_posi` AS `kar_posi`,`a`.`kar_equi` AS `kar_equi`,`a`.`kar_epta` AS `kar_epta`,`a`.`kar_comi` AS `kar_comi`,`a`.`alma` AS `alma`,`a`.`kar_idco` AS `idcosto`,`a`.`idkar` AS `idkar`,`a`.`idart` AS `Coda`,`a`.`cant` AS `cant`,`a`.`prec` AS `prec`,`c`.`valor` AS `valor`,`c`.`igv` AS `igv`,`c`.`impo` AS `impo`,`c`.`fech` AS `fech`,`c`.`fecr` AS `fecr`,`c`.`form` AS `form`,`c`.`deta` AS `deta`,`c`.`exon` AS `exon`,`c`.`ndo2` AS `ndo2`,`c`.`rcom_entr` AS `rcom_entr`,`c`.`idcliente` AS `idclie`,`d`.`razo` AS `razo`,`d`.`nruc` AS `nruc`,`d`.`dire` AS `dire`,`d`.`ciud` AS `ciud`,`d`.`ndni` AS `ndni`,`a`.`tipo` AS `tipo`,`c`.`tdoc` AS `tdoc`,`c`.`ndoc` AS `ndoc`,`c`.`dolar` AS `dolar`,`c`.`mone` AS `mone`,`b`.`descri` AS `descri`,0 AS `idcaja`,`a`.`kar_unid` AS `unid`,`b`.`premay` AS `pre1`,`b`.`peso` AS `peso`,`b`.`premen` AS `pre2`,ifnull(`z`.`vend_idrv`,0) AS `nidrv`,`c`.`vigv` AS `vigv`,`c`.`idcliente` AS `idcliente`,`c`.`codt` AS `codt`,`b`.`pre3` AS `pre3`,`b`.`cost` AS `costo`,`b`.`uno` AS `uno`,`b`.`dos` AS `dos`,`b`.`tre` AS `tre`,`b`.`cua` AS `cua`,(`b`.`uno` + `b`.`dos`) AS `TAlma`,`c`.`fusua` AS `fusua`,`p`.`nomv` AS `Vendedor`,`q`.`nomb` AS `Usuario`,`b`.`prod_cod1` AS `prod_cod1`,`a`.`kar_lote` AS `kar_lote`,`a`.`kar_fvto` AS `kar_fvto`,`c`.`rcom_dsct` AS `rcom_dsct`,`c`.`rcom_porc` AS `rcom_porc`,`c`.`rcom_otro` AS `rcom_otro`,`c`.`rcom_mens` AS `rcom_mens` from ((((((`fe_rcom` `c` join `fe_kar` `a` on((`a`.`idauto` = `c`.`idauto`))) join `fe_art` `b` on((`b`.`idart` = `a`.`idart`))) join `fe_vend` `p` on((`p`.`idven` = `a`.`codv`))) join `fe_usua` `q` on((`q`.`idusua` = `c`.`idusua`))) join `fe_clie` `d` on((`d`.`idclie` = `c`.`idcliente`))) left join (select `fe_rvendedor`.`vend_idau` AS `vend_idau`,`fe_rvendedor`.`vend_idrv` AS `vend_idrv` from `fe_rvendedor` where (`fe_rvendedor`.`vend_acti` = 'A')) `z` on((`z`.`vend_idau` = `c`.`idauto`))) where ((`c`.`acti` <> 'I') and (`a`.`acti` <> 'I')) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vpdtespagoc` AS select `b`.`rcre_idcl` AS `idclie`,`a`.`ndoc` AS `ndoc`,`s`.`saldo` AS `importe`,`a`.`mone` AS `mone`,`a`.`banc` AS `banc`,`b`.`rcre_fech` AS `fech`,`x`.`razo` AS `razo`,`x`.`fono` AS `fono`,`x`.`dire` AS `dire`,`x`.`ciud` AS `ciud`,`s`.`fevto` AS `fevto`,`a`.`tipo` AS `tipo`,`a`.`dola` AS `dola`,ifnull(`c`.`ndoc`,'') AS `docd`,`a`.`nrou` AS `nrou`,`a`.`banco` AS `banco`,`a`.`idcred` AS `idcred`,`b`.`rcre_idau` AS `idauto`,`d`.`nomv` AS `nomv`,`a`.`ncontrol` AS `ncontrol`,`b`.`rcre_codv` AS `rcre_codv`,`a`.`estd` AS `estd`,`b`.`rcre_idrc` AS `rcre_idrc` from (((((`vpdtesventas` `s` join `fe_clie` `x` on((`x`.`idclie` = `s`.`rcre_idcl`))) join `fe_cred` `a` on((`a`.`idcred` = `s`.`ncontrol`))) join `fe_rcred` `b` on((`b`.`rcre_idrc` = `a`.`cred_idrc`))) left join `fe_rcom` `c` on((`c`.`idauto` = `b`.`rcre_idau`))) join `fe_vend` `d` on((`d`.`idven` = `b`.`rcre_codv`))) order by `s`.`fevto`,`c`.`ndoc` */;
...
/*View structure for view vpdtesventas */

/*!50001 DROP TABLE IF EXISTS `vpdtesventas` */;
/*!50001 DROP VIEW IF EXISTS `vpdtesventas` */;

/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vpdtesventas` AS select cast(sum((`d`.`impo` - `d`.`acta`)) as decimal(12,2)) AS `saldo`,`d`.`ncontrol` AS `ncontrol`,max(`d`.`fevto`) AS `fevto`,`r`.`rcre_idcl` AS `rcre_idcl`,`d`.`mone` AS `mone` from (`fe_rcred` `r` join `fe_cred` `d` on((`d`.`cred_idrc` = `r`.`rcre_idrc`))) where ((`d`.`acti` = 'A') and (`r`.`rcre_Acti` = 'A')) group by `r`.`rcre_idcl`,`d`.`ncontrol`,`d`.`mone` having (`saldo` <> 0) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vrcompras` AS select `c`.`ndoc` AS `ndoc`,`c`.`valor` AS `valor`,`c`.`igv` AS `igv`,`c`.`impo` AS `impo`,`c`.`pimpo` AS `pimpo`,`c`.`fech` AS `fech`,`c`.`fecr` AS `fecr`,`c`.`form` AS `form`,`c`.`exon` AS `exon`,`c`.`ndo2` AS `ndo2`,`c`.`idauto` AS `idauto`,`c`.`deta` AS `deta`,`c`.`tcom` AS `tcom`,`c`.`vigv` AS `vigv`,`c`.`idprov` AS `idprov`,`c`.`tdoc` AS `tdoc`,`c`.`dolar` AS `dolar`,`c`.`mone` AS `mone`,`p`.`razo` AS `razo`,`p`.`dire` AS `dire`,`p`.`ciud` AS `ciud`,`p`.`nruc` AS `nruc`,ifnull(`x`.`idcaja`,0) AS `Idcaja`,`c`.`codt` AS `codt`,`c`.`fusua` AS `fusua`,`w`.`nomb` AS `Usuario`,`c`.`rcom_vend` AS `rcom_vend` from (((`fe_rcom` `c` join `fe_prov` `p` on((`p`.`idprov` = `c`.`idprov`))) left join `fe_caja` `x` on((`x`.`idauto` = `c`.`idauto`))) join `fe_usua` `w` on((`w`.`idusua` = `c`.`idusua`))) where (`c`.`acti` <> 'I') */;
