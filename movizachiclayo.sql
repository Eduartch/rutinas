...
MySQL - 8.0.41 : Database - syscom_bdcmoviza
*********************************************************************
*/
...
nidcosto integer,ndolar float,tigv decimal(5,3),nper decimal(5,3)) RETURNS int
BEGIN
declare nid integer default 0;
...
np1 float,np2 float,np3 float,npeso float,ccat integer,cmar integer,ctipro char,nflete integer,cm char,cidpc varchar(45),
ncome float,ncomc float,nutil1 float,nutil2 float,nutil3 float,nidusua integer,nsmin float,nsmax float,
nidcosto integer,ndolar float,tigv decimal(5,3),nper decimal(5,3),ndetra decimal(5,3)) RETURNS int
BEGIN
declare nid integer default 0;
INSERT INTO fe_art(descri,unid,prec,cost,premay,premen,pre3,peso,idcat,idmar,tipro,idflete,tmon,fechc,idpc,prod_come,
prod_comc,prod_uti1,prod_uti2,prod_uti3,prod_idus,prod_smin,prod_smax,prod_idco,prod_dola,prod_tigv,prod_perc,prod_detr)
VALUES (cdesc,cunid,nprec,ncosto,np1,np2,np3,npeso,ccat,cmar,ctipro,nflete,cm,localtime,
cidpc,ncome,ncomc,nutil1,nutil2,nutil3,nidusua,nsmin,nsmax,nidcosto,ndolar,tigv,nper,ndetra);
select last_insert_id() into nid from fe_art group by last_insert_id();
return nid;
END */$$
DELIMITER ;
...
/*!50003 CREATE FUNCTION `FunIngresaDPedidos1`(ncoda integer,ncant float,nprec float,nidauto integer,nper decimal(5,2)) RETURNS int
BEGIN
declare id integer default 0;
...
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 integer,vcom float,perc decimal(5,2)) RETURNS int
BEGIN
declare nidk integer default 0;
if ct='C' then
  INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_idco,codv,kar_alma1)
...
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 integer,vcom float,ncoda1 integer) RETURNS int
BEGIN
declare nidk integer default 0;
if ct='C' then
...
cest char,cmon char,crefe varchar(60),dfecha date,dfevto date,
ctipo char,cdocp varchar(12),nimpo float,ninic float,
idven integer,nimpoo float,nidus integer,nidtda integer,cpc varchar(45),cforma char) RETURNS int
BEGIN
declare id integer default 0;
declare id1 integer default 0;
...
/*!50003 CREATE FUNCTION `FunverificaNombredeProducto`(cdescri varchar(100),nid integer) RETURNS int
BEGIN
declare sw integer;
DECLARE CONTINUE HANDLER FOR NOT FOUND SET sw=0;
if nid=0 then
...
/*!50003 CREATE FUNCTION `funingrecanjesvtas`(dfecha DATE,nimpo DECIMAL(12,2),nvtas DECIMAL(12,2),dfi DATE,dff DATE,nidus INTEGER) RETURNS int
BEGIN
DECLARE nid INTEGER;
...
ndetraccion DECIMAL(10,2),nicbper decimal(8,2),coddetra VARCHAR(10)) RETURNS int
BEGIN
DECLARE nid INTEGER;
SET nid=0;
INSERT INTO fe_correvtas(auto_corr)VALUES(CONCAT(ctdoc,cndoc));
...
INSERT INTO fe_husua(hisu_idus,hisu_fechain,hisu_idauto) VALUES (nidus,NOW(),nid);
return nid;
END */$$
DELIMITER ;
...
select last_insert_id() into nid from fe_rcom group by last_insert_id();
if n1>0 and n2>0 and n3>0 then
   call IngresaCuentasV(nv,nigv,nt,n1,n2,n3,"H","H","D",nid,npvta,0);
end if;
INSERT INTO fe_husua(hisu_idus,hisu_fechain,hisu_idauto) VALUES (nus,NOW(),nid);
return nid;
END */$$
...
n1 INTEGER,n2 INTEGER,n3 INTEGER,cdetalle1 VARCHAR(120),nidcanje INTEGER) RETURNS int
BEGIN
DECLARE nid INTEGER;
SET nid=0;
INSERT INTO fe_correvtas(auto_corr)VALUES(CONCAT(ctdoc,cndoc));
...
set nid=0;
if opt=0 then
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idprov,tipom,fusua,idusua,codt,rcom_nitem,rcom_idtr)
   ...
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,rcom_nitem,rcom_idtr)
...
caten varchar(80),cforma varchar(100),cplazo varchar(80),cvalidez varchar(80),centrega varchar(100),
cdetalle varchar(150),cmone char,cauto char,cincl char,sper char,agente char,ndias integer) RETURNS int
BEGIN
declare nid integer default 0;
...
select last_insert_id() into nid from fe_rped group by last_insert_id();
INSERT INTO fe_husua(hisu_idus,hisu_fechain,hisu_idrped) VALUES (cusua,NOW(),nid);
return nid;
END */$$
DELIMITER ;
...
if opt=0 then
   ...
   if ctg='T' then
         INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,rcom_reci,rcom_carg)
         VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,LOCALTIME,nus,nidcodt,'P',npvta);
      else
	INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,pimpo)
	VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,localtime,nus,nidcodt,npvta);
   end if;
   select last_insert_id() into nid from fe_rcom group by last_insert_id();
   if n1>0 and n2>0 and n3>0 then
...
np3 float,npeso float,ccat integer,cmar integer,ctipro char,nflete integer,cm char,nprecio float,
nidgrupo integer,nutil1 float,nutil2 float,nutil3 float,ncome float,
ncomc float,nidus integer,ncoda integer,nsmin float,nsmax float,nidcosto integer,ndolar float,cestado char,tigv decimal(5,3),nper decimal(6,4),ndetra decimal(6,4))
BEGIN
UPDATE fe_art SET descri=cdesc,unid=cunid,cost=ncosto,premay=np1,premen=np2,pre3=np3,peso=npeso,idcat=ccat,idmar=cmar,tipro=ctipro,idflete=nflete,tmon=cm,
prec=nprecio,prod_uti1=nutil1,prod_uti2=nutil2,prod_uti3=nutil3,
prod_come=ncome,prod_comc=ncomc,prod_uact=nidus,prod_fact=localtime,prod_smax=nsmax,
prod_smin=nsmin,prod_idco=nidcosto,prod_dola=ndolar,prod_acti=cestado,prod_tigv=tigv,
...
DECLARE cbuscar VARCHAR(80);
SET cbuscar=CONCAT('%',TRIM(abuscar),+'%');
SELECT idart,descri,unid,uno,dos,tre,cua,cin,premay AS pre1,premen AS pre2,pre3,
ROUND(IF(tmon='S',(a.prec*prod_tigv)+b.prec,(a.prec*prod_tigv*nd)+b.prec),2) AS costo,c.idgrupo,c.dcat,prod_dola,
...
INSERT INTO fe_husua(hisu_idus,hisu_fechain,hisu_idauto,hisu_est) VALUES (nu,NOW(),nid,'M');
END */$$
DELIMITER ;
...
INSERT INTO fe_husua(hisu_idus,hisu_fechain,hisu_idrped,hisu_est) VALUES (cusua,NOW(),nidauto,'M');
END */$$
DELIMITER ;
