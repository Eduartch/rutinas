...
MySQL - 10.3.39-MariaDB-log : Database - csysven_bdcmovi
*********************************************************************
*/
...
DECLARE cdeta VARCHAR(300) DEFAULT '';
CASE
...
/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `AnulapagosConNotasCredito` */$$

/*!50003 CREATE */ /*!50003 TRIGGER `AnulapagosConNotasCredito` AFTER UPDATE ON `fe_nccom` FOR EACH ROW begin
...
/*!50003 CREATE FUNCTION `FunCreaCLiente`(cruc varchar(11),crazo varchar(150),
cdire varchar(150),cciud varchar(150),cfono varchar(15),cfax varchar(15),cdni varchar (11),
...
/*!50003 CREATE FUNCTION `FuncreaProductos`(cdesc varchar(60),cunid varchar(4),nprec float,ncosto float,
...
nidcosto integer,ndolar float,tigv decimal(5,3),nper decimal(5,3)) RETURNS int(11)
BEGIN
declare nid integer default 0;
INSERT INTO fe_art(descri,unid,prec,cost,premay,premen,pre3,peso,idcat,idmar,tipro,idflete,tmon,fechc,idpc,prod_come,
prod_comc,prod_uti1,prod_uti2,prod_uti3,prod_idus,prod_smin,prod_smax,prod_idco,prod_dola,prod_tigv,prod_perc)
VALUES (cdesc,cunid,nprec,ncosto,np1,np2,np3,npeso,ccat,cmar,ctipro,nflete,cm,localtime,
cidpc,ncome,ncomc,nutil1,nutil2,nutil3,nidusua,nsmin,nsmax,nidcosto,ndolar,tigv,nper);
...
/* Function  structure for function  `FuncreaProductos11` */

/*!50003 DROP FUNCTION IF EXISTS `FuncreaProductos11` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FuncreaProductos11`(cdesc VARCHAR(60),cunid VARCHAR(4),nprec FLOAT,ncosto FLOAT,
np1 FLOAT,np2 FLOAT,np3 FLOAT,npeso FLOAT,ccat INTEGER,cmar INTEGER,ctipro CHAR,nflete INTEGER,cm CHAR,cidpc VARCHAR(45),
ncome FLOAT,ncomc FLOAT,nutil1 FLOAT,nutil2 FLOAT,nutil3 FLOAT,nidusua INTEGER,nsmin FLOAT,nsmax FLOAT,
ccodigo1 VARCHAR(15),ndolar FLOAT,tigv DECIMAL(5,3),nper DECIMAL(5,3),ndetra DECIMAL(5,3)) RETURNS int(11)
BEGIN
DECLARE nid INTEGER DEFAULT 0;
INSERT INTO fe_art(descri,unid,prec,cost,premay,premen,pre3,peso,idcat,idmar,tipro,idflete,tmon,fechc,idpc,prod_come,
prod_comc,prod_uti1,prod_uti2,prod_uti3,prod_idus,prod_smin,prod_smax,prod_dola,prod_tigv,prod_perc,prod_detr)
VALUES (cdesc,cunid,nprec,ncosto,np1,np2,np3,npeso,ccat,cmar,ctipro,nflete,cm,LOCALTIME,
cidpc,ncome,ncomc,nutil1,nutil2,nutil3,nidusua,nsmin,nsmax,ndolar,tigv,nper,ndetra);
SELECT LAST_INSERT_ID() INTO nid FROM fe_art GROUP BY LAST_INSERT_ID();
RETURN nid;
END */$$
DELIMITER ;
...
/*!50003 CREATE FUNCTION `FunCreaProveedor`(cruc varchar(11),crazo varchar(150),cdire varchar(150),cciud varchar(150),
...
set nvalor=0;
if ctipo='V' then
   SELECT cast(venta as decimal(6,4)) into nvalor FROM fe_mon WHERE fech=dfecha;
  else
   SELECT cast(valor as decimal(6,4)) into nvalor FROM fe_mon WHERE fech=dfecha;
end if;
return nvalor;
END */$$
DELIMITER ;
...
/* Function  structure for function  `funingrecanjesvtas` */

/*!50003 DROP FUNCTION IF EXISTS `funingrecanjesvtas` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `funingrecanjesvtas`(dfecha DATE,nimpo DECIMAL(12,2),nvtas DECIMAL(12,2),dfi DATE,dff DATE,nidus INTEGER) RETURNS int(11)
BEGIN
DECLARE nid INTEGER;
INSERT INTO fe_canjesvtas(canj_fech,canj_impo,canj_vtas,canj_feci,canj_fecf,canj_fope,canj_idus)
VALUES(dfecha,nimpo,nvtas,dfi,dff,LOCALTIME,nidus);
SELECT LAST_INSERT_ID() INTO nid FROM fe_canjesvtas GROUP BY LAST_INSERT_ID();
RETURN nid;
END */$$
DELIMITER ;
...
cndoc varchar(10),ctdoc varchar(2),nimpo float,cform char,cusua integer,cidpcped varchar(45),nidven integer,nidtienda integer,ctp char,
caten varchar(80),cforma varchar(100),cplazo varchar(80),cvalidez varchar(80),centrega varchar(80),
cdetalle varchar(150),cmone char,cauto char,cincl char,sper char,agente char,ndias integer) RETURNS int(11)
BEGIN
declare nid integer default 0;
INSERT INTO fe_rped(fech,idclie,ndoc,tdoc,impo,form,rped_idus,idpcped,fecho,idven,idtienda,
tipopedido,aten,forma,plazo,validez,entrega,
detalle,rped_mone,rped_auto,rped_incl,rped_sper,rped_agen,rped_dias)
VALUES(dfech,nidclie,cndoc,ctdoc,nimpo,cform,cusua,cidpcped,localtime,nidven,nidtienda,ctp,caten,cforma,
cplazo,cvalidez,centrega,cdetalle,cmone,cauto,cincl,sper,agente,ndias);
select last_insert_id() into nid from fe_rped group by last_insert_id();
return nid;
END */$$
DELIMITER ;
...
n1 integer,n2 integer,n3 integer,nitem integer,npvta decimal(12,2)) RETURNS int(11)
BEGIN
...
if opt=0 then
   ...
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idprov,tipom,fusua,idusua,codt,rcom_nitem,pimpo,rcom_reci)
   VALUES (ctdoc,cform,cndoc,dfecha,dfechar,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,localtime,nus,nidcodt,nitem,npvta,'P');
   ...
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,rcom_nitem,pimpo,rcom_reci)
   VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,localtime,nus,nidcodt,nitem,npvta,'P');
   select last_insert_id() into nid from fe_rcom group by last_insert_id();
   if n1>0 and n2>0 and n3>0 then
...
/* Function  structure for function  `FunIngresaCabeceraCVTraspaso` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaCabeceraCVTraspaso` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaCabeceraCVTraspaso`(
...
set nid=0;
if opt=0 then
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idprov,tipom,fusua,idusua,codt,rcom_nitem,rcom_idtr,rcom_reci)
   VALUES (ctdoc,cform,cndoc,dfecha,dfechar,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,localtime,nus,nidcodt,nitem,npvta,'P');
   select last_insert_id() into nid from fe_rcom group by last_insert_id();
  else
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,rcom_nitem,rcom_idtr,rcom_reci)
   VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,localtime,nus,nidcodt,nitem,npvta,'P');
   select last_insert_id() into nid from fe_rcom group by last_insert_id();
end if;
return nid;
...
/* Function  structure for function  `FunIngresaCabeceraVentasCanjeada` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaCabeceraVentasCanjeada` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaCabeceraVentasCanjeada`(
ctdoc varchar(2),cform char,cndoc varchar(12),dfecha date,dfechar date,cdetalle varchar(120),
nv float,nigv float,nt float,cndo2 varchar(10),cm char,
ndolar float,ni float,ctg char,ccodp integer,cmvto char,nus integer,opt integer,nidcodt integer,
n1 integer,n2 integer,n3 integer,nidautop integer,npvta decimal(12,2)) RETURNS int(11)
BEGIN
declare nid,idce,idve,nidctaper integer;
declare ctipo char;
set nid=0;
...
/* Function  structure for function  `FunIngresaCabeceravtascanjeado` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaCabeceravtascanjeado` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaCabeceravtascanjeado`(
ctdoc VARCHAR(2),cform CHAR,cndoc VARCHAR(12),dfecha DATE,dfechar DATE,cdetalle VARCHAR(120),
...
n1 INTEGER,n2 INTEGER,n3 INTEGER,cdetalle1 VARCHAR(120),nidcanje INTEGER) RETURNS int(11)
BEGIN
DECLARE nid INTEGER;
SET nid=0;
INSERT INTO fe_correvtas(auto_corr)VALUES(CONCAT(ctdoc,cndoc));
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,rcom_idtr,rcom_entr)
   VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,LOCALTIME,nus,nidcodt,nidcanje,'C');
   SELECT LAST_INSERT_ID() INTO nid FROM fe_rcom GROUP BY LAST_INSERT_ID();
...
begin
declare id integer;
declare fopex datetime;
SELECT DATE_SUB(NOW(), INTERVAL 2 HOUR) INTO fopex;
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,lcaj_idus,lcaj_clpr,lcaj_fope,lcaj_form)values
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidcp,fopex,'E');
...
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidcp,nidauto,fopex);
SELECT LAST_INSERT_ID() INTO id FROM fe_lcaja GROUP BY LAST_INSERT_ID();
RETURN id;
END */$$
DELIMITER ;
...
/*!50003 CREATE FUNCTION `FunIngresaDatosLcajaECreditos`(dfecha DATE,cndoc VARCHAR(12),cdeta VARCHAR(120),idcta INTEGER,sdeudor DECIMAL(12,2),
sacreedor DECIMAL(12,2),cmone CHAR,ndolar DECIMAL(5,3),nidus INTEGER,nidcp INTEGER,nidauto INTEGER,cform CHAR,cdcto varCHAR(20)) RETURNS int(11)
BEGIN
DECLARE nid INTEGER;
DECLARE fopex DATETIME;
INSERT INTO fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
...
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidcp,nidauto,cform,fopex,cdcto);
SELECT LAST_INSERT_ID() INTO nid FROM fe_lcaja GROUP BY LAST_INSERT_ID();
RETURN nid;
END */$$
DELIMITER ;
...
n1 INTEGER,n2 INTEGER,n3 INTEGER,nvend INTEGER,nperc decimal(10,2),nexon DECIMAL(12,2),ndscto DECIMAL(12,2),
ndetraccion DECIMAL(10,2),nicbper decimal(8,2),coddetra VARCHAR(10)) RETURNS int(11)
BEGIN
DECLARE nid INTEGER;
SET nid=0;
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idcliente,
   tipom,fusua,idusua,codt,rcom_exon,rcom_vend,rcom_mdet,rcom_icbper,rcom_detr,pimpo)
   VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,'V',LOCALTIME,nus,nidcodt,
   nexon,nvend,ndetraccion,nicbper,coddetra,nperc);
   SELECT LAST_INSERT_ID() INTO nid FROM fe_rcom GROUP BY LAST_INSERT_ID();
 IF n1>0 AND n2>0 AND n3>0 THEN
    IF nexon>0 THEN
...
/* Function  structure for function  `FunIngresaDPedidos1` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaDPedidos1` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaDPedidos1`(ncoda integer,ncant float,nprec float,nidauto integer,nper decimal(5,2)) RETURNS int(11)
BEGIN
declare id integer default 0;
INSERT INTO fe_ped(idart,cant,prec,idautop,pedi_perc)
VALUES(ncoda,ncant,nprec,nidauto,nper);
set id=id+10;
return id;
END */$$
DELIMITER ;
...
BEGIN
declare id integer default 0;
update fe_guiac set guic_acti='I' where guic_idka=nidkar;
...
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 integer,vcom float,perc decimal(5,2)) RETURNS int(11)
BEGIN
declare nidk integer default 0;
if ct='C' then
  INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_idco,codv,kar_alma1)
  ...
  INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_idco,codv,kar_alma1,kar_comi,kar_perc)
  VALUES (nid,cc,ct,npr,nct,tmvto,cincl,calma,nidcosto1,ccodv,0,vcom,perc);
end if;
select last_insert_id() into nidk from fe_kar group by last_insert_id();
return nidk;
END */$$
...
/* Function  structure for function  `FunIngresaKardextr` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaKardextr` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaKardextr`(nid integer,cc integer,ct char,npr float,
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 integer,vcom float,ncoda1 integer) RETURNS int(11)
BEGIN
declare nidk integer default 0;
if ct='C' then
  INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_idco,codv,kar_alma1,kar_coda)
  VALUES (nid,cc,ct,npr,nct,tmvto,cincl,calma,nidcosto1,ccodv,calma,ncoda1);
 else
  INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_idco,codv,kar_alma1,kar_comi,kar_coda)
  VALUES (nid,cc,ct,npr,nct,tmvto,cincl,calma,nidcosto1,ccodv,0,vcom,ncoda1);
end if;
select last_insert_id() into nidk from fe_kar group by last_insert_id();
return nidk;
END */$$
...
cndoc varchar(12),nacta float,cesta char,cmone char,cb1 varchar(120),dfech date,
dfevto date,ctipo char,nctrl integer,cnrou varchar(40),nidrc float,cpc varchar(45),
...
select gene_idce into idce from fe_gene where idgene=1;
   ...
   VALUES (ctdoc,cform,cndoc,dfecha,dfechar,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,localtime,nus,nidcodt,nitem,nexon,npvta);
   select last_insert_id() into nid from fe_rcom group by last_insert_id();
   if n1>0 and n2>0 and n3>0 then
      call IngresaCuentas(nv,0,0,0,nigv,0,0,nt,n1,0,0,0,n2,0,0,n3,"D","","","","D","","","H",nid);
   end if;
   if idce>0 and cform='E' then
      Call ProIngresaDatosLcajaE1(dfecha,"",concat("Compra ef. ",cndoc),idce,0,nt,cm,ndolar,nus,ccodp,nid);
   end if;
return nid;
END */$$
DELIMITER ;
...
/* Function  structure for function  `FunRegistraCreditos1` */

/*!50003 DROP FUNCTION IF EXISTS `FunRegistraCreditos1` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunRegistraCreditos1`(nauto integer,nid integer,cndoc varchar(12),
cest char,cmon char,crefe varchar(60),dfecha date,dfevto date,
ctipo char,cdocp varchar(12),nimpo float,ninic float,
idven integer,nimpoo float,nidus integer,nidtda integer,cpc varchar(45),cforma char) RETURNS int(11)
BEGIN
declare id integer default 0;
declare id1 integer default 0;
insert into fe_rcred(rcre_idcl,rcre_fech,rcre_idau,rcre_impc,rcre_idus,rcre_codt,rcre_idpc,rcre_inic,rcre_codv,rcre_form)
values(nid,dfecha,nauto,nimpoo,nidus,nidtda,cpc,ninic,idven,cforma);
select last_insert_id() into id1 from fe_rcred group by last_insert_id();
INSERT INTO fe_cred(fech,fevto,impo,ndoc,estd,mone,banc,tipo,cred_idrc,cred_idus,cred_fope)
values(dfecha,dfevto,nimpo,cndoc,cest,cmon,crefe,ctipo,id1,nidus,current_date());
select last_insert_id() into id from fe_cred group by last_insert_id();
...
/*!50003 CREATE FUNCTION `FunSiestaRegistradoDctoPago`(cdcto varchar(15)) RETURNS int(11)
begin
declare vdvto integer default 0;
...
declare id integer;
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
lcaj_idus,lcaj_clpr,lcaj_tran,lcaj_form,lcaj_fope)values
...
SELECT DATEDIFF(CURDATE(),MAX(a.fech)) INTO ndias FROM
...
ORDER BY DATEDIFF(CURDATE(),MAX(a.fech)) DESC LIMIT 0,1;
...
/*!50003 CREATE FUNCTION `FunverificaNombredeProducto`(cdescri varchar(100),nid integer) RETURNS int(11)
BEGIN
declare sw integer;
DECLARE CONTINUE HANDLER FOR NOT FOUND SET sw=0;
if nid=0 then
...
WHERE TRIM(a.ndoc)=TRIM(cd) AND a.acti='A' AND b.acti='A' LIMIT 1;
IF vdvto>0 THEN
  RETURN 0;
 ELSE
   RETURN 1;
...
WHERE b.acti<>'I' GROUP BY  idart,alma) AS a WHERE (a.tcompras-a.tventas)<>0 ;
...
ctdoc VARCHAR(2),cform CHAR,cndoc VARCHAR(12),dfecha DATE,cdetalle VARCHAR(120),
nv DECIMAL(12,2),nigv DECIMAL(12,2),nt DECIMAL(12,2),cndo2 VARCHAR(15),cm CHAR,
ndolar FLOAT,ni FLOAT,ctg CHAR,ccodp INTEGER,ndetraccion DECIMAL(10,2),nus INTEGER,nicbper DECIMAL(5,2),nidcodt INTEGER,
n1 INTEGER,n2 INTEGER,n3 INTEGER,nexon DECIMAL(12,2),nperc DECIMAL(8,2),nidauto INTEGER,nidven INTEGER,coddetra VARCHAR(10))
BEGIN
...
mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom='V',idusua1=nus,codt=nidcodt,rcom_exon=nexon,rcom_mdet=ndetraccion,rcom_detr=coddetra, 
rcom_icbper=nicbper,pimpo=nperc  WHERE idauto=nidauto;
CALL ProDesactivaLcajaE(nidauto);
IF n1>0 AND n2>0 AND n3>0 THEN
     ...
     IF @i1>0 THEN
        CALL ProActualizaCuentasV(nv,nigv,nt,n1,n2,n3,@i1,@i2,@i3,"H","H","D",0,0,@i4);
     ELSE
       IF ctdoc<>'20' THEN
          CALL IngresaCuentasV(nv,nigv,nt,n1,n2,n3,"H","H","D",nidauto,0,0);
      END IF;
     END IF;
END IF;
END */$$
...
BEGIN
update fe_rcom set tdoc=ctdoc,form=cform,ndoc=cndoc,fech=dfecha,fecr=dfechar,deta=cdetalle,valor=nv,igv=nigv,impo=nt,ndo2=cndo2,
mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,rcom_canj=nidguia where idauto=nidauto;
call ProDesactivaLcajaE(nidauto);
if n1>0 and n2>0 and n3>0 then
...
/*!50003 CREATE PROCEDURE `ProActualizaCliente`(nid integer,cruc varchar(11),crazo varchar(150),
cdire varchar(150),cciud varchar(150),cfono varchar(15),cfax varchar(15),cdni varchar (11),
...
caten varchar(80),cforma varchar(80),cplazo varchar(80),cvalidez varchar(80),centrega varchar(80),cdetalle varchar(150),
cmone char,nidauto integer,cincl char,sper char,agente char)
BEGIN
UPDATE fe_rped SET fech=dfech,idclie=nidclie,ndoc=cndoc,impo=nimpo,form=cform,idven=nidven,facturado='N',tdoc=ctdoc,
tipopedido='P',aten=caten,forma=cforma,plazo=cplazo,validez=cvalidez,entrega=centrega,detalle=cdetalle,rped_incl=cincl,
rped_sper=sper,rped_agen=agente  WHERE idautop=nidauto;
END */$$
DELIMITER ;
...
cndoc VARCHAR(10),ctdoc VARCHAR(2),nimpo FLOAT,cform CHAR,cusua INTEGER,nidven INTEGER,nidtienda INTEGER,ctp CHAR,
caten VARCHAR(80),cforma VARCHAR(100),cplazo VARCHAR(80),cvalidez VARCHAR(80),centrega VARCHAR(80),
cdetalle VARCHAR(150),cmone CHAR,cauto CHAR,nidauto INTEGER,cincl CHAR,sper CHAR,agente CHAR,ndias INTeger)
BEGIN
UPDATE fe_rped SET fech=dfech,idclie=nidclie,ndoc=cndoc,impo=nimpo,form=cform,idven=nidven,facturado='N',
tdoc=ctdoc,rped_auto=cauto,
tipopedido='P',aten=caten,forma=cforma,plazo=cplazo,validez=cvalidez,entrega=centrega,detalle=cdetalle,
rped_incl=cincl,
rped_sper=sper,rped_agen=agente,rped_dias=ndias  WHERE idautop=nidauto;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProActualizaDetallePedidos1` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizaDetallePedidos1` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizaDetallePedidos1`(ncoda integer,ncant float,nprec float,nr integer,ctipoa char,
nper decimal(5,2))
BEGIN
if ctipoa='A' then
     UPDATE fe_ped SET acti='I' WHERE idped=nr;
   else
    UPDATE fe_ped SET idart=ncoda,cant=ncant,prec=nprec,pedi_perc=nper WHERE idped=nr;
end if;
END */$$
DELIMITER ;
...
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 integer,nidkar integer,op integer,xcom float,nper decimal(5,2))
BEGIN
if op=0 then
  Update fe_kar set Acti='I'  where idkar=nidkar;
 else
  Update fe_kar set
  idauto=nid,idart=cc,tipo=ct,prec=npr,cant=nct,ttip=tmvto,incl=cincl,
  alma=calma,kar_idco=nidcosto1,codv=ccodv,kar_comi=xcom,kar_perc=nper where idkar=nidkar;
end if;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProActualizaNroTraspaso` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizaNroTraspaso` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizaNroTraspaso`(nidauto1 integer,nidauto2 integer)
begin
update fe_rcom set rcom_idtr=niduto2 where idauto=nidauto1;
end */$$
DELIMITER ;
...
cc integer,dfe date,npr float,cnd integer,idp integer,cmda char,ni decimal(5,3),ndolar float,nidcosto integer)
BEGIN
...
if npr>0 then
	...
	IF @ufc<=dfe then
	   ...
	   set costor=round((npr*ni)+@ncostof,2);
	   ...
	   prod_idco=nidcosto,prod_dola=ndolar,prod_tigv=ni WHERE idart=cc;
	end if;
end if;
END */$$
DELIMITER ;
...
ncomc float,nidus integer,ncoda integer,nsmin float,nsmax float,nidcosto integer,ndolar float,cestado char,tigv decimal(5,3),nper decimal(6,4))
BEGIN
...
prod_smin=nsmin,prod_idco=nidcosto,prod_dola=ndolar,prod_acti=cestado,prod_tigv=tigv,prod_perc=nper  WHERE idart=ncoda;
END */$$
DELIMITER ;

/* Procedure structure for procedure `ProActualizaProductos11` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizaProductos11` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizaProductos11`(cdesc varchar(60),cunid varchar(4),ncosto float,np1 float,np2 float,
np3 float,npeso float,ccat integer,cmar integer,ctipro char,nflete integer,cm char,nprecio float,
nidgrupo integer,nutil1 float,nutil2 float,nutil3 float,ncome float,
ncomc float,nidus integer,ncoda integer,nsmin float,nsmax float,ccodigo varchar(15),ndolar float,cestado char,tigv decimal(5,3),nper decimal(6,4),ndetra decimal(6,4))
BEGIN
UPDATE fe_art SET descri=cdesc,unid=cunid,cost=ncosto,premay=np1,premen=np2,pre3=np3,peso=npeso,idcat=ccat,idmar=cmar,tipro=ctipro,idflete=nflete,tmon=cm,
prec=nprecio,prod_uti1=nutil1,prod_uti2=nutil2,prod_uti3=nutil3,
prod_come=ncome,prod_comc=ncomc,prod_uact=nidus,prod_fact=localtime,prod_smax=nsmax,
prod_smin=nsmin,prod_dola=ndolar,prod_acti=cestado,prod_tigv=tigv,
prod_perc=nper,prod_detr=ndetra  WHERE idart=ncoda;
END */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProActualizaProveedor`(nid integer,cruc varchar(11),crazo varchar(150),
cdire varchar(150),cciud varchar(150),cfono varchar(15),cfax varchar(15),
...
/* Procedure structure for procedure `ProAnulaOTraspasos` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProAnulaOTraspasos` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProAnulaOTraspasos`(nid integer,nu integer,nu1 integer)
begin
...
if nidauto=0 then
   ...
   set @cn1:=cndoc;
   set @df:=dfecha;
   select @idclave:=idauto,@nidcodt:=codt,@tcom:=tcom from fe_rcom where tdoc=ctdoc and ndoc=cndoc and tipom =ctipo;
   set nid=@idclave;
  else
   set nid=nidauto;
   select @ct:=tdoc,@cn1:=ndoc,@df:=fech,@nidcodt:=codt,@tcom:=tcom from fe_rcom where idauto=nid;
end if;
if nid>0 then
   update fe_rcom set acti='I',idusua1=nu,rcom_idus=nu1 where idauto=nid;
    if ctipo='V' then
      ...
      select @nguia:=guia_idgui from fe_guias where guia_idau=nid;
       ...
       update fe_rgcompra set rgco_acti='I' where rgco_idau=nid;
   end if;
   if @tcom='T' then
        SELECT @nguia:=guia_idgui FROM fe_guias WHERE guia_idau=nid;
...
update fe_lcaja set lcaj_acti='I' where lcaj_idcr=nid;
END */$$
DELIMITER ;
...
BEGIN
select if(tipro='S',3500,uno) as uno,if(tipro='S',3500,dos) as dos,if(tipro='S',3500,tre) as tre,
if(tipro='S',3500,cua) as cua,IF(tipro='S',3500,cin) as cin,IF(tipro='S',3500,sei) AS sei  from fe_art where idart=nidart;
END */$$
DELIMITER ;
...
cdcto VARCHAR(20),ctdoc VARCHAR(2),nidtda INTEGER)
BEGIN
INSERT INTO fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
...
INSERT INTO fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
lcaj_idus,lcaj_clpr,lcaj_idau,lcaj_form,lcaj_fope,lcaj_dcto,lcaj_tdoc,lcaj_codt)VALUES
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidcp,nidauto,cform,fopex,cdcto,ctdoc,nidtda);
END */$$
DELIMITER ;
...
BEGIN
DECLARE fopex DATETIME;
SELECT DATE_SUB(NOW(), INTERVAL 1 HOUR) INTO fopex;
INSERT INTO fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
lcaj_idus,lcaj_clpr,lcaj_idau,lcaj_form,lcaj_dcto,lcaj_tdoc,lcaj_codt,lcaj_fope)VALUES
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidcp,nidauto,cform,cdcto,ctdoc,nidtda,fopex);
END */$$
DELIMITER ;
...
BEGIN
SELECT nomb,idalma,dire,ciud,sucuidserie,ubigeo FROM fe_sucu where idalma in(1,4,5,6)  order by idalma;
END */$$
DELIMITER ;
...
if opt=0 then
   ...
   clie_feac,ifnull(c.zona_nomb,'') as Zona,ifnull(a.clie_idzo,0) as clie_idzo,clie_rete
...
/* Procedure structure for procedure `ProMuestraProducto` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProMuestraProducto` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProMuestraProducto`(nid integer)
begin
select idart,descri FROM fe_art  where idart=nid;
end */$$
DELIMITER ;
...
DECLARE cbuscar VARCHAR(80);
SET cbuscar=CONCAT('%',TRIM(abuscar),+'%');
SELECT idart,descri,unid,uno,dos,tre,cua,cin,sei,premay AS pre1,premen AS pre2,pre3,
ROUND(IF(tmon='S',(a.prec*prod_tigv)+b.prec,(a.prec*prod_tigv*nd)+b.prec),2) AS costo,c.idgrupo,c.dcat,prod_dola,
ROUND(IF(tmon='S',(a.prec*prod_tigv),(a.prec*prod_tigv*nd)),2) AS costosf,b.prec AS flete,prod_tigv,
IFNULL(d.cost_cost,0) AS costor,IFNULL(d.cost_prec,0) AS precr,IFNULL(d.cost_mone,'')  AS moner,
CAST(IFNULL(d.cost_idco,0) AS UNSIGNED) AS cost_idco,IFNULL(d.cost_flet,0)  AS fleter,IFNULL(d.cost_dola,0) AS dolar,
peso,a.prec,tipro,idmar,a.idcat,cost,tmon,a.idflete,prod_uti1,prod_uti2,prod_uti3,prod_idus,sei,
prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,IFNULL(o.razo,'') AS proveedor,
IFNULL(yy.ndoc,'') AS ndoc,IFNULL(yy.fech,'') AS fech,ulfc,prod_perc,prod_icbper,IFNULL(usc.nomb,'') AS usuacreo,
IFNULL(usm.nomb,'') AS usuamodifico,prod_detr
FROM fe_art  AS a 
...
LEFT JOIN fe_usua AS usc ON usc.idusua=a.prod_idus
LEFT JOIN fe_usua AS usm ON usm.idusua=a.prod_uact
...
peso,a.prec,tipro,idmar,a.idcat,cost,tmon,a.idflete,prod_uti1,prod_uti2,prod_uti3,prod_idus,
prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,ifnull(o.razo,'') as proveedor,
ifnull(yy.ndoc,'') as ndoc,ifnull(yy.fech,'') as fech,ulfc,prod_perc,prod_detr,ifnull(usc.nomb,'') as usuacreo,
ifnull(usm.nomb,'') as usuamodifico
FROM fe_art  as a 
inner join fe_fletes as b on(b.idflete=a.idflete)
inner join fe_cat as c on(c.idcat=a.idcat) 
left join fe_costos as d on(d.cost_idco=a.prod_idco)
...
set cbuscar=concat('%',trim(cbusca),+'%');
if opt=0 then
   select nomb,tipo,activo,idusua,0 as Uno,1 as uno,2 as dos,3 as tres,4 as cuatro,5 as cinco,6 as seis,0 as cero,idalma,clave from fe_usua where nomb like cbuscar order by nomb;
end if;
if opt=1 then
   select nomb,tipo,activo,idusua,0 as Uno,1 as uno,2 as dos,3 as tres,4 AS cuatro,5 AS cinco,6 AS seis,0 as cero,idalma,clave from fe_usua where idusua=nid order by nomb;
end if;
END */$$
DELIMITER ;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vmuestraventas` AS select `c`.`idusua` AS `idusua`,`a`.`kar_comi` AS `kar_comi`,`a`.`codv` AS `codv`,`a`.`idauto` AS `idauto`,`c`.`codt` AS `alma`,`a`.`kar_perc` AS `kar_perc`,`a`.`kar_idco` AS `idcosto`,`a`.`idkar` AS `idkar`,`a`.`idart` AS `Coda`,`a`.`cant` AS `cant`,`a`.`prec` AS `prec`,`c`.`valor` AS `valor`,`c`.`igv` AS `igv`,`c`.`impo` AS `impo`,`c`.`fech` AS `fech`,`c`.`fecr` AS `fecr`,`c`.`form` AS `form`,`c`.`deta` AS `deta`,`c`.`exon` AS `exon`,`c`.`ndo2` AS `ndo2`,`c`.`rcom_entr` AS `rcom_entr`,`c`.`idcliente` AS `idclie`,`d`.`razo` AS `razo`,`d`.`nruc` AS `nruc`,`d`.`dire` AS `dire`,`d`.`ciud` AS `ciud`,`d`.`ndni` AS `ndni`,`a`.`tipo` AS `tipo`,`c`.`tdoc` AS `tdoc`,`c`.`ndoc` AS `ndoc`,`c`.`dolar` AS `dolar`,`c`.`mone` AS `mone`,`b`.`descri` AS `descri`,0 AS `idcaja`,`b`.`unid` AS `unid`,`b`.`premay` AS `pre1`,`b`.`peso` AS `peso`,`b`.`premen` AS `pre2`,cast(0 as decimal(4,2)) AS `nidrv`,`c`.`vigv` AS `vigv`,`a`.`dsnc` AS `dsnc`,`a`.`dsnd` AS `dsnd`,`a`.`gast` AS `gast`,`c`.`idcliente` AS `idcliente`,`c`.`codt` AS `codt`,`b`.`pre3` AS `pre3`,`b`.`cost` AS `costo`,`b`.`uno` AS `uno`,`b`.`dos` AS `dos`,`b`.`uno` + `b`.`dos` AS `TAlma`,`c`.`fusua` AS `fusua`,`p`.`nomv` AS `Vendedor`,`q`.`nomb` AS `Usuario`,`c`.`rcom_icbper` AS `rcom_icbper`,`a`.`kar_icbper` AS `kar_icbper`,`c`.`rcom_mens` AS `rcom_mens` from (((((`fe_art` `b` join `fe_kar` `a` on(`a`.`idart` = `b`.`idart`)) join `fe_rcom` `c` on(`a`.`idauto` = `c`.`idauto`)) join `fe_clie` `d` on(`c`.`idcliente` = `d`.`idclie`)) join `fe_vend` `p` on(`p`.`idven` = `a`.`codv`)) join `fe_usua` `q` on(`q`.`idusua` = `c`.`idusua`)) where `c`.`acti` <> 'I' and `a`.`acti` <> 'I' */;
