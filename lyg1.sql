...
/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `AnulaPagosRetencion` */$$

/*!50003 CREATE */ /*!50003 TRIGGER `AnulaPagosRetencion` AFTER UPDATE ON `fe_dret` FOR EACH ROW BEGIN
...
select last_insert_id() into nid from fe_rped group by last_insert_id();
INSERT INTO fe_husua(hisu_idus,hisu_fechain,hisu_idrped) VALUES (nidus,NOW(),nid);
return nid;
END */$$
DELIMITER ;
...
INSERT INTO fe_husua(hisu_idus,hisu_fechain,hisu_idauto) VALUES (nidus,NOW(),id);
RETURN id;
END */$$
DELIMITER ;
...
open cursor1;
start transaction;
UPDATE fe_art SET uno=0,dos=0,tre=0,cua=0,cin=0,sei=0,sie=0,och=0,nue=0;
repeat
...
ncomc float,nidus integer,ncoda integer,nsmax float, nsmin float,ccodigo1 varchar(20),ndolar float,ce char,nutil0 decimal(10,6),
nctoferta decimal(10,2),nmin1 decimal(10,2),nmax1 decimal(10,2),nmin2 decimal(10,2),nmax2 decimal(10,2),
...
BEGIN
select uno,dos,tre,cua,cin,sei,sie,och,nue,uno+dos+tre+cua+cin+sei+sie+och+nue as tstock from fe_art where idart=nidart;
END */$$
DELIMITER ;
...
BEGIN
SELECT nomb,idalma,dire,ciud,sucuidserie,ubigeo,sucu_idus FROM fe_sucu where idalma in(1,2,3,4,9) ORDER BY idalma;
END */$$
DELIMITER ;
...
declare cbuscar varchar(80);
set cbuscar=concat('%',trim(abuscar),+'%');
SELECT idart,descri,unid,uno,dos,tre,cua,cero,nue,
...
ifnull(yy.ndoc,'') as ndoc,ifnull(yy.fech,'') as fech,ulfc
FROM fe_art  as a 
...
CASE opt
WHEN 1 THEN
    SELECT idart,descri,unid,uno,dos,tre,cua,cero,cin,sei,sie,och,nue,
...
if opt=1 then
    ...
    If(a.prod_uti0>1,Round(If(tmon='S',Round(a.Prec*v.igv+b.Prec,2)*prod_uti0,((a.prec*v.igv*nd)+b.prec)*prod_uti0),2),0) As pre0,
    round(if(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*if(prod_dola>nd,prod_dola,nd))+b.prec),2) as costo,c.idgrupo,c.dcat,prod_dola,
    round(if(tmon='S',(a.prec*v.igv),(a.prec*v.igv*nd)),2) as costosf,b.prec as flete,m.dmar,
    mid(descri,1,length(trim(descri))-length(trim(m.dmar))) as descri1,prod_cod1,
    cast(0 as unsigned) as costor,cast(0 as unsigned) as precr,''  as moner,
...
WHEN 1 THEN
    ...
    g.desgrupo AS grupo,b.desflete,u.nomb AS usuacreo,us.nomb AS usuamodifico,prod_depr,prod_ocan,prod_uti0,prod_ocom,
    prod_ubi1,prod_ubi2,prod_ubi3,prod_ubi4,prod_ubi5,prod_codb,prod_cmay,prod_umin,prod_umax,prod_dmin,prod_dmax,prod_tmin,prod_tmax
    FROM fe_art  AS a 
    INNER JOIN fe_fletes AS b ON(b.idflete=a.idflete)
    INNER JOIN fe_mar AS m ON m.idmar=a.idmar
