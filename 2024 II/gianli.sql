...
MySQL - 8.0.39 : Database - syscom_bdgianli3
*********************************************************************
*/
...
IF new.acti='I' THEN
 ...
 ELSE
   if (new.idusua1<>old.idusua1) then   
...
ncosto decimal(12,6),nmargen decimal(8,2),cmoneda char(1),cestilo char(1),ncomi decimal(6,3),lista integer) RETURNS int
BEGIN
...
set ntdoc=0;
set nid=0;
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,rcom_otro,pimpo,rcom_vend)
   VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,localtime,nus,nidcodt,ngrati,npvta,nidven);
   select last_insert_id() into nid from fe_rcom group by last_insert_id();
   if n1>0 and n2>0 and n3>0 then
      call IngresaCuentasV(nv,nigv,nt,n1,n2,n3,"H","H","D",nid,npvta,nidctaper);
   end if;
...
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidcp,nidauto,cform,LOCALTIME,cdcto,1);
SELECT LAST_INSERT_ID() INTO nid FROM fe_lcaja GROUP BY LAST_INSERT_ID();
RETURN nid;
END */$$
DELIMITER ;
...
BEGIN
declare id integer default 0;
INSERT INTO fe_ped(idart,cant,prec,idautop,dped_unid,dped_equi,dped_posi,dped_idep,dped_comi,dped_epta,dped_cost)
VALUES(ncoda,ncant,nprec,nidauto,cunid,nequi,npos,idepta,comi,idepta,ncosto);
select last_insert_id() into id from fe_ped group by last_insert_id();
return id;
END */$$
DELIMITER ;
...
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 integer,vcom float,nequi decimal(12,8),
cuni varchar(30),epta integer,npos integer,ncosto decimal(12,4),nigv decimal(6,4),nidt integer) RETURNS int
BEGIN
declare nidk integer default 0;
if ct='C' then
  INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_idco,codv,kar_alma1,kar_equi,kar_unid,kar_epta,kar_posi)
...
if opt=0 then
   ...
   if n1>0 and n2>0 and n3>0 then
      select @i1:=sum(xx.nid1),@i5:=sum(xx.nid2),@i6:=sum(xx.nid3),@i8:=sum(xx.nid4)
      from (select
...
declare idve,nidctaper integer;
    update fe_rcom set tdoc=ctdoc,form=cform,ndoc=cndoc,fech=dfecha,fecr=dfechar,deta=cdetalle,valor=nv,igv=nigv,impo=nt,ndo2=cndo2,
   mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,rcom_arch="",
...
/* Procedure structure for procedure `ProActualizaCosrosProductoEpta` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizaCosrosProductoEpta` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizaCosrosProductoEpta`(
cc integer,npr decimal(12,3),cmda char,dfecha date,nidusua integer,idepta integer)
BEGIN
   UPDATE fe_art SET prod_uact=nidusua,prod_fact=dfecha WHERE idart=cc;
   update fe_epta set epta_cost=npr,epta_mone=cmda where epta_idep=idepta;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProActualizaCosTosProductoEpta` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizaCosTosProductoEpta` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizaCosTosProductoEpta`(
cc integer,npr decimal(12,3),cmda char,dfecha date,nidusua integer)
BEGIN
   declare vigv decimal(6,4) default 0;
   select igv into vigv from fe_gene where idgene=1;
   UPDATE fe_art SET prod_uact=nidusua,prod_fact=dfecha,prec=npr/vigv WHERE idart=cc;
   update fe_epta as e  set epta_cost=e.epta_cant*(npr),epta_mone=cmda where epta_idar=cc and epta_acti='A';
END */$$
DELIMITER ;
...
cmone char,nidauto integer,vigv decimal(7,4),cgarantia varchar(120))
BEGIN
...
ncant decimal(10,2),idepta integer,opt integer,ncosto decimal(12,6),nmargen decimal(8,4),cmoneda char(1),
...
SELECT a.pres_desc,if(epta_mone='S',b.epta_cost+g.prec,b.epta_cost+(g.prec/nd)) as epta_cost,b.epta_marg,
round(if(b.epta_cost>0,if(b.epta_mone='S',if(b.epta_esti='M',
round((b.epta_cost+g.prec)*((b.epta_marg/100)+1),2),round((b.epta_cost+g.prec)*((b.epta_marg/100)+1),2)),
if(b.epta_esti='M',
round(((b.epta_cost*nd)+g.prec)*((b.epta_marg/100)+1),2),
round(((b.epta_cost*nd)+g.prec)*((b.epta_marg/100)+1),2))),b.epta_prec),2) as epta_prec,
round(epta_comi*100,3) as epta_comi,b.epta_cant,
if(b.epta_mone='S',round(b.epta_cost/((100-nu)/100),2),round((b.epta_cost*nd)/((100-nu)/100),2)) as precio1,
if(b.epta_mone='S',(b.epta_cost+g.prec),round((b.epta_cost+g.prec)*nd,2)) as costo,
b.epta_pres,b.epta_idar,b.epta_idep,b.epta_mone,b.epta_esti,b.epta_list
FROM fe_epta as b  LEFT JOIN fe_presentaciones as a  ON b.epta_pres=a.pres_idpr
inner join fe_art as f on f.idart=b.epta_idar inner join fe_fletes as g on g.idflete=f.idflete
...
if(b.epta_mone='S',if(b.epta_esti='M',round((b.epta_cost+(f.prec*b.epta_cant))*((b.epta_marg/100)+1),2),round((b.epta_cost+(f.prec*b.epta_cant))*((b.epta_marg/100)+1),2)),
if(b.epta_esti='M',round(((b.epta_cost*nd)+(f.prec*b.epta_cant))*((b.epta_marg/100)+1),2),round(((b.epta_cost*nd)+(f.prec*b.epta_cant))*((b.epta_marg/100)+1),2))),b.epta_prec) as epta_prec,
if(b.epta_mone='S',round(b.epta_cost/((100-v.igv)/100),2),round((b.epta_cost*nd)/((100-v.igv)/100),2)) as precio1,
if(b.epta_mone='S',b.epta_cost+(f.prec*b.epta_cant),round(b.epta_cost*nd,2)+f.prec) as costo,
b.epta_cant,b.epta_idar,b.epta_idep,b.epta_esti,epta_comi FROM fe_epta  as b inner join fe_presentaciones as a
on a.pres_idpr=b.epta_pres inner join fe_art as p on p.idart=b.epta_idar inner join fe_fletes as f on f.idflete=p.idflete ,fe_gene as v
where b.epta_acti='A' and b.epta_idar=nidart order by b.epta_prec desc;
END */$$
DELIMITER ;
...
IF opt=1 THEN
    ...
    prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,IFNULL(o.razo,'') AS proveedor,
    IFNULL(yy.ndoc,'') AS ndoc,IFNULL(yy.fech,'') AS fech,ulfc,prod_ent1,prod_ent2,
    g.desgrupo AS grupo,b.desflete,u.nomb AS usuacreo,us.nomb AS usuamodifico
    FROM fe_art  AS a 
...
if opt=1 then
  ...
  peso,a.prec,tipro,idmar,a.idcat,cost,tmon,a.idflete,prod_uti1,prod_uti2,prod_uti3,prod_idus,prod_equi1,prod_equi2,
  prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,ifnull(o.razo,'') as proveedor,
  ifnull(y.ndoc,'') as ndoc,ifnull(y.fech,'') as fech,prod_idpc,prod_idpm,prod_cod1 as cod2,cast(idart as char) as prod_cod1,prod_acti,prod_alma
  FROM fe_art  as a inner join fe_fletes as b on(b.idflete=a.idflete)
  inner join fe_cat as c on(c.idcat=a.idcat) left join fe_costos as d on(d.cost_idco=a.prod_idco)
  left join fe_rcom as y on (y.idauto=a.prod_idau) left join fe_prov as o on (o.idprov=y.idprov)
  WHERE descri LIKE cbuscar and prod_acti<>'I' ORDER BY DESCRI;
...
IF opt=1 THEN
  ...
  peso,a.prec,tipro,idmar,a.idcat,cost,tmon,a.idflete,prod_uti1,prod_uti2,prod_uti3,prod_idus,prod_equi1,prod_equi2,
  prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,IFNULL(o.razo,'') AS proveedor,
  IFNULL(yy.ndoc,'') AS ndoc,IFNULL(yy.fech,'') AS fech,prod_idpc,prod_idpm,prod_cod1 AS cod2,prod_cod1,prod_acti,prod_alma,
...
/*!50003 CREATE PROCEDURE `ProMuestraTProductos`(abuscar varchar(80),nd float)
BEGIN
declare cbuscar varchar(80);
declare vigv decimal(5,3);
set cbuscar=concat('%',trim(abuscar),+'%');
...
peso,a.prec,tipro,idmar,a.idcat,cost,tmon,a.idflete,prod_uti1,prod_uti2,prod_uti3,prod_idus,prod_equi1,prod_equi2,
prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,ifnull(o.razo,'') as proveedor,
ifnull(y.ndoc,'') as ndoc,ifnull(y.fech,'') as fech,prod_cod1,prod_acti,prod_alma
FROM fe_art  as a inner join fe_fletes as b on(b.idflete=a.idflete)
inner join fe_cat as c on(c.idcat=a.idcat) left join fe_costos as d on(d.cost_idco=a.prod_idco)
left join fe_rcom as y on (y.idauto=a.prod_idau) left join fe_prov as o on (o.idprov=y.idprov)
WHERE descri LIKE cbuscar ORDER BY DESCRI;
...
set cbuscar=concat('%',trim(cbusca),+'%');
if opt=0 then
   select nomb,tipo,activo,idusua,0 as Uno,1 as uno,2 as dos,3 as tres,clave,idalma from fe_usua where nomb like cbuscar and activo='S';
end if;
if opt=1 then
   select nomb,tipo,activo,idusua,0 as Uno,1 as uno,2 as dos,3 as tres,clave,idalma from fe_usua where idusua=nid and activo='S';
end if;
END */$$
DELIMITER ;
...
declare cbuscar varchar(80);
set cbuscar=concat('%',trim(cbusca),+'%');
select nomv,idven,vend_tipo from fe_vend where nomv like cbuscar and vend_acti<>'I' order by nomv;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `Stockx` */

/*!50003 DROP PROCEDURE IF EXISTS  `Stockx` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `Stockx`()
BEGIN
DECLARE done INT DEFAULT 0;
declare ccoda integer;
declare tcompras float;
declare cursor1 cursor for
select idart,cant from fe_stock;
DECLARE CONTINUE HANDLER FOR SQLSTATE '02000' SET done = 1;
open cursor1;
start transaction;
repeat
    fetch cursor1 into ccoda,tcompras;
    insert into fe_kar (idart,cant,kar_equi)values(ccoda,tcompras,1);
until done end repeat;
commit;
END */$$
DELIMITER ;
...
/*Table structure for table `vlprecios` */

DROP TABLE IF EXISTS `vlprecios`;

/*!50001 DROP VIEW IF EXISTS `vlprecios` */;
/*!50001 DROP TABLE IF EXISTS `vlprecios` */;

/*!50001 CREATE TABLE  `vlprecios`(
 `idart` int ,
 `descri` varchar(180) ,
 `unid` varchar(15) ,
 `prod_unid1` varchar(15) ,
...
/*Table structure for table `vvtasreales` */

DROP TABLE IF EXISTS `vvtasreales`;

/*!50001 DROP VIEW IF EXISTS `vvtasreales` */;
/*!50001 DROP TABLE IF EXISTS `vvtasreales` */;

/*!50001 CREATE TABLE  `vvtasreales`(
 `idauto` int 
)*/;
...
/*View structure for view vlprecios */

/*!50001 DROP TABLE IF EXISTS `vlprecios` */;
/*!50001 DROP VIEW IF EXISTS `vlprecios` */;

/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vlprecios` AS select `a`.`idart` AS `idart`,`a`.`descri` AS `descri`,`a`.`unid` AS `unid`,`a`.`prod_unid1` AS `prod_unid1`,cast(if((`a`.`uno` > 0),if(((`a`.`uno` % `a`.`prod_equi1`) = 0),(`a`.`uno` / `a`.`prod_equi1`),if(((`a`.`uno` % `a`.`prod_equi1`) = 0),(`a`.`uno` DIV `a`.`prod_equi1`),truncate((`a`.`uno` / `a`.`prod_equi1`),0))),0.00) as decimal(12,2)) AS `prod_unim`,cast(if((`a`.`uno` > 0),if(((`a`.`uno` % `a`.`prod_equi1`) = 0),0.00,(`a`.`uno` % `a`.`prod_equi1`)),`a`.`uno`) as decimal(12,2)) AS `prod_unin`,cast(if((`a`.`dos` > 0),if(((`a`.`dos` % `a`.`prod_equi1`) = 0),(`a`.`dos` / `a`.`prod_equi1`),if(((`a`.`dos` % `a`.`prod_equi1`) = 0),(`a`.`dos` DIV `a`.`prod_equi1`),truncate((`a`.`dos` / `a`.`prod_equi1`),0))),0.00) as decimal(12,2)) AS `prod_dunim`,cast(if((`a`.`dos` > 0),if(((`a`.`dos` % `a`.`prod_equi1`) = 0),0.00,(`a`.`dos` % `a`.`prod_equi1`)),`a`.`dos`) as decimal(12,2)) AS `prod_dunin`,cast(if((`a`.`tre` > 0),if(((`a`.`tre` % `a`.`prod_equi1`) = 0),(`a`.`tre` / `a`.`prod_equi1`),if(((`a`.`tre` % `a`.`prod_equi1`) = 0),(`a`.`tre` DIV `a`.`prod_equi1`),truncate((`a`.`tre` / `a`.`prod_equi1`),0))),0.00) as decimal(12,2)) AS `prod_tunim`,cast(if((`a`.`tre` > 0),if(((`a`.`tre` % `a`.`prod_equi1`) = 0),0.00,(`a`.`tre` % `a`.`prod_equi1`)),`a`.`tre`) as decimal(12,2)) AS `prod_tunin`,cast(if((`a`.`cua` > 0),if(((`a`.`cua` % `a`.`prod_equi1`) = 0),(`a`.`cua` / `a`.`prod_equi1`),if(((`a`.`cua` % `a`.`prod_equi1`) = 0),(`a`.`cua` DIV `a`.`prod_equi1`),truncate((`a`.`cua` / `a`.`prod_equi1`),0))),0.00) as decimal(12,2)) AS `prod_cunim`,cast(if((`a`.`cua` > 0),if(((`a`.`cua` % `a`.`prod_equi1`) = 0),0.00,(`a`.`cua` % `a`.`prod_equi1`)),`a`.`cua`) as decimal(12,2)) AS `prod_cunin`,round(if((`a`.`tmon` = 'S'),((`a`.`prec` * `a`.`prod_tigv`) + `b`.`prec`),(((`a`.`prec` * `a`.`prod_tigv`) * `v`.`dola`) + `b`.`prec`)),2) AS `costo`,`c`.`idgrupo` AS `idgrupo`,`c`.`dcat` AS `dcat`,ifnull(round(if((`a`.`tmon` = 'S'),`a`.`premay`,((((`a`.`prec` * `a`.`prod_tigv`) * `v`.`dola`) + `b`.`prec`) * `a`.`prod_uti3`)),2),0) AS `pre1`,ifnull(round(if((`a`.`tmon` = 'S'),`a`.`premen`,((((`a`.`prec` * `a`.`prod_tigv`) * `v`.`dola`) + `b`.`prec`) * `a`.`prod_uti2`)),2),0) AS `pre2`,ifnull(round(if((`a`.`tmon` = 'S'),`a`.`pre3`,((((`a`.`prec` * `a`.`prod_tigv`) * `v`.`dola`) + `b`.`prec`) * `a`.`prod_uti1`)),2),0) AS `pre3`,`a`.`prod_tigv` AS `prod_tigv`,round(if((`a`.`tmon` = 'S'),(`a`.`prec` * `a`.`prod_tigv`),((`a`.`prec` * `a`.`prod_tigv`) * `v`.`dola`)),2) AS `costosf`,`b`.`prec` AS `flete`,`a`.`ulfc` AS `ulfc`,`a`.`uno` AS `uno`,`a`.`dos` AS `dos`,`a`.`tre` AS `tre`,`a`.`cua` AS `cua`,ifnull(`d`.`cost_cost`,0) AS `costor`,ifnull(`d`.`cost_prec`,0) AS `precr`,ifnull(`d`.`cost_mone`,'') AS `moner`,cast(ifnull(`d`.`cost_idco`,0) as unsigned) AS `cost_idco`,ifnull(`d`.`cost_flet`,0) AS `fleter`,ifnull(`d`.`cost_dola`,0) AS `dolar`,`a`.`peso` AS `peso`,`a`.`prec` AS `prec`,`a`.`tipro` AS `tipro`,`a`.`idmar` AS `idmar`,`a`.`idcat` AS `idcat`,`a`.`cost` AS `cost`,`a`.`tmon` AS `tmon`,`a`.`idflete` AS `idflete`,`a`.`prod_uti1` AS `prod_uti1`,`a`.`prod_uti2` AS `prod_uti2`,`a`.`prod_uti3` AS `prod_uti3`,`a`.`prod_idus` AS `prod_idus`,`a`.`prod_equi1` AS `prod_equi1`,`a`.`prod_equi2` AS `prod_equi2`,`a`.`prod_come` AS `prod_come`,`a`.`prod_comc` AS `prod_comc`,`a`.`ulpc` AS `ulpc`,`a`.`prod_uact` AS `prod_uact`,`a`.`prod_fact` AS `prod_fact`,`a`.`fechc` AS `fechc`,`a`.`prod_smax` AS `prod_smax`,`a`.`prod_smin` AS `prod_smin`,ifnull(`o`.`razo`,'') AS `proveedor`,ifnull(`y`.`ndoc`,'') AS `ndoc`,ifnull(`y`.`fech`,'') AS `fech` from ((((((`fe_art` `a` join `fe_fletes` `b` on((`b`.`idflete` = `a`.`idflete`))) join `fe_cat` `c` on((`c`.`idcat` = `a`.`idcat`))) left join `fe_costos` `d` on((`d`.`cost_idco` = `a`.`prod_idco`))) left join `fe_rcom` `y` on((`y`.`idauto` = `a`.`prod_idau`))) left join `fe_prov` `o` on((`o`.`idprov` = `y`.`idprov`))) join `fe_gene` `v`) where (`a`.`prod_acti` <> 'I') order by `a`.`descri` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vmuestracotizaciones` AS select `a`.`dped_unid` AS `dped_unid`,`a`.`dped_epta` AS `idepta`,`a`.`dped_equi` AS `dped_equi`,`a`.`dped_cost` AS `costo`,`a`.`idart` AS `idart`,`b`.`prod_tigv` AS `prod_tigv`,`b`.`descri` AS `descri`,`x`.`pres_desc` AS `unid`,`a`.`cant` AS `cant`,ifnull(`m`.`idven`,0) AS `idven`,ifnull(`m`.`nomv`,'') AS `Vendedor`,`b`.`uno` AS `uno`,`b`.`dos` AS `dos`,`b`.`tre` AS `tre`,`b`.`cua` AS `cua`,`a`.`prec` AS `prec`,`b`.`premay` AS `premay`,`b`.`premen` AS `premen`,`c`.`fech` AS `fech`,`c`.`idautop` AS `idautop`,`c`.`impo` AS `impo`,`c`.`form` AS `form`,`c`.`ndoc` AS `ndoc`,`a`.`dped_equi` AS `equi`,`c`.`aten` AS `aten`,`c`.`forma` AS `forma`,`a`.`dped_posi` AS `dped_posi`,`a`.`dped_epta` AS `dped_epta`,`c`.`plazo` AS `plazo`,`c`.`validez` AS `validez`,`c`.`entrega` AS `entrega`,`c`.`detalle` AS `detalle`,ifnull(`d`.`idclie`,0) AS `idclie`,ifnull(`d`.`razo`,'') AS `razo`,ifnull(`d`.`nruc`,'') AS `nruc`,ifnull(`d`.`dire`,'') AS `dire`,`c`.`rped_vigv` AS `rped_vigv`,`c`.`rped_mone` AS `rped_mone`,ifnull(`d`.`ciud`,'') AS `ciud`,`d`.`fono` AS `fono`,`d`.`fax` AS `fax`,`a`.`idped` AS `nreg` from ((((((`fe_ped` `a` join `fe_rped` `c` on((`a`.`idautop` = `c`.`idautop`))) join `fe_art` `b` on((`b`.`idart` = `a`.`idart`))) join `fe_epta` `v` on((`v`.`epta_idep` = `a`.`dped_epta`))) join `fe_presentaciones` `x` on((`x`.`pres_idpr` = `v`.`epta_pres`))) left join `fe_clie` `d` on((`d`.`idclie` = `c`.`idclie`))) left join `fe_vend` `m` on((`m`.`idven` = `c`.`idven`))) where ((`a`.`acti` <> 'I') and (`c`.`acti` <> 'I')) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vmuestrapedidosvtas` AS select `a`.`dped_cost` AS `costo`,`c`.`tdoc` AS `tdoc`,`c`.`form` AS `form`,`c`.`idclie` AS `idclie`,`a`.`idart` AS `idart`,`b`.`prod_tigv` AS `prod_tigv`,`b`.`descri` AS `descri`,`x`.`pres_desc` AS `unid`,`a`.`cant` AS `cant`,`a`.`prec` AS `prec`,`b`.`premay` AS `premay`,`b`.`premen` AS `premen`,`c`.`fech` AS `fech`,`c`.`idautop` AS `idautop`,`c`.`impo` AS `impo`,`c`.`ndoc` AS `ndoc`,`b`.`prod_idco` AS `idco`,`b`.`uno` AS `uno`,`b`.`dos` AS `dos`,`b`.`tre` AS `tre`,`b`.`cua` AS `cua`,`a`.`dped_equi` AS `dped_equi`,`n`.`nomb` AS `Usuario`,`c`.`rped_idus` AS `rped_idus`,`c`.`idpcped` AS `idpcped`,`c`.`fecho` AS `fecho`,`m`.`nomv` AS `nomv`,`c`.`idven` AS `idven`,`c`.`idtienda` AS `idtienda`,`c`.`facturado` AS `facturado`,`a`.`dped_posi` AS `dped_posi`,`c`.`aten` AS `aten`,`c`.`forma` AS `forma`,`b`.`prod_equi1` AS `equi1`,`b`.`prod_equi2` AS `equi2`,`c`.`plazo` AS `plazo`,`c`.`validez` AS `validez`,`b`.`premay` AS `pre1`,`b`.`premen` AS `pre2`,ifnull(`b`.`pre3`,0) AS `pre3`,`a`.`idped` AS `nreg`,`a`.`dped_comi` AS `dped_comi`,`a`.`dped_idep` AS `dped_epta` from (((((((`fe_ped` `a` join `fe_rped` `c` on((`a`.`idautop` = `c`.`idautop`))) join `fe_art` `b` on((`b`.`idart` = `a`.`idart`))) join `fe_fletes` `p` on((`p`.`idflete` = `b`.`idflete`))) join `fe_vend` `m` on((`m`.`idven` = `c`.`idven`))) join `fe_usua` `n` on((`n`.`idusua` = `c`.`rped_idus`))) join `fe_epta` `v` on((`v`.`epta_idep` = `a`.`dped_epta`))) join `fe_presentaciones` `x` on((`x`.`pres_idpr` = `v`.`epta_pres`))) where ((`c`.`acti` = 'A') and (`a`.`acti` = 'A')) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vpdtespago` AS select `a`.`ndoc` AS `ndoc`,`a`.`fech` AS `fech`,`a`.`dola` AS `dola`,`a`.`nrou` AS `nrou`,`a`.`banc` AS `banc`,`a`.`iddeu` AS `iddeu`,max(`a`.`fevto`) AS `fevto`,round(sum((`a`.`impo` - `a`.`acta`)),2) AS `saldo`,`b`.`rdeu_idpr` AS `Idpr`,`b`.`rdeu_impc` AS `ImporteC`,'C' AS `situa`,`b`.`rdeu_idau` AS `Idauto`,`a`.`ncontrol` AS `ncontrol`,`a`.`tipo` AS `tipo`,`a`.`banco` AS `banco`,ifnull(`c`.`ndoc`,'0') AS `docd`,ifnull(`c`.`tdoc`,'0') AS `tdoc`,`b`.`rdeu_mone` AS `Moneda`,`b`.`rdeu_codt` AS `Codt`,`b`.`rdeu_idrd` AS `Idrd`,`b`.`rdeu_idct` AS `rdeu_idct` from ((`fe_deu` `a` join `fe_rdeu` `b` on((`b`.`rdeu_idrd` = `a`.`deud_idrd`))) left join `fe_rcom` `c` on((`c`.`idauto` = `b`.`rdeu_idau`))) where ((`b`.`rdeu_Acti` <> 'I') and (`a`.`acti` <> 'I')) group by `a`.`ncontrol` having (round(sum((`a`.`impo` - `a`.`acta`)),2) > 0) order by max(`a`.`fevto`) */;
...
/*View structure for view vvtasreales */

/*!50001 DROP TABLE IF EXISTS `vvtasreales` */;
/*!50001 DROP VIEW IF EXISTS `vvtasreales` */;

/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vvtasreales` AS select `fe_kar`.`idauto` AS `idauto` from `fe_kar` where ((`fe_kar`.`tipo` = 'V') and (`fe_kar`.`acti` = 'A') and (`fe_kar`.`alma` > 0)) group by `fe_kar`.`idauto` order by `fe_kar`.`idauto` */;
