...
MySQL - 8.0.43 : Database - syscom_bdminath
*********************************************************************
*/
...
/*!50003 CREATE FUNCTION `FunIngresaDatosLibroDiario`(dfech date,ndebe decimal(12,2),
nhaber decimal(12,2),cglosa varchar(120),ct char(1),cnume varchar(10),nidcta integer,ccond char,nit integer,ncomp varchar(15),nidcl integer,
...
begin
declare id integer;
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,lcaj_idus,lcaj_codt,lcaj_tran,lcaj_form)values
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidt,'T','E');
select last_insert_id() into id from fe_lcaja group by last_insert_id();
return id;
end */$$
DELIMITER ;
...
select rdeu_idrd into xi from fe_rdeu  where rdeu_acti<>'I' and rdeu_idau=nid limit 1;
...
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidcp,nidauto,cform,localtime,cdcto,ctdoc,2);
end */$$
DELIMITER ;
...
BEGIN
SELECT * FROM fe_sucu  where idalma in(1,2,3,4) order by idalma;
END */$$
DELIMITER ;
...
if opt=1 then
    ...
    round(if(tmon='S',(a.prec*v.igv),(a.prec*v.igv*nd)),2) as costosf,b.prec as flete,m.dmar,g.desgrupo AS grupo,
    ...
    b.desflete,u.nomb AS usuacreo,us.nomb AS usuamodifico
    FROM fe_art  as a 
