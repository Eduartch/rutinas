...
MySQL - 8.0.37 : Database - syscom_bdcampos
*********************************************************************
*/
...
/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `actualizaproductosbarras` */$$

/*!50003 CREATE */ /*!50003 TRIGGER `actualizaproductosbarras` AFTER UPDATE ON `fe_art` FOR EACH ROW BEGIN
	IF ((new.descri <> old.descri) || (new.prod_cod1 <> old.prod_cod1) || (new.coda1 <> old.coda1)) then
	      INSERT INTO fe_aproductos(prod_idar,prod_idus,prod_fope,prod_barrasold,prod_barrasnew,prod_provold,prod_provnew,prod_descriold,prod_descrinew) 
	      VALUES (old.idart,new.prod_uact,LOCALTIME,old.prod_cod1,new.prod_cod1,old.coda1,new.coda1,old.descri,new.descri);
       END IF;
       IF new.prod_acti='I' THEN
	      INSERT INTO fe_aproductos(prod_idar,prod_idus,prod_fope,prod_barrasold,prod_barrasnew,prod_provold,prod_provnew,prod_descriold,prod_descrinew) 
	      VALUES (old.idart,new.prod_uact,LOCALTIME,old.prod_cod1,new.prod_cod1,old.coda1,new.coda1,old.descri,'ANULADO');
       END IF;
END */$$
...
/*!50003 CREATE FUNCTION `FuncreaProductos`(cdesc varchar(180),cunid varchar(4),nprec float,ncosto float,
np1 float,np2 float,np3 float,npeso float,ccat integer,cmar integer,ctipro char,nflete integer,cm char,cidpc varchar(45),
ncome float,ncomc float,nutil1 float,nutil2 float,nutil3 float,nidusua integer,nsmax float, nsmin float,ccodigo1 varchar(30),ndolar float,coda1 varchar(50),tigv FLOAT) RETURNS int
BEGIN
declare nid integer default 0;
INSERT INTO fe_art(descri,unid,prec,cost,premay,premen,pre3,peso,idcat,idmar,tipro,idflete,tmon,fechc,idpc,prod_come,
prod_comc,prod_uti1,prod_uti2,prod_uti3,prod_idus,prod_smin,prod_smax,prod_cod1,prod_dola,coda1,prod_tigv)
VALUES (cdesc,cunid,nprec,ncosto,np1,np2,np3,npeso,ccat,cmar,ctipro,nflete,cm,localtime,
cidpc,ncome,ncomc,nutil1,nutil2,nutil3,nidusua,nsmin,nsmax,ccodigo1,ndolar,coda1,tigv);
...
nv float,nigv float,nt float,cndo2 varchar(15),cm char,
ndolar float,ni float,ctg char,ccodp integer,cmvto char,nus integer,opt integer,nidcodt integer,
n1 integer,n2 integer,n3 integer,nitem integer,npvta float,exon decimal(8,2)) RETURNS int
BEGIN
...
if opt=0 then
   ...
   dolar,vigv,tcom,idprov,tipom,fusua,idusua,codt,rcom_nitem,rcom_tipo,pimpo,rcom_fech,rcom_exon)
   VALUES (ctdoc,cform,cndoc,dfecha,dfechar,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,
   ni,ctg,ccodp,cmvto,localtime,nus,nidcodt,nitem,ctipo,npvta,dfecha,exon);
...
nv FLOAT,nigv FLOAT,nt FLOAT,cndo2 VARCHAR(25),cm CHAR,
ndolar FLOAT,ni FLOAT,ctg CHAR,ccodp INTEGER,cmvto CHAR,nus INTEGER,nidcodt INTEGER,
n1 INTEGER,n2 INTEGER,n3 INTEGER,nitem DECIMAL(12,2),npvta FLOAT,reten DECIMAL(6,2),vuelto decimal(10,2)) RETURNS int
BEGIN
DECLARE nid INTEGER DEFAULT 0;
DECLARE ctipo CHAR;
SET nid=0;
INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,
dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,rcom_tipo,pimpo,rcom_fech,rcom_mret,rcom_vuelto,rcom_exon)
VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,0,nigv,nt,cndo2,cm,ndolar,1,
ctg,ccodp,cmvto,LOCALTIME,nus,nidcodt,ctipo,npvta,dfecha,reten,vuelto,nv);
SELECT LAST_INSERT_ID() INTO nid FROM fe_rcom GROUP BY LAST_INSERT_ID();
IF n1>0 AND n2>0 AND n3>0 THEN
   CALL IngresaCuentasV(nv,nigv,nt,n1,n2,n3,"H","H","D",nid);
END IF;
...
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidt,localtime);
select last_insert_id() into id from fe_lcaja group by last_insert_id();
return id;
end */$$
DELIMITER ;
...
nv DECIMAL(12,2),nigv DECIMAL(12,2),nt DECIMAL(12,2),cndo2 VARCHAR(25),cm CHAR,
...
/*!50003 CREATE FUNCTION `FunIngresaDPedidos`(ncoda integer,ncant float,nprec float,nidauto integer,igv CHAR(1),
unidpres varchar(50),equipres decimal(12,2),eptaidep integer) RETURNS int
BEGIN
INSERT INTO fe_ped(idart,cant,prec,idautop,incl,unidpres,equipres,eptaidep)
VALUES(ncoda,ncant,nprec,nidauto,igv,unidpres,equipres,eptaidep);
return 10;
END */$$
DELIMITER ;
...
/*!50003 CREATE FUNCTION `FunIngresaGuias`(dfecha DATE,cptop VARCHAR(100),cptoll VARCHAR(150),
...
dfechat DATETIME,nidus INTEGER,cdeta VARCHAR(150),nidtr INTEGER,cndoc VARCHAR(12),nidtda INTEGER,cdcto VARCHAR(60),df DATE,nidpr INTEGER,cubigeo VARCHAR(8),cconductor VARCHAR(150),cbrevete VARCHAR(15)) RETURNS int
BEGIN
DECLARE id INTEGER;
INSERT INTO fe_guias(guia_fech,guia_ptop,guia_ptoll,guia_idau,guia_fect,guia_idus,guia_fope,guia_deta,guia_idtr,guia_ndoc,guia_moti,
...
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 integer,vcom float,epta INTEGER, kar_unid VARCHAR(100), kar_equi DECIMAL(12,2),exon decimal(5,2),lote VARCHAR(15),fechavto DATE) RETURNS int
BEGIN
declare nidk integer default 0;
if ct='C' then
  INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_idco,codv,kar_alma1,kar_epta,kar_unid,kar_equi,kar_tigv,kar_lote,kar_fvto)
  VALUES (nid,cc,ct,npr,nct,tmvto,cincl,calma,nidcosto1,ccodv,calma,epta,kar_unid,kar_equi,exon,lote,fechavto);
 else
...
nct FLOAT,cincl CHAR,tmvto CHAR,ccodv INTEGER,calma INTEGER,nidcosto1 DECIMAL(12,4),vcom FLOAT,epta integer, kar_unid varchar(100), kar_equi decimal(12,2),
lote VARCHAR(15),fechavto DATE) RETURNS int
BEGIN
DECLARE nidk INTEGER DEFAULT 0;
INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_cost,codv,kar_alma1,kar_comi,kar_icbper,kar_epta,kar_unid,kar_equi,kar_lote,kar_fvto)
VALUES (nid,cc,'V',npr,nct,tmvto,cincl,calma,nidcosto1,ccodv,0,vcom,nicbper,epta,kar_unid,kar_equi,lote,fechavto);
SELECT LAST_INSERT_ID() INTO nidk FROM fe_kar GROUP BY LAST_INSERT_ID();
RETURN nidk;
END */$$
DELIMITER ;
...
SELECT LAST_INSERT_ID() INTO nid FROM fe_cred GROUP BY LAST_INSERT_ID();
IF LEFT(cnrou,4)='a@@a' THEN
   UPDATE fe_rcred SET rcre_idacta=nid WHERE rcre_idrc=nidrc;
END IF;  
RETURN nid;
END */$$
DELIMITER ;
...
set sw:=(SELECT count(rcre_idau) FROM fe_cred as b 
inner join fe_rcred as a on(a.rcre_idrc=b.cred_idrc) 
WHERE b.acta>0 AND a.rcre_idau=nid AND acti<>'I' and b.ncontrol>0 group by a.rcre_idau limit 1);
set sw1:=(SELECT count(rcre_idau) FROM fe_rcred as b 
WHERE b.rcre_inic>0 AND b.rcre_idau=nid AND b.rcre_acti<>'I'  group by b.rcre_idau);
...
BEGIN
   if ctipo="C" then
      if nalma=0 then
          UPDATE fe_art SET cero=cero+(ccant*equi) WHERE idart=coda;
      end if;
      if nalma=1 then
          UPDATE fe_art SET uno=uno+(ccant*equi),prod_acti='A' WHERE idart=coda;
      end if;
      if nalma=2 then
          UPDATE fe_art SET dos=dos+(ccant*equi),prod_acti='A' WHERE idart=coda;
      end if;
      if nalma=3 then
          UPDATE fe_art SET tre=tre+(ccant*equi),prod_acti='A' WHERE idart=coda;
      end if;
      if nalma=4 then
          UPDATE fe_art SET cua=cua+(ccant*equi),prod_acti='A' WHERE idart=coda;
     end if;
     IF nalma=5 THEN
          UPDATE fe_art SET cin=cin+(ccant*equi),prod_acti='A' WHERE idart=coda;
     END IF;
     IF nalma=6 THEN
          UPDATE fe_art SET sei=sei+(ccant*equi),prod_acti='A' WHERE idart=coda;
     END IF;
      IF nalma=7 THEN
          UPDATE fe_art SET sie=sie+(ccant*equi),prod_acti='A' WHERE idart=coda;
     END IF;
        IF nalma=8 THEN
          UPDATE fe_art SET och=och+(ccant*equi),prod_acti='A' WHERE idart=coda;
     END IF;
   end if;
   if ctipo="V" then
      if nalma=0 then
     ...
     END IF;
       IF nalma=7 THEN
          UPDATE fe_art SET sie=sie-(ccant*equi) WHERE idart=coda;
     END IF;
        IF nalma=8 THEN
          UPDATE fe_art SET och=och-(ccant*equi) WHERE idart=coda;
     END IF;
  end if;
   if ctipo="I" then
      if nalma=0 then
          UPDATE fe_art SET cero=(ccant*equi) WHERE idart=coda;
      end if;
      if nalma=1 then
          UPDATE fe_art SET uno=(ccant*equi) WHERE idart=coda;
      end if;
      if nalma=2 then
          UPDATE fe_art SET dos=(ccant*equi) WHERE idart=coda;
      end if;
      if nalma=3 then
          UPDATE fe_art SET tre=(ccant*equi) WHERE idart=coda;
      end if;
      if nalma=4 then
          UPDATE fe_art SET cua=(ccant*equi) WHERE idart=coda;
       end if;
       IF nalma=5 THEN
          UPDATE fe_art SET cin=(ccant*equi) WHERE idart=coda;
       END IF;
       IF nalma=6 THEN
          UPDATE fe_art SET sei=(ccant*equi) WHERE idart=coda;
       END IF;
       IF nalma=7 THEN
          UPDATE fe_art SET sie=(ccant*equi) WHERE idart=coda;
       END IF;
       IF nalma=8 THEN
          UPDATE fe_art SET och=(ccant*equi) WHERE idart=coda;
       END IF;
  end if;
END */$$
DELIMITER ;
...
BEGIN
select idgene from fe_gene where idgene=1;
END */$$
DELIMITER ;

/* Procedure structure for procedure `CalcularStock10` */

/*!50003 DROP PROCEDURE IF EXISTS  `CalcularStock10` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `CalcularStock10`()
BEGIN
DECLARE done INT DEFAULT 0;
declare ct varchar(1) default 'I';
declare saldo float;
...
nv DECIMAL(12,2),nigv DECIMAL(12,2),nt DECIMAL(12,2),cndo2 VARCHAR(10),cm CHAR,
ndolar FLOAT,ni FLOAT,ctg CHAR,ccodp INTEGER,cmvto CHAR,nus INTEGER,opt INTEGER,nidcodt INTEGER,
n1 INTEGER,n2 INTEGER,n3 INTEGER,nitems INTEGER,npvta FLOAT,nidauto INTEGER,exon decimal(8,2))
BEGIN
...
IF opt=0 THEN
   ...
   mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idprov=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,rcom_nitem=nitems,pimpo=npvta,rcom_exon=exon WHERE idauto=nidauto;
...
nus INTEGER,reten DECIMAL(8,2),nidcodt INTEGER,n1 INTEGER,n2 INTEGER,
n3 INTEGER,nitems INTEGER,npvta FLOAT,nidauto INTEGER)
BEGIN
UPDATE fe_rcom SET tdoc=ctdoc,form=cform,ndoc=cndoc,fech=dfecha,fecr=dfecha,deta=cdetalle,valor=0,igv=nigv,impo=nt,ndo2=cndo2,
mone=cm,dolar=ndolar,vigv=1,tcom=ctg,idcliente=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,rcom_mret=reten,rcom_exon=nv,rcom_vuelto=0 WHERE idauto=nidauto;
CALL ProDesactivaLcajaE(nidauto);
IF n1>0 AND n2>0 AND n3>0 THEN
...
/*!50003 CREATE PROCEDURE `ProActualizaDetallePedidos`(ncoda integer,ncant float,nprec float,nr integer,ctipoa char,igv char(1),
unidpres VARCHAR(50),equipres DECIMAL(12,2),eptaidep INTEGER)
BEGIN
if ctipoa='A' then
     UPDATE fe_ped SET acti='I' WHERE idped=nr;
   else
    UPDATE fe_ped SET idart=ncoda,cant=ncant,prec=nprec,incl=igv,
    unidpres=unidpres,equipres=equipres,eptaidep=eptaidep
     WHERE idped=nr;
end if;
END */$$
DELIMITER ;
...
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 integer,nidkar integer,op integer,xcom float,epta INTEGER, kunid VARCHAR(100), 
kequi DECIMAL(12,2),exon DECIMAL(5,2),lote VARCHAR(15),fechavto DATE)
BEGIN
if op=0 then
  ...
  alma=calma,kar_idco=nidcosto1,codv=ccodv,kar_comi=xcom,kar_epta=epta,kar_unid=kunid,kar_equi=kequi,kar_tigv=exon,
  kar_lote=lote,kar_fvto=fechavto
   where idkar=nidkar;
end if;
END */$$
DELIMITER ;
...
nct FLOAT,cincl CHAR,tmvto CHAR,ccodv INTEGER,calma INTEGER,nidcosto1 DECIMAL(12,4),nidkar INTEGER,op INTEGER,xcom FLOAT,epta INTEGER, kunid VARCHAR(100), kequi DECIMAL(12,2),
lote VARCHAR(15),fechavto DATE)
BEGIN
IF op=0 THEN
  UPDATE fe_kar SET acti='I' WHERE idkar=nidkar;
 ELSE
  ...
  alma=calma,kar_cost=nidcosto1,codv=ccodv,kar_comi=xcom,kar_epta=epta,kar_unid=kunid,kar_equi=kequi,
  kar_lote=lote,kar_fvto=fechavto
  WHERE idkar=nidkar;
END IF;
END */$$
...
cc INTEGER,dfe DATE,npr FLOAT,cnd INTEGER,idp INTEGER,cmda CHAR,ni FLOAT,ndolar FLOAT,nidcosto INTEGER,precioprec DECIMAL(8,2),idep INT,cantequi DECIMAL(8,2))
BEGIN
DECLARE costor FLOAT;
DECLARE vigv FLOAT;
DECLARE cmensaje VARCHAR(100);
DECLARE costop FLOAT;
SELECT igv INTO vigv FROM fe_gene WHERE idgene=1;
SELECT CONVERT('00/00/0000',CHAR),prec,tmon INTO @ufc,@nprecio,@cmoneda FROM fe_art WHERE idart=cc;
IF @ufc<=dfe THEN
   SELECT prod_uti1,prod_uti2,prod_uti3,idflete INTO @nutil1,@nutil2,@nutil2,@nidflete FROM fe_art WHERE idart=cc;
   SELECT prec INTO @ncostof FROM fe_fletes WHERE idflete=@nidflete;
   IF nidcosto=1 THEN
        SET cmensaje=CONCAT("Promediado con el costo ", ROUND(@nprecio,5), " Moneda " , @cmoneda);
      ELSE
       SET cmensaje="";
   END IF;
   SET costop=ROUND(npr/cantequi,2);
   SET costor=ROUND((costop*vigv)+@ncostof,2);
   UPDATE fe_art SET prec=costop,cost=costop*vigv,prod_idau=cnd,ulpc=idp,tmon=cmda,ulfc=dfe,
   premay=ROUND(costor*@nutil1,2),premen=ROUND(costor*@nutil2,2),pre3=IF(@nutil3<>0,ROUND(costor*@nutil3,2),0),
   prod_prom=nidcosto,prod_dola=ndolar,prod_depr=cmensaje WHERE idart=cc;
END IF;
END */$$
DELIMITER ;
...
ncomc float,nidus integer,ncoda integer,nsmax float, nsmin float,ccodigo1 varchar(20),ndolar float,ce char,txtcoda1 varchar(50),tigv FLOAT)
BEGIN
...
prod_smin=nsmin,prod_cod1=ccodigo1,prod_dola=ndolar,prod_acti=ce, coda1=txtcoda1, prod_tigv=tigv WHERE idart=ncoda;
END */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProActualizaStock`(coda integer,nalma integer,ccant float,ctipo char(1),nequi FLOAT,ncaant float)
BEGIN
   if ctipo="C" then
      if nalma=1 then
...
IN ctipo CHAR,IN nidauto INTEGER,IN nu INTEGER, IN sw CHAR,dfecha DATE,IN nu1 INTEGER,in idtienda integer)
BEGIN
DECLARE nid INTEGER;
...
IF nid>0 THEN
   ...
   IF ctipo='V' THEN
      ...
      IF @nguia>0 THEN
         CALL ProAnulaEntregaFisica(@nguia,10);
      END IF;
    ELSE
...
IF sw='S' AND ctipo='V' THEN
   ...
   CALL PROingresa_anulada(@df,@cn1,@ct,nu,@nidconcepto,idtienda);
END IF;
SET estado=NULL;
COMMIT;
SET estado:="Ok";
...
if nid>0 then
   ...
   if ctipo='V' then
      ...
      if sw='S' then
       ...
       update fe_guiac set guic_tipo='P',guic_idac=0 where guic_idac=nid;
       update fe_guiac set guic_acti='I'  where guic_idau=nid;
       update fe_guias set guia_acti='I' where guia_idau=nid and guia_moti='D';
   end if;
 else
      select sucu_idus into nidcajero from fe_sucu where idalma=@ncodt;
      if nidcajero<=0 then
...
UPDATE fe_rcred SET rcre_inic=0 WHERE rcre_idacta=nid;
END */$$
DELIMITER ;
...
BEGIN
select uno,dos,tre,cua,cin,sei,sie,och from fe_art where idart=nidart;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProIngresaDatosLcajaeEfectivo12` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProIngresaDatosLcajaeEfectivo12` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProIngresaDatosLcajaeEfectivo12`(dfecha date,cndoc varchar(12),cdeta varchar(100),idcta integer,sdeudor decimal(12,2),
sacreedor decimal(12,2),cmone char,ndolar decimal(5,3),nidus integer,nidcp integer,nidauto integer,cform char,
...
cmotivo VARCHAR(50),cxml LONGBLOB ,cticket VARCHAR(30),carchivo VARCHAR(100),chash VARCHAR(30),nidauto INTEGER)
BEGIN
...
nigv DECIMAL(12,2),ngrati DECIMAL(12,2),cxml LONGBLOB,chash VARCHAR(30),carchivo VARCHAR(100),cticket VARCHAR(30))
BEGIN
...
/*!50003 CREATE PROCEDURE `Proingresa_anulada`(dfecha Date,cndoc varchar(12),ctdoc varchar(2),nidus integer,nidcon integer,codt integer)
begin
 select @nc:=idclie from fe_clie where idclie=1;
 insert into fe_rcom(idcliente,fech,fecr,ndoc,tdoc,tipom,deta,ndo2,tcom,form,mone,exon,fusua,idusua,codt)
 values(@nc,dfecha,dfecha,cndoc,ctdoc,'V','','','K','E','S',0,localtime,nidus,codt);
...
begin
 ...
 values(@nc,dfecha,dfecha,cndoc,ctdoc,'V','','','K','E','S','N',localtime,nidus,0,1);
 SELECT @na:=LAST_INSERT_ID() FROM fe_rcom group by LAST_INSERT_ID();
Call ProIngresaDatosLcajaEefectivo(dfecha,"","*** ANULADA ***",0,0,0,'S',0,nidus,0,@na,'E',cndoc,ctdoc);
END */$$
DELIMITER ;
...
BEGIN
SELECT nomb,idalma,dire,ciud,sucuidserie,ubigeo,sucu_idus FROM fe_sucu where idalma in(1,2,3,4) ORDER BY idalma;
END */$$
DELIMITER ;
...
if opt=0 then
   select idclie,nruc,razo,ndni,dire,ciud,ifnull(fono,'0') as fono,ifnull(fax,'0') as fax,clie_rete,
...
set cbuscar=concat('%',trim(cbusca),+'%');
SELECT tdoc,nomb,idtdoc FROM fe_tdoc WHERE nomb LIKE cbuscar and dcto_acti<>'I' ORDER BY tdoc;
END */$$
DELIMITER ;
...
WHERE desflete LIKE cbusca and flet_acti<>'I'  group by a.idflete ORDER BY idflete;
END */$$
DELIMITER ;
...
set cbusca=concat('%',trim(abuscar),+'%');
if nidgrupo=0 then
   SELECT a.idcat,a.dcat,a.util1,a.util2,ifnull(count(b.idart),0) as Total_Productos,a.idgrupo,g.desgrupo as grupo
   FROM fe_cat as a 
   inner join fe_grupo as g on g.idgrupo=a.idgrupo
   left join (select b.idart,b.idcat from fe_art as b where prod_acti='A') as b on b.idcat=a.idcat
   ...
   INNER JOIN fe_grupo AS g ON g.idgrupo=a.idgrupo
   left join (select b.idart,b.idcat from fe_art as b where prod_acti='A') as b on b.idcat=a.idcat
   WHERE dcat LIKE cbusca and line_acti<>'I' and a.idgrupo=nidgrupo  group by a.idcat ORDER BY dcat;
end if;
END */$$
DELIMITER ;
...
case opt
when 1 then
    SELECT idart,descri,unid,uno,dos,tre,cua,cero,cin,sei,sie,och,
    round(prod_uti1*round(if(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*if(prod_dola>nd,prod_dola,nd))+b.prec),2),2) as pre1,
    round(prod_uti2*round(if(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*if(prod_dola>nd,prod_dola,nd))+b.prec),2),2) as pre2,
    round(prod_uti3*round(if(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*if(prod_dola>nd,prod_dola,nd))+b.prec),2),2) as pre3,
    ROUND(prod_uti0*ROUND(IF(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*IF(prod_dola>nd,prod_dola,nd))+b.prec),2),2) AS pre0,
    round(if(tmon='S',(a.prec*a.prod_tigv)+b.prec,(a.prec*a.prod_tigv*if(prod_dola>nd,prod_dola,nd))+b.prec),2) as costo,c.idgrupo,c.dcat,prod_dola,
    round(if(tmon='S',(a.prec*v.igv),(a.prec*v.igv*nd)),2) as costosf,b.prec as flete,m.dmar,prod_tigv,
    ...
    prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,ifnull(o.razo,'') as proveedor,
    ifnull(yy.ndoc,'') as ndoc,ifnull(yy.fech,'') as fech,ulfc,prod_icbper,prod_deta,
    g.desgrupo AS grupo,b.desflete,u.nomb AS usuacreo,us.nomb AS usuamodifico,prod_uti0,prod_ocan,prod_ocom,prod_depr,a.coda1,
    prod_codb,prod_cmay,
     p.pres_desc,e.epta_prec,epta_cant,epta_idep
    FROM fe_art  as a 
    inner join fe_fletes as b on(b.idflete=a.idflete)
    ...
    WHERE descri LIKE cbuscar and prod_acti<>'I' AND epta_acti='A' ORDER BY DESCRI,epta_prec;
 when 0 then
    SELECT idart,descri,unid,uno,dos,tre,cua,cero,cin,sei,sie,och,
    round(prod_uti1*round(if(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*if(prod_dola>nd,prod_dola,nd))+b.prec),2),2) as pre1,
    round(prod_uti2*round(if(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*if(prod_dola>nd,prod_dola,nd))+b.prec),2),2) as pre2,
    ...
    ROUND(prod_uti0*ROUND(IF(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*IF(prod_dola>nd,prod_dola,nd))+b.prec),2),2) AS pre0,
   ROUND(IF(tmon='S',(a.prec*a.prod_tigv)+b.prec,(a.prec*a.prod_tigv*IF(prod_dola>nd,prod_dola,nd))+b.prec),2) AS costo,c.idgrupo,c.dcat,prod_dola,
    round(if(tmon='S',(a.prec*v.igv),(a.prec*v.igv*nd)),2) as costosf,b.prec as flete,m.dmar,a.coda1,prod_tigv,
    mid(descri,1,length(trim(descri))-length(trim(m.dmar))) as descri1,prod_cod1,
    CAST(0 AS UNSIGNED) AS costor,CAST(0 AS UNSIGNED) AS precr,''  AS moner,
    CAST(0 AS UNSIGNED) AS cost_idco,CAST(0 AS UNSIGNED) AS fleter,v.dola AS dolar,
    peso,a.prec,tipro,a.idmar,a.idcat,cost,tmon,a.idflete,prod_uti1,prod_uti2,prod_uti3,
    prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,ifnull(o.razo,'') as proveedor,
    ifnull(yy.ndoc,'') as ndoc,ifnull(yy.fech,'') as fech,ulfc,prod_icbper,prod_deta,
    g.desgrupo AS grupo,b.desflete,u.nomb AS usuacreo,us.nomb AS usuamodifico,prod_uti0,prod_ocan,prod_ocom,prod_depr,
    prod_codb,prod_cmay,
     p.pres_desc,e.epta_prec,epta_cant,epta_idep
    FROM fe_art  as a 
    inner join fe_fletes as b on(b.idflete=a.idflete)
    ...
    LEFT JOIN `fe_presentaciones` p ON (e.epta_pres=p.pres_idpr)
    LEFT JOIN fe_usua AS us ON us.idusua=a.prod_uact,fe_gene as v
    WHERE prod_cod1 LIKE cbuscar and prod_acti<>'I' AND epta_acti='A' ORDER BY DESCRI,epta_prec;
when 2 then    
    SELECT idart,descri,unid,uno,dos,tre,cua,cero,cin,sei,sie,och,
    ROUND(prod_uti1*ROUND(IF(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*IF(prod_dola>nd,prod_dola,nd))+b.prec),2),2) AS pre1,
    ROUND(prod_uti2*ROUND(IF(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*IF(prod_dola>nd,prod_dola,nd))+b.prec),2),2) AS pre2,
    ...
    MID(descri,1,LENGTH(TRIM(descri))-LENGTH(TRIM(m.dmar))) AS descri1,prod_cod1,a.coda1,
    CAST(0 AS UNSIGNED) AS costor,CAST(0 AS UNSIGNED) AS precr,''  AS moner,prod_tigv,
    CAST(0 AS UNSIGNED) AS cost_idco,CAST(0 AS UNSIGNED) AS fleter,v.dola AS dolar,
    peso,a.prec,tipro,a.idmar,a.idcat,cost,tmon,a.idflete,prod_uti1,prod_uti2,prod_uti3,
    prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,IFNULL(o.razo,'') AS proveedor,
    IFNULL(yy.ndoc,'') AS ndoc,IFNULL(yy.fech,'') AS fech,ulfc,prod_icbper,prod_deta,
    ...
    LEFT JOIN `fe_presentaciones` p ON (e.epta_pres=p.pres_idpr)
    LEFT JOIN fe_usua AS us ON us.idusua=a.prod_uact,fe_gene AS v
    WHERE a.coda1 LIKE cbuscar AND prod_acti<>'I' AND epta_acti='A' ORDER BY DESCRI,epta_prec;    
end case;
END */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProMuestraProductos10`(abuscar varchar(80),nd float,opt integer,nid integer)
BEGIN
declare cbuscar varchar(80);
if opt=1 then
    set cbuscar=concat('%',trim(abuscar),+'%');
    SELECT idart,descri,unid,uno,dos,tre,cua,cero,cin,sei,sie,
    round(prod_uti1*round(if(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*if(prod_dola>nd,prod_dola,nd))+b.prec),2),2) as pre1,
    round(prod_uti2*round(if(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*if(prod_dola>nd,prod_dola,nd))+b.prec),2),2) as pre2,
    round(prod_uti3*round(if(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*if(prod_dola>nd,prod_dola,nd))+b.prec),2),2) as pre3,
    ROUND(prod_uti0*ROUND(IF(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*IF(prod_dola>nd,prod_dola,nd))+b.prec),2),2) AS pre0,
    ...
    g.desgrupo AS grupo,b.desflete,u.nomb AS usuacreo,us.nomb AS usuamodifico,prod_uti0,prod_ocan,prod_ocom,prod_depr,
    prod_ubi1,prod_ubi2,prod_ubi3,prod_ubi4,prod_ubi5,prod_codb
    FROM fe_art  as a 
    inner join fe_fletes as b on(b.idflete=a.idflete)
    inner join fe_mar as m on m.idmar=a.idmar
    inner join fe_cat as c on(c.idcat=a.idcat) 
...
/* Procedure structure for procedure `ProMuestraProductos1admin` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProMuestraProductos1admin` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProMuestraProductos1admin`(abuscar VARCHAR(80),nd FLOAT,opt INTEGER,nid INTEGER)
BEGIN
DECLARE cbuscar VARCHAR(80);
SET cbuscar=CONCAT('%',TRIM(abuscar),+'%');
CASE opt
...
WHEN 2 THEN    
    ...
    ROUND(IF(tmon='S',(a.prec*a.prod_tigv)+b.prec,(a.prec*a.prod_tigv*IF(prod_dola>nd,prod_dola,nd))+b.prec),2) AS costo,c.idgrupo,c.dcat,prod_dola,
    ROUND(IF(tmon='S',(a.prec*v.igv),(a.prec*v.igv*nd)),2) AS costosf,b.prec AS flete,m.dmar,
    MID(descri,1,LENGTH(TRIM(descri))-LENGTH(TRIM(m.dmar))) AS descri1,prod_cod1,a.coda1,prod_tigv,
    CAST(0 AS UNSIGNED) AS costor,CAST(0 AS UNSIGNED) AS precr,''  AS moner,
    CAST(0 AS UNSIGNED) AS cost_idco,CAST(0 AS UNSIGNED) AS fleter,v.dola AS dolar,
    peso,a.prec,tipro,a.idmar,a.idcat,cost,tmon,a.idflete,prod_uti1,prod_uti2,prod_uti3,
    prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,IFNULL(o.razo,'') AS proveedor,
    ...
    LEFT JOIN fe_usua AS u ON u.idusua=a.prod_idus
    LEFT JOIN fe_usua AS us ON us.idusua=a.prod_uact,fe_gene AS v
    WHERE a.coda1 LIKE cbuscar AND prod_acti<>'I' ORDER BY DESCRI;    
END CASE;
END */$$
DELIMITER ;
...
if opt=2 then
   select idprov,nruc,razo,ndni,dire,ciud,fono,fax,email,celu,refe,prov_rpm,prov_idus,prov_actu,fechprov,prov_feac,ubig
   from fe_prov where idprov like abuscar and prov_acti<>'I' order by razo;
end if;
END */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProMuestraTodosLosProductos`(abuscar VARCHAR(80),nd FLOAT,opt INTEGER,nid INTEGER)
BEGIN
DECLARE cbuscar VARCHAR(80);
SET cbuscar=CONCAT('%',TRIM(abuscar),+'%');
case opt
when 1 THEN
    ...
    g.desgrupo AS grupo,b.desflete,u.nomb AS usuacreo,us.nomb AS usuamodifico,prod_depr,
    prod_ubi1,prod_ubi2,prod_ubi3,prod_ubi4,prod_ubi5,prod_codb,prod_cmay
    FROM fe_art  AS a 
    INNER JOIN fe_fletes AS b ON(b.idflete=a.idflete)
    INNER JOIN fe_mar AS m ON m.idmar=a.idmar
    INNER JOIN fe_cat AS c ON(c.idcat=a.idcat) 
