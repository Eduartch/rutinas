...
MySQL - 8.0.41 : Database - syscom_bdeybtr
*********************************************************************
*/
...
WHEN old.prec<>new.prec THEN
    SET cdeta=CONCAT("Cambio Precio ",CAST(old.premay AS CHAR), " Nuevo ",CAST(new.premay AS CHAR));
...
WHEN old.prod_uti1<>new.prod_uti1 THEN
    SET cdeta=CONCAT("Cambio Utilidad 1 ",CAST(old.prod_uti1 AS CHAR), " Nuevo Margen 1 ",CAST(new.prod_uti1 AS CHAR));
    INSERT INTO fe_aproductos(prod_idar,prod_idus,prod_fope,prod_deta)VALUES(old.idart,new.prod_uact,LOCALTIME,cdeta);
WHEN old.prod_uti2<>new.prod_uti2 THEN
    SET cdeta=CONCAT("Cambio Utilidad 2 ",CAST(old.prod_uti2 AS CHAR), " Nuevo Margen 2 ",CAST(new.prod_uti2 AS CHAR));
    INSERT INTO fe_aproductos(prod_idar,prod_idus,prod_fope,prod_deta)VALUES(old.idart,new.prod_uact,LOCALTIME,cdeta);
WHEN old.prod_uti3<>new.prod_uti3 THEN
    SET cdeta=CONCAT("Cambio Utilidad 3 ",CAST(old.prod_uti3 AS CHAR), " Nuevo Margen 3 ",CAST(new.prod_uti3 AS CHAR));
    INSERT INTO fe_aproductos(prod_idar,prod_idus,prod_fope,prod_deta)VALUES(old.idart,new.prod_uact,LOCALTIME,cdeta);
...
/*!50003 CREATE FUNCTION `FunCreaProyecto`(cnombre VARCHAR(100),nidclie INTEGER) RETURNS int
BEGIN
DECLARE nid INTEGER;
...
nv DECIMAL(12,2),nigv DECIMAL(12,2),nt DECIMAL(12,2),cndo2 VARCHAR(10),cm CHAR,
ndolar FLOAT,ni FLOAT,ctg CHAR,ccodp INTEGER,cmvto CHAR,nus INTEGER,opt INTEGER,nidcodt INTEGER,
n1 INTEGER,n2 INTEGER,n3 INTEGER,nidcostos INTEGER,npvta FLOAT,nexon DECIMAL(12,2),notros DECIMAL(12,2)) RETURNS int
BEGIN
DECLARE nid INTEGER;
SET nid=0;
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,
   idprov,tipom,fusua,idusua,codt,rcom_ccos,rcom_exon,rcom_otro)
   VALUES (ctdoc,cform,cndoc,dfecha,dfechar,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,LOCALTIME,nus,nidcodt,nidcostos,nexon,notros);
   SELECT LAST_INSERT_ID() INTO nid FROM fe_rcom GROUP BY LAST_INSERT_ID();
   IF n1>0 AND n2>0 AND n3>0 THEN
      CALL IngresaCuentas(nv,0,0,0,nigv,0,0,nt,n1,0,0,0,n2,0,0,n3,"D","","","","D","","","H",nid);
   END IF;
...
/* Procedure structure for procedure `PROACTUALIZADATOSDIARIOI` */

/*!50003 DROP PROCEDURE IF EXISTS  `PROACTUALIZADATOSDIARIOI` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `PROACTUALIZADATOSDIARIOI`(dfech datetime,ndebe decimal(12,2)
...
if opt=0 then
   ...
   update fe_ldiario set ldia_fech=dfech,ldia_debe=ndebe,ldia_haber=nhaber,ldia_glosa=cglosa,ldia_tipo=ct,ldia_nume=cnume,
   ldia_idcta=nidcta,ldia_cond=ccond,ldia_tran=ctran,
   ldia_item=nit,ldia_comp=ncomp,ldia_idcv=nidcl,ldia_idcc=nidpr,ldia_mone=cmone,ldia_codt=nidtda,ldia_inic='I' where ldia_idld=idd;
end if;
END */$$
DELIMITER ;
...
nv DECIMAL(12,2),nigv DECIMAL(12,2),nt DECIMAL(12,2),cndo2 VARCHAR(10),cm CHAR,
ndolar FLOAT,ni FLOAT,ctg CHAR,ccodp INTEGER,cmvto CHAR,nus INTEGER,opt INTEGER,nidcodt INTEGER,
n1 INTEGER,n2 INTEGER,n3 INTEGER,idccosto INTEGER,npvta FLOAT,nidauto INTEGER,nexon DECIMAL(12,2),notros DECIMAL(12,2))
BEGIN
   ...
   codt=nidcodt,rcom_ccos=idccosto,rcom_exon=nexon,rcom_otro=notros WHERE idauto=nidauto;
END */$$
DELIMITER ;
...
SELECT gene_idce INTO idce FROM fe_gene WHERE idgene=1;
   ...
   rcom_ccos=idccosto,rcom_exon=nexon,rcom_otro=notros WHERE idauto=nidauto;
   CALL ProDesactivaLcajaE(nidauto);
 END */$$
DELIMITER ;
...
a.cban_idpr,cban_clpr,a.cban_idca,cban_dola,cban_idco from fe_cbancos as a 
