...
MySQL - 8.0.39 : Database - syscom_bdcyhexp
*********************************************************************
*/
...
nv float,nigv float,nt float,cndo2 varchar(10),cm char,
ndolar float,ni float,ctg char,ccodp integer,cmvto char,nus integer,opt integer,nidcodt integer,
n1 integer,n2 integer,n3 integer,nitem integer,npvta decimal(12,2),nvendedor integer) RETURNS int
BEGIN
...
n1 integer,n2 integer,n3 integer,nitem integer,idtr decimal(10,2),nexon decimal(12,2),ndscto decimal(12,2),nvend integer) RETURNS int
BEGIN
declare nid integer;
set nid=0;
INSERT INTO fe_correvtas(auto_corr)VALUES(CONCAT(ctdoc,cndoc));
...
/*!50003 CREATE FUNCTION `FunRegistraRSeries`(cserie varchar(60),nidauto integer) RETURNS int
Begin
Declare nid integer default 0;
...
nv decimal(12,2),nigv decimal(12,2),nt decimal(12,2),cndo2 varchar(10),cm char,
ndolar decimal(6,4),ni decimal(6,4),ctg char,ccodp integer,cmvto char,nus integer,nidcodt integer,
n1 integer,n2 integer,n3 integer,nvend integer,idtr decimal(10,2),nexon decimal(12,2),ndscto decimal(12,2)) RETURNS int
BEGIN
declare nid integer;
set nid=0;
INSERT INTO fe_correvtas(auto_corr)VALUES(CONCAT(ctdoc,cndoc));
...
/*!50003 CREATE FUNCTION `FuncreaProductos`(cdesc varchar(150),cunid varchar(20),nprec decimal(13,8),ncosto decimal(13,8),
np1 float,np2 float,np3 float,npeso float,ccat integer,cmar integer,ctipro char,nflete integer,cm char,cidpc varchar(45),
ncome float,tigv decimal(6,3),nutil1 DECIMAL(10,8),nutil2 DECIMAL(10,8),nutil3 DECIMAL(10,8),nidusua integer,nsmin float,nsmax float,
ccoda varchar(20),ndolar float) RETURNS int
BEGIN
declare nid integer default 0;
INSERT INTO fe_art(descri,unid,prec,cost,premay,premen,pre3,peso,idcat,idmar,tipro,idflete,tmon,fechc,idpc,prod_come,
...
BEGIN
DELETE FROM fe_dseries;
ALTER TABLE fe_dseries AUTO_INCREMENT=1;
DELETE FROM fe_rseries;
ALTER TABLE fe_rseries AUTO_INCREMENT=1;
...
/*!50003 CREATE PROCEDURE `ProActualizaProductos`(cdesc varchar(150),cunid varchar(20),ncosto decimal(13,8),
np1 float,np2 float,
np3 float,npeso float,ccat integer,cmar integer,ctipro char,nflete integer,cm char,nprecio decimal(12,8),
ccoda varchar(20),nutil1 decimal(10,8),nutil2 decimal(10,8),nutil3 decimal(10,8),ncome float,
...
/*!50003 CREATE PROCEDURE `ProMuestraProductos10`(abuscar VARCHAR(80),nd FLOAT,opt INTEGER,nid VARCHAR(20))
BEGIN
DECLARE cbuscar VARCHAR(80);
case 
when opt=0 then
    ...
    IFNULL(ROUND(IF(tmon='S',((a.prec*prod_tigv)+b.prec)*prod_uti1,((a.prec*prod_tigv*nd)+b.prec)*prod_uti1),3),0) AS pre1,
    IFNULL(ROUND(IF(tmon='S',((a.prec*prod_tigv)+b.prec)*prod_uti2,((a.prec*prod_tigv*nd)+b.prec)*prod_uti2),3),0) AS pre2,
    IFNULL(ROUND(IF(tmon='S',((a.prec*prod_tigv)+b.prec)*prod_uti3,((a.prec*prod_tigv*nd)+b.prec)*prod_uti3),3),0) AS pre3,
    ROUND(IF(tmon='S',(a.prec*prod_tigv)+b.prec,(a.prec*prod_tigv*nd)+b.prec),3) AS costo,c.idgrupo,c.dcat,prod_dola,prod_cod1 AS cod1,
    ROUND(IF(tmon='S',(a.prec*prod_tigv),(a.prec*prod_tigv*nd)),3) AS costosf,b.prec AS flete,prod_cod1,
    IFNULL(d.cost_cost,0) AS costor,IFNULL(d.cost_prec,0) AS precr,IFNULL(d.cost_mone,'')  AS moner,
    CAST(IFNULL(d.cost_idco,0) AS UNSIGNED) AS cost_idco,IFNULL(d.cost_flet,0)  AS fleter,IFNULL(d.cost_dola,0) AS dolar,
...
when opt=1 THEN
    SET cbuscar=CONCAT('%',TRIM(abuscar),+'%');
    SELECT idart,descri,unid,uno,dos,tre,cua,cero,
    cast(ROUND(IF(tmon='S',((a.prec*prod_tigv)+b.prec)*prod_uti1,((a.prec*prod_tigv*nd)+b.prec)*prod_uti1),4) as decimal(10,4)) AS pre1,
    cast(ROUND(IF(tmon='S',((a.prec*prod_tigv)+b.prec)*prod_uti2,((a.prec*prod_tigv*nd)+b.prec)*prod_uti2),4) as decimal(10,4)) AS pre2,
    cast(ROUND(IF(tmon='S',((a.prec*prod_tigv)+b.prec)*prod_uti3,((a.prec*prod_tigv*nd)+b.prec)*prod_uti3),4) AS DECIMAL(10,4)) AS pre3,
    ROUND(IF(tmon='S',(a.prec*prod_tigv)+b.prec,(a.prec*prod_tigv*nd)+b.prec),4) AS costo,c.idgrupo,c.dcat,prod_dola,prod_cod1 AS cod1,
    ROUND(IF(tmon='S',(a.prec*prod_tigv),(a.prec*prod_tigv*nd)),3) AS costosf,b.prec AS flete,prod_cod1,
    IFNULL(d.cost_cost,0) AS costor,IFNULL(d.cost_prec,0) AS precr,IFNULL(d.cost_mone,'')  AS moner,
    CAST(IFNULL(d.cost_idco,0) AS UNSIGNED) AS cost_idco,IFNULL(d.cost_flet,0)  AS fleter,IFNULL(d.cost_dola,0) AS dolar,
    peso,a.prec,tipro,idmar,a.idcat,cost,tmon,a.idflete,prod_uti1,prod_uti2,prod_uti3,
