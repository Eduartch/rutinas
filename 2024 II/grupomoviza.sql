...
MySQL - 5.7.42 : Database - eduar_bdgpomoviza
*********************************************************************
*/
...
if new.acti='I' then
   ...
   update fe_lcaja set lcaj_acti='I' where lcaj_idau=old.idauto and lcaj_acti='A';
   update fe_serviciocorte set serv_acti='I' where serv_idau=old.idauto;
...
/*!50003 CREATE FUNCTION `FunCreaAlmacen`(cnomb varchar(50),cdire varchar(100),cciud varchar(50),nser integer,nidus integer) RETURNS int(11)
begin
...
/*!50003 CREATE FUNCTION `FunCreaEpta`(niart integer,idp integer,nprec decimal(12,2),ncant float,nprecio1 decimal(12,2)) RETURNS int(11)
BEGIN
declare idep integer default 0;
insert into fe_epta(epta_idar,epta_pres,epta_prec,epta_cant,epta_pre1)values(niart,idp,nprec,ncant,nprecio1);
...
/*!50003 CREATE FUNCTION `FunCreaPresentaciones`(cdesc varchar(60),ncant float) RETURNS int(11)
BEGIN
...
equi1 float,equi2 float,ndetra decimal(6,4)) RETURNS int(11)
BEGIN
declare nid integer default 0;
INSERT INTO fe_art(descri,unid,prec,cost,peso,idcat,idmar,tipro,idflete,tmon,fechc,idpc,prod_come,
prod_comc,prod_idus,prod_smin,prod_smax,prod_idco,prod_dola,prod_unid1,prod_equi1,prod_equi2,prod_detr)
VALUES (cdesc,cunid,nprec,ncosto,npeso,ccat,cmar,ctipro,nflete,cm,localtime,
cidpc,ncome,ncomc,nidusua,nsmin,nsmax,nidcosto,ndolar,cuni1,equi1,equi2,ndetra);
...
/*!50003 CREATE FUNCTION `FuncreaProductos1`(cdesc varchar(120),cunid varchar(4),nprec float,ncosto float,
npeso float,ccat integer,cmar integer,ctipro char,nflete integer,cm char,cidpc varchar(45),
nidgrupo integer,ncome float,ncomc float,nidusua integer,nsmin float,nsmax float,ccoda1 varchar(6),ndolar float) RETURNS int(11)
BEGIN
declare nid integer default 0;
INSERT INTO fe_art(descri,unid,prec,cost,peso,idcat,idmar,tipro,idflete,tmon,fechc,idpc,prod_come,
...
declare nid integer default 0;
if ctp='p' then
   INSERT INTO fe_rped(fech,idclie,ndoc,tdoc,impo,form,rped_idus,idpcped,fecho,idven,idtienda,tipopedido,aten,forma,plazo,validez,entrega,detalle,rped_mone,rped_serv)
   VALUES(dfech,nidclie,cndoc,ctdoc,nimpo,cform,cusua,cidpcped,localtime,nidven,nidtienda,ctp,caten,cforma,cplazo,cvalidez,centrega,"",cmone,left(cdetalle,1));
else
...
if opt=0 then
   ...
   dolar,vigv,tcom,idprov,tipom,fusua,idusua,codt,rcom_nitem,pimpo)
   VALUES (ctdoc,cform,cndoc,dfecha,dfechar,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,
   ni,ctg,ccodp,cmvto,localtime,nus,nidcodt,nitem,npvta);
   ...
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,
   dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,rcom_nitem,pimpo)
   VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,
   ctg,ccodp,cmvto,localtime,nus,nidcodt,nitem,npvta);
   select last_insert_id() into nid from fe_rcom group by last_insert_id();
   if n1>0 and n2>0 and n3>0 then
...
/* Function  structure for function  `FunIngresaCabeceraCV1` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaCabeceraCV1` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaCabeceraCV1`(
ctdoc varchar(2),cform char,cndoc varchar(10),dfecha date,dfechar date,cdetalle varchar(120),
nv float,nigv float,nt float,cndo2 varchar(10),cm char,
ndolar float,ni float,ctg char,ccodp integer,cmvto char,nus integer,opt integer,nidcodt integer,
n1 integer,n2 integer,n3 integer,nitem integer,npvta float,nidin integer) RETURNS int(11)
BEGIN
...
n1 INTEGER,n2 INTEGER,n3 INTEGER,ctipovta char,npvta decimal(10,2),nicbper DECIMAL(6,2)) RETURNS int(11)
BEGIN
DECLARE nid INTEGER;
SET nid=0;
INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,
dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,pimpo,rcom_icbper,exon)
VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,
ctg,ccodp,cmvto,LOCALTIME,nus,nidcodt,npvta,nicbper,ctipovta);
SELECT LAST_INSERT_ID() INTO nid FROM fe_rcom GROUP BY LAST_INSERT_ID();
 IF n1>0 AND n2>0 AND n3>0 THEN
      CALL IngresaCuentasV(nv,nigv,nt,n1,n2,n3,"H","H","D",nid);
   END IF;
...
nidauto integer,nidart integer,nflete float,nprec float,cmone char,ndola float) RETURNS int(11)
BEGIN
declare nid integer;
...
/*!50003 CREATE FUNCTION `FunIngresaDatosLcajaE1`(dfecha date,cndoc varchar(12),cdeta varchar(100),idcta integer,sdeudor decimal(12,2),
...
cdcto char(15),nidtda integer) RETURNS int(11)
begin
declare nid integer;
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
...
cform char,cdcto char(15),nidtda integer) RETURNS int(11)
begin
declare nid integer;
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
...
sacreedor DECIMAL(12,2),cmone CHAR,ndolar DECIMAL(5,3),nidus INTEGER,nidcp INTEGER,nidauto INTEGER,
cform CHAR,cdcto VARCHAR(12),ctdoc VARCHAR(2),ncodt INTEGER) RETURNS int(10)
BEGIN
DECLARE nid INTEGER DEFAULT 0;
...
cdeta VARCHAR(100),idcta INTEGER,sdeudor DECIMAL(12,2),
sacreedor DECIMAL(12,2),cmone CHAR,ndolar DECIMAL(5,3),nidus INTEGER,nidcp INTEGER,nidauto INTEGER,
cform CHAR,cdcto VARCHAR(12),ctdoc VARCHAR(2),ncodt INTEGER,nturno INTEGER,ndscto DECIMAL(12,2),nidtran INTEGER) RETURNS int(10)
BEGIN
DECLARE nid INTEGER DEFAULT 0;
INSERT INTO fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_adebe,lcaj_deud,lcaj_mone,lcaj_dola,
lcaj_idus,lcaj_clpr,lcaj_idau,lcaj_form,lcaj_dcto,lcaj_tdoc,lcaj_codt,lcaj_idtra,lcaj_fope,lcaj_acre,lcaj_ahaber)VALUES
(dfecha,cndoc,cdeta,idcta,0,sdeudor,cmone,ndolar,nidus,nidcp,nidauto,cform,cdcto,ctdoc,ncodt,nidtran,LOCALTIME,sacreedor,sacreedor);
SELECT LAST_INSERT_ID() INTO nid FROM fe_lcaja GROUP BY LAST_INSERT_ID();
RETURN nid;
END */$$
DELIMITER ;
...
/*!50003 CREATE FUNCTION `FunIngresaDPedidos`(ncoda integer,ncant float,nprec float,nidauto integer,epta integer,equi decimal(12,6),npos integer) RETURNS int(11)
BEGIN
declare id integer default 0;
INSERT INTO fe_ped(idart,cant,prec,idautop,dped_epta,dped_equi,dped_posi)
VALUES(ncoda,ncant,nprec,nidauto,epta,equi,npos);
select last_insert_id() into id from fe_ped group by last_insert_id();
return id;
END */$$
DELIMITER ;
...
/*!50003 CREATE FUNCTION `FunIngresaGuiasT`(`dfecha` DATETIME, `cptop` VARCHAR(100), `cptoll` VARCHAR(150), `nidauto` INTEGER, `dfechat` DATETIME, `nidus` INTEGER, `cdeta` VARCHAR(150), `nidtr` INTEGER,
 `cndoc` VARCHAR(12), `nidt` INTEGER) RETURNS int(11)
BEGIN
DECLARE id INTEGER;
...
VALUES(dfecha,cptop,cptoll,nidauto,dfechat,nidus,LOCALTIME,cdeta,nidtr,cndoc,'T',nidt,"");
SELECT LAST_INSERT_ID() INTO id FROM  fe_guias GROUP BY LAST_INSERT_ID();
RETURN id;
END */$$
DELIMITER ;
...
/*!50003 CREATE FUNCTION `FunIngresaGuiasxComprasRemitente`(dfecha DATETIME,cptop VARCHAR(150),cptoll VARCHAR(150),nidauto INTEGER,
dfechat DATETIME,nidus INTEGER,cdeta VARCHAR(150),nidtr INTEGER,cndoc VARCHAR(12),nidtda INTEGER,cdcto VARCHAR(30),df DATE,nidpr INTEGER,cubigeo VARCHAR(8)) RETURNS int(11)
BEGIN
DECLARE id INTEGER;
...
dfechat DATE,nidus INTEGER,cdeta VARCHAR(150),nidtr INTEGER,cndoc VARCHAR(12),nidtda INTEGER,cubigeo VARCHAR(8)) RETURNS int(11)
BEGIN
DECLARE id INTEGER;
INSERT INTO fe_guias(guia_fech,guia_ptop,guia_ptoll,guia_idau,guia_fect,guia_idus,guia_fope,guia_deta,guia_idtr,guia_ndoc,guia_moti,guia_codt,guia_ubig)
...
nct float,cincl char,ccodv integer,ct1 char,cdeta varchar(50),nidtda integer,nidtda1 integer,na1 integer,nequi decimal(6,2)) RETURNS int(11)
BEGIN
declare nidk integer default 0;
...
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 integer,vcom float,nequi decimal(12,8),cuni varchar(12),epta integer,npos integer) RETURNS int(11)
BEGIN
declare nidk integer default 0;
if ct='C' then
  ...
  INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_idco,codv,kar_alma1,kar_comi,kar_equi,kar_unid,kar_epta,kar_posi)
  VALUES (nid,cc,ct,npr,nct,tmvto,cincl,calma,nidcosto1,ccodv,0,vcom,nequi,cuni,epta,npos);
end if;
select last_insert_id() into nidk from fe_kar group by last_insert_id();
return nidk;
END */$$
...
nct FLOAT,cincl CHAR,nchek INTEGER,ccodv INTEGER,calma INTEGER,nidcosto1 INTEGER,vcom FLOAT,nequi DECIMAL(12,8),
cuni VARCHAR(30),epta INTEGER,npos INTEGER,ncosto DECIMAL(12,4),nigv DECIMAL(6,4)) RETURNS int(11)
BEGIN
DECLARE nidk INTEGER DEFAULT 0;
INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_idco,codv,kar_alma1,kar_comi,
kar_equi,kar_unid,kar_epta,kar_posi,kar_tigv,kar_cost,kar_icbper,kar_chek)
VALUES (nid,cc,'V',npr,nct,'K',cincl,calma,nidcosto1,ccodv,0,vcom,nequi,cuni,epta,npos,nigv,ncosto,nicbper,nchek);
SELECT LAST_INSERT_ID() INTO nidk FROM fe_kar GROUP BY LAST_INSERT_ID();
RETURN nidk;
END */$$
DELIMITER ;

/* Function  structure for function  `FunIngresaKardexMarca` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaKardexMarca` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaKardexMarca`(nid integer,cc integer,ct char,npr float,
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 integer,vcom float,nequi decimal(12,8),cuni varchar(12),
epta integer,npos integer,chkkardex integer) RETURNS int(11)
BEGIN
declare nidk integer default 0;
if ct='C' then
  INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_idco,codv,kar_alma1,kar_equi,kar_unid,kar_epta,kar_posi,kar_chek)
  VALUES (nid,cc,ct,npr,nct,tmvto,cincl,calma,nidcosto1,ccodv,calma,nequi,cuni,epta,npos,chkkardex);
 else
  INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_idco,codv,kar_alma1,kar_comi,kar_equi,kar_unid,kar_epta,kar_posi,kar_chek)
  VALUES (nid,cc,ct,npr,nct,tmvto,cincl,calma,nidcosto1,ccodv,0,vcom,nequi,cuni,epta,npos,chkkardex);
end if;
select last_insert_id() into nidk from fe_kar group by last_insert_id();
return nidk;
END */$$
...
/*!50003 CREATE FUNCTION `FUNINGRESANOTASCREDITOventas1`(nid0 integer,nid1 integer,nidcr integer) RETURNS int(11)
BEGIN
declare id integer default 0;
...
dfevto DATETIME,nacta FLOAT,cndoc VARCHAR(14),cesta CHAR,cmone CHAR,cb1 VARCHAR(100),ctipo CHAR,
...
SET nid=0;
   ...
   idprov,tipom,fusua,idusua,codt,rcom_exon,rcom_otro)
   VALUES (ctdoc,cform,cndoc,dfecha,dfechar,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,LOCALTIME,nus,nidcodt,nexon,notros);
   SELECT LAST_INSERT_ID() INTO nid FROM fe_rcom GROUP BY LAST_INSERT_ID();
   IF n1>0 AND n2>0 AND n3>0 THEN
      CALL IngresaCuentas(nv,0,0,0,nigv,0,0,nt,n1,0,0,0,n2,0,0,n3,"D","","","","D","","","H",nid);
   END IF;
...
nidus integer,nidtda integer,ninic float,cpc varchar(45),cform char) RETURNS int(11)
BEGIN
...
idven integer,nimpoo decimal(12,2),nidus integer,nidtda integer,cpc varchar(45),cform char) RETURNS int(11)
BEGIN
declare id integer default 0;
declare id1 integer default 0;
insert into fe_rcred(rcre_idcl,rcre_fech,rcre_idau,rcre_impc,rcre_idus,rcre_codt,rcre_idpc,rcre_inic,rcre_codv,rcre_form)
...
/*!50003 CREATE FUNCTION `FunVerificaSiGuiaEstaIngresada`(nd VARCHAR(12)) RETURNS int(11)
BEGIN
DECLARE vdvto INTEGER DEFAULT 0;
DECLARE CONTINUE HANDLER FOR NOT FOUND SET vdvto=0;
...
/*!50003 CREATE PROCEDURE `astock`(coda integer,nalma integer, ccant float,ctipo char(1),equi float)
BEGIN
case 
   when ctipo="C" then
      if nalma=1 then
      ...
      IF nalma=5 THEN
          UPDATE fe_art SET cin=cin+(ccant*equi) WHERE idart=coda;
     END IF;
         IF nalma=6 THEN
          UPDATE fe_art SET sei=sei+(ccant*equi) WHERE idart=coda;
     END IF;
when ctipo="V" then
      if nalma=1 then
          UPDATE fe_art SET uno=uno-(ccant*equi) WHERE idart=coda;
 ...
 when ctipo="I" then
      ...
      IF nalma=5 THEN
          UPDATE fe_art SET cin=ccant,
          prod_quniM=IF(ccant>0,IF(MOD(ccant,equi)=0,ccant/equi,ccant DIV equi),0),
          prod_qunin=IF(ccant>0,IF(MOD(ccant,equi)=0,0,MOD(ccant,equi)),ccant) WHERE idart=coda;
     END IF;
     IF nalma=6 THEN
          UPDATE fe_art SET sei=ccant,
          prod_suniM=IF(ccant>0,IF(MOD(ccant,equi)=0,ccant/equi,ccant DIV equi),0),
          prod_sunin=IF(ccant>0,IF(MOD(ccant,equi)=0,0,MOD(ccant,equi)),ccant) WHERE idart=coda;
     END IF;
end case;
END */$$
DELIMITER ;
...
b.acti<>'I'  and e.acti='A' group by  idart,alma) as a where (a.tcompras-a.tventas)>0;
DECLARE CONTINUE HANDLER FOR SQLSTATE '02000' SET done = 1;
open cursor1;
start transaction;
UPDATE fe_art SET uno=0,dos=0,tre=0,cua=0,cin=0,prod_unim=0,prod_unin=0,
prod_dunim=0,prod_dunin=0,prod_tunim=0,prod_tunin=0,prod_cunim=0,prod_cunin=0,prod_qunim=0,prod_qunin=0;
repeat
...
nv decimal(12,2),nigv decimal(12,2),nt decimal(12,2),cndo2 varchar(10),cm char,
ndolar float,ni float,ctg char,ccodp integer,cmvto char,nus integer,opt integer,nidcodt integer,
n1 integer,n2 integer,n3 integer,ctipovta char,npvta float,nidauto integer)
BEGIN
...
if opt=0 then
   ...
   select gene_idve into idve from fe_gene where idgene=1;
   update fe_rcom set tdoc=ctdoc,form=cform,ndoc=cndoc,fech=dfecha,fecr=dfechar,deta=cdetalle,valor=nv,igv=nigv,impo=nt,ndo2=cndo2,
   mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,exon=ctipovta,rcom_arch="" where idauto=nidauto;
   call ProDesactivaLcajaE(nidauto);
   if n1>0 and n2>0 and n3>0 then
...
n3 INTEGER,ctipovta char,npvta FLOAT,nidauto INTEGER)
BEGIN
...
mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,rcom_icbper=nicbper,exon=ctipovta WHERE idauto=nidauto;
CALL ProDesactivaLcajaE(nidauto);
IF n1>0 AND n2>0 AND n3>0 THEN
...
BEGIN
update fe_autorizado set auto_idau=nidauto where auto_idpe=nidp;
END */$$
DELIMITER ;
...
if ctp='p' then
  ...
  tipopedido='p',aten=caten,forma=cforma,plazo=cplazo,validez=cvalidez,entrega=centrega,detalle="",rped_serv=left(cdetalle,1) WHERE idautop=nidauto;
else
  UPDATE fe_rped SET fech=dfech,idclie=nidclie,ndoc=cndoc,impo=nimpo,form=cform,idven=nidven,facturado='N',tdoc=ctdoc,
...
/*!50003 CREATE PROCEDURE `ProActualizaDetallePedidos`(ncoda integer,ncant float,nprec float,nr integer,idepta integer,equi decimal(8,4),ctipoa char,npos integer)
BEGIN
if ctipoa='A' then
     UPDATE fe_ped SET acti='I' WHERE idped=nr;
   else
     UPDATE fe_ped SET idart=ncoda,cant=ncant,prec=nprec,
     dped_equi=equi,dped_epta=idepta,dped_posi=npos WHERE idped=nr;
end if;
END */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProActualizaEpta`(niart integer,idp integer,nprec decimal(12,2),ncant float,idepta integer,
opt integer,nprecio1 decimal(12,2))
BEGIN
if opt=0 then
   update fe_epta set epta_acti='I' where epta_idep=idepta;
  else
   update fe_epta set epta_idar=niart,epta_pres=idp,epta_prec=nprec,epta_cant=ncant,epta_pre1=nprecio1 where epta_idep=idepta;
end if;
END */$$
DELIMITER ;
...
nidkar integer,op integer,nequi float,cunid varchar(8),nidepta integer,xcom float,npos integer)
BEGIN
if op=0 then
  ...
  kar_equi=nequi,kar_unid=cunid,kar_epta=nidepta,kar_posi=npos where idkar=nidkar;
end if;
END */$$
DELIMITER ;
...
nct FLOAT,cincl CHAR,nchek INTEGER,ccodv INTEGER,calma INTEGER,nidcosto1 INTEGER,
...
IF op=0 THEN
 ...
 ELSE
  UPDATE fe_kar SET
  idauto=nid,idart=cc,kar_icbper=nicbper,prec=npr,cant=nct,ttip='K',incl=cincl,
  alma=calma,kar_idco=nidcosto1,codv=ccodv,kar_comi=xcom,
  kar_equi=nequi,kar_unid=cunid,kar_epta=nidepta,kar_posi=npos,kar_cost=ncosto,kar_tigv=nigv,kar_chek=nchek WHERE idkar=nidkar;
END IF;
END */$$
DELIMITER ;

/* Procedure structure for procedure `ProActualizaKardexMarca` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizaKardexMarca` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizaKardexMarca`(nid integer,cc integer,ct char,npr float,
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 integer,
nidkar integer,op integer,nequi float,cunid varchar(8),nidepta integer,xcom float,npos integer,chkkardex integer)
BEGIN
if op=0 then
  Update fe_kar set Acti='I' where idkar=nidkar;
 else
  ...
  idauto=nid,idart=cc,tipo=ct,prec=npr,cant=nct,ttip=tmvto,incl=cincl,
  alma=calma,kar_idco=nidcosto1,codv=ccodv,kar_comi=xcom,
  kar_equi=nequi,kar_unid=cunid,kar_epta=nidepta,kar_posi=npos,kar_chek=chkkardex where idkar=nidkar;
end if;
END */$$
DELIMITER ;
...
declare vigv decimal(6,3);
select igv into vigv from fe_gene where idgene=1;
SELECT ulfc into @ufc FROM fe_art WHERE idart=cc;
   ...
   prod_idco=nidcosto,prod_cost=npr*vigv WHERE idart=cc;
END */$$
DELIMITER ;
...
npeso float,ccat integer,cmar integer,ctipro char,nflete integer,cm char,cidpc varchar(45),
ncome float,ncomc float,nidusua integer,nsmin float,nsmax float,nidcosto integer,ndolar decimal(6,4),cuni1 varchar(8),
equi1 decimal(12,2),equi2 decimal(12,2),ncoda integer,ndetra decimal(6,4))
BEGIN
...
prod_smin=nsmin,prod_idco=nidcosto,prod_dola=ndolar,prod_unid1=cuni1,prod_equi1=equi1,prod_equi2=equi2,prod_detr=ndetra,
prod_cost=nprec*vigv WHERE idart=ncoda;
END */$$
DELIMITER ;
...
BEGIN
   ...
   ndo2=cndo2,idusua1=nus,codt=nidcodt,tcom=ctg,tipom=cmvto WHERE idauto=nidauto;
...
ctipo CHAR,nidauto INTEGER,nu INTEGER,sw CHAR,dfecha DATE, nu1 INTEGER,cdeta VARCHAR(200))
BEGIN
DECLARE nid INTEGER;
...
IF nidauto=0 THEN
   ...
   SET @cn1:=cndoc;
   SET @df:=dfecha;
   SELECT @idclave:=idauto,@nidalma:=codt FROM fe_rcom WHERE tdoc=ctdoc AND ndoc=cndoc AND tipom =ctipo;
   SET nid=@idclave;
  ELSE
   SET nid=nidauto;
   SELECT @ct:=tdoc,@cn1:=ndoc,@df:=fech,@nidalma:=codt FROM fe_rcom WHERE idauto=nid;
END IF;
IF nid>0 THEN
   ...
   UPDATE fe_traspaso SET tras_acti='I' WHERE tras_idau1=nid;
    IF ctipo='V' THEN
      ...
      SELECT @nguia:=guia_idgui FROM fe_guias WHERE guia_idau=nid limit 1;
      IF @nguia>0 THEN
         CALL ProAnulaEntregaFisica(@nguia,nu);
      END IF;
      IF sw='S'  THEN
         CALL PROingresa_anulada(@df,@cn1,@ct,nu,cdeta,@nidalma);
       END IF;
    ELSE
...
begin
select rcom_idin from fe_rcom where rcom_idin=nidauto and acti='A' and codt=nidt;
end */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProEditaAlmacen`(cnomb VARCHAR(80),cdire VARCHAR(100),cciud VARCHAR(100),
nser INTEGER,nidus INTEGER,nid INTEGER,nmetavtas decimal(12,2))
BEGIN
UPDATE fe_sucu SET nomb=cnomb,dire=cdire,ciud=cciud,sucuidserie=nser,sucu_idus=nidus,sucu_meta=nmetavtas WHERE idalma=nid;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProGrabaIncidencia` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProGrabaIncidencia` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProGrabaIncidencia`(nidus integer,nidua integer,nidautop integer,nidauto integer,cdeta varchar(200))
begin
insert into fe_incidencia(inci_idua,inci_idus,inci_idap,inci_idau,inci_deta,inci_fope)
values(nidua,nidus,nidautop,nidauto,cdeta,localtime);
end */$$
DELIMITER ;
...
sacreedor DECIMAL(12,2),cmone CHAR,ndolar DECIMAL(5,3),nidus INTEGER,nidcp INTEGER,nidauto INTEGER,cform CHAR,
cdcto VARCHAR(14),ctdoc VARCHAR(2),nidtda INTEGER)
BEGIN
INSERT INTO fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
...
/*!50003 CREATE PROCEDURE `Proingresa_anulada`(dfecha Datetime,cndoc varchar(12),ctdoc varchar(2),
nidus integer,cglosa varchar(200), nidalma integer)
begin
 ...
 values(@nc,dfecha,dfecha,cndoc,ctdoc,'V','','','K','E','S','N',localtime,nidus,nidalma);
 SELECT @na:=LAST_INSERT_ID() FROM fe_rcom group by LAST_INSERT_ID();
 insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
 lcaj_idus,lcaj_clpr,lcaj_idau,lcaj_form,lcaj_dcto,lcaj_tdoc,lcaj_codt,lcaj_fope)values
 (dfecha,cndoc,cglosa,0,0,0,'S',0,nidus,@nc,@na,'E',cndoc,ctdoc,nidalma,localtime);
END */$$
DELIMITER ;
...
BEGIN
SELECT nomb,idalma,dire,ciud,sucuidserie,sucu_idus,sucu_meta FROM fe_sucu  WHERE idalma IN (1,2,4,5,6) ORDER BY idalma;
END */$$
DELIMITER ;
...
BEGIN
SELECT nomb,idalma,dire,ciud,sucuidserie,sucu_meta FROM fe_sucu where idalma in (1,2,4,5,6)  order by idalma;
END */$$
DELIMITER ;
...
select banc_nomb,banc_idba,banc_idco from fe_bancos where banc_nomb like cb1  and banc_acti='A' order by banc_nomb;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `PROMuestraClientesx` */

/*!50003 DROP PROCEDURE IF EXISTS  `PROMuestraClientesx` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `PROMuestraClientesx`(in cbusca varchar(80),in opt integer,in nid integer)
BEGIN
declare cbuscar varchar(80);
set cbuscar=trim(cbusca);
if opt=0 then
   select idclie,nruc,razo,ndni,dire,ciud,ifnull(fono,'0') as fono,ifnull(fax,'0') as fax,
   ifnull(celu,'0') as celu,refe,clie_tipo,clie_codv,clie_lcre,clie_corr,clie_rpm,clie_idus,clie_actu,fechclie,clie_feac,ifnull(c.zona_nomb,'') as Zona,ifnull(a.clie_idzo,0) as clie_idzo
   from fe_clie as a left join fe_zona as c on c.zona_idzo=a.clie_idzo where  clie_acti<>'I' and trim(razo)=cbuscar order by razo;
end if;
if opt=1 then
   select idclie,nruc,razo,ndni,dire,ciud,ifnull(fono,'0') as fono,ifnull(fax,'0') as fax,
   ifnull(celu,'0') as celu,refe,clie_tipo,clie_codv,clie_lcre,clie_corr,clie_rpm,clie_idus,clie_actu,fechclie,clie_feac,ifnull(c.zona_nomb,'') as Zona,ifnull(a.clie_idzo,0) as clie_idzo
...
/* Procedure structure for procedure `ProMuestraIncidencia` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProMuestraIncidencia` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProMuestraIncidencia`(dfi date,dff date)
begin
select u.nomb as Usuario_Activo,w.nomb as Usuario_Autorizo,inci_deta as Detalle,inci_fope as Fecha from fe_incidencia as f
inner join fe_usua as u on u.idusua=f.inci_idus inner join fe_usua as w on w.idusua=f.inci_idua
where cast(inci_fope as date) between dfi and dff order by inci_fope desc;
end */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProMuestraPresentacionesP`()
BEGIN
SELECT a.pres_desc,b.epta_cant,b.epta_prec,b.epta_pre1,b.epta_pres,b.epta_idar,b.epta_idep
FROM fe_epta as b  LEFT JOIN fe_presentaciones as a  ON b.epta_pres=a.pres_idpr where b.epta_acti='A' and a.pres_acti='A';
END */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProMuestraPresentacionesXProducto`(nidart INTEGER)
BEGIN
SELECT a.pres_desc,b.epta_prec,b.epta_pre1,b.epta_cant,b.epta_idar,b.epta_idep FROM fe_epta  AS b INNER JOIN fe_presentaciones AS a
ON a.pres_idpr=b.epta_pres WHERE b.epta_acti='A' AND b.epta_idar=nidart;
END */$$
DELIMITER ;
...
CAST(IF(cin>0,IF(MOD(cin,prod_equi1)=0,cin/prod_equi1,
IF(MOD(cin,prod_equi1)=0,cin DIV prod_equi1,TRUNCATE(cin/prod_equi1,0))),0.00) AS DECIMAL(12,2)) AS prod_qunim,
CAST(IF(cin>0,IF(MOD(cin,prod_equi1)=0,0.00,MOD(cin,prod_equi1)),cin) AS DECIMAL(12,2)) AS prod_qunin,
CAST(IF(sei>0,IF(MOD(sei,prod_equi1)=0,sei/prod_equi1,
IF(MOD(sei,prod_equi1)=0,sei DIV prod_equi1,TRUNCATE(sei/prod_equi1,0))),0.00) AS DECIMAL(12,2)) AS prod_sunim,
CAST(IF(sei>0,IF(MOD(sei,prod_equi1)=0,0.00,MOD(sei,prod_equi1)),sei) AS DECIMAL(12,2)) AS prod_sunin,
ROUND(IF(tmon='S',(a.prec*g.igv)+b.prec,(a.prec*g.igv*nd)+b.prec),2) AS costo,c.idgrupo,c.dcat,
IFNULL(ROUND(IF(tmon='S',premay,((a.prec*g.igv*nd)+b.prec)*prod_uti3),2),0) AS pre1,
IFNULL(ROUND(IF(tmon='S',premen,((a.prec*g.igv*nd)+b.prec)*prod_uti2),2),0) AS pre2,
IFNULL(ROUND(IF(tmon='S',pre3,((a.prec*g.igv*nd)+b.prec)*prod_uti1),2),0) AS pre3,
ROUND(IF(tmon='S',(a.prec*g.igv),(a.prec*g.igv*nd)),2) AS costosf,b.prec AS flete,ulfc,uno,dos,tre,cua,cin,sei,prod_detr,
IFNULL(d.cost_cost,0) AS costor,IFNULL(d.cost_prec,0) AS precr,IFNULL(d.cost_mone,'')  AS moner,
CAST(IFNULL(d.cost_idco,0) AS UNSIGNED) AS cost_idco,IFNULL(d.cost_flet,0)  AS fleter,IFNULL(d.cost_dola,0) AS dolar,
...
IFNULL(yy.ndoc,'') AS ndoc,IFNULL(yy.fech,'') AS fech,prod_cost,prod_icbper,u.nomb AS usuacreo,uu.nomb AS usuamodifico
FROM fe_art  AS a 
...
LEFT JOIN fe_usua AS uu ON uu.idusua=a.prod_uact
,fe_gene AS g
...
/*!50003 CREATE PROCEDURE `ProMuestraProductosConStock`(abuscar varchar(80),nd float,nalma integer)
BEGIN
declare cbuscar varchar(80);
set cbuscar=concat('%',trim(abuscar),+'%');
case
  when nalma=1 then
        ...
        IF(MOD(sei,prod_equi1)=0,sei DIV prod_equi1,TRUNCATE(sei/prod_equi1,0))),0.00) AS DECIMAL(12,2)) AS prod_sunim,
        CAST(IF(sei>0,IF(MOD(sei,prod_equi1)=0,0.00,MOD(sei,prod_equi1)),sei) AS DECIMAL(12,2)) AS prod_sunin,
        if(tmon='S',prod_cost,prod_cost*nd) as costo,
        c.idgrupo,c.dcat,
        ifnull(round(if(tmon='S',premay,((a.prec*g.igv*nd)+b.prec)*prod_uti3),2),0) as pre1,
        ifnull(round(if(tmon='S',premen,((a.prec*g.igv*nd)+b.prec)*prod_uti2),2),0) as pre2,
        ifnull(round(if(tmon='S',pre3,((a.prec*g.igv*nd)+b.prec)*prod_uti1),2),0) as pre3,
        ...
        peso,a.prec,tipro,idmar,a.idcat,cost,tmon,a.idflete,prod_uti1,prod_uti2,prod_uti3,prod_idus,prod_equi1,prod_equi2,
        prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,ifnull(o.razo,'') as proveedor,
        ifnull(yy.ndoc,'') as ndoc,ifnull(yy.fech,'') as fech,prod_icbper
        FROM fe_art  as a 
        inner join fe_fletes as b on(b.idflete=a.idflete)
        inner join fe_cat as c on(c.idcat=a.idcat) 
        left join fe_costos as d on(d.cost_idco=a.prod_idco)
        ...
        WHERE descri LIKE cbuscar and prod_acti<>'I' and (uno>0 or tre>0) ORDER BY DESCRI;
when nalma=2 then
        SELECT idart,descri,unid,prod_unid1,
        cast(IF(uno>0,IF(MOD(uno,prod_equi1)=0,uno/prod_equi1,
        if(mod(uno,prod_equi1)=0,uno div prod_equi1,truncate(uno/prod_equi1,0))),0.00) as decimal(12,2)) as prod_unim,
        ...
        IF(MOD(sei,prod_equi1)=0,sei DIV prod_equi1,TRUNCATE(sei/prod_equi1,0))),0.00) AS DECIMAL(12,2)) AS prod_sunim,
        CAST(IF(sei>0,IF(MOD(sei,prod_equi1)=0,0.00,MOD(sei,prod_equi1)),sei) AS DECIMAL(12,2)) AS prod_sunin,
        IF(tmon='S',prod_cost,prod_cost*nd) AS costo,c.idgrupo,c.dcat,
        ifnull(round(if(tmon='S',premay,((a.prec*g.igv*nd)+b.prec)*prod_uti3),2),0) as pre1,
        ifnull(round(if(tmon='S',premen,((a.prec*g.igv*nd)+b.prec)*prod_uti2),2),0) as pre2,
        ifnull(round(if(tmon='S',pre3,((a.prec*g.igv*nd)+b.prec)*prod_uti1),2),0) as pre3,
        round(if(tmon='S',(a.prec*g.igv),(a.prec*g.igv*nd)),2) as costosf,b.prec as flete,ulfc,uno,dos,tre,cua,cin,sei,prod_detr,
        ...
        left join fe_rcom as yy on (yy.idauto=a.prod_idau) 
        left join fe_prov as o on (o.idprov=yy.idprov), fe_gene as g
        WHERE descri LIKE cbuscar and prod_acti<>'I' and dos>0 ORDER BY DESCRI;
when nalma=3 then
        SELECT idart,descri,unid,prod_unid1,
        cast(IF(uno>0,IF(MOD(uno,prod_equi1)=0,uno/prod_equi1,
        if(mod(uno,prod_equi1)=0,uno div prod_equi1,truncate(uno/prod_equi1,0))),0.00) as decimal(12,2)) as prod_unim,
        ...
        left join fe_rcom as yy on (yy.idauto=a.prod_idau) 
        left join fe_prov as o on (o.idprov=yy.idprov), fe_gene as g
        WHERE descri LIKE cbuscar and prod_acti<>'I' and (tre>0 or uno>0) ORDER BY DESCRI;
WHEN nalma=4 THEN
        SELECT idart,descri,unid,prod_unid1,
        cast(IF(uno>0,IF(MOD(uno,prod_equi1)=0,uno/prod_equi1,
        if(mod(uno,prod_equi1)=0,uno div prod_equi1,truncate(uno/prod_equi1,0))),0.00) as decimal(12,2)) as prod_unim,
        ...
        left join fe_rcom as yy on (yy.idauto=a.prod_idau) 
        left join fe_prov as o on (o.idprov=yy.idprov), fe_gene as g
        WHERE descri LIKE cbuscar and prod_acti<>'I' and cua>0 ORDER BY DESCRI;
 when nalma=5 then
        SELECT idart,descri,unid,prod_unid1,
        CAST(IF(uno>0,IF(MOD(uno,prod_equi1)=0,uno/prod_equi1,
        IF(MOD(uno,prod_equi1)=0,uno DIV prod_equi1,TRUNCATE(uno/prod_equi1,0))),0.00) AS DECIMAL(12,2)) AS prod_unim,
        ...
        LEFT JOIN fe_rcom AS yy ON (yy.idauto=a.prod_idau) 
        LEFT JOIN fe_prov AS o ON (o.idprov=yy.idprov), fe_gene as g
        WHERE descri LIKE cbuscar AND prod_acti<>'I' AND cin>0 ORDER BY DESCRI;
   else
        SELECT idart,descri,unid,prod_unid1,
        CAST(IF(uno>0,IF(MOD(uno,prod_equi1)=0,uno/prod_equi1,
        IF(MOD(uno,prod_equi1)=0,uno DIV prod_equi1,TRUNCATE(uno/prod_equi1,0))),0.00) AS DECIMAL(12,2)) AS prod_unim,
        ...
        LEFT JOIN fe_rcom AS yy ON (yy.idauto=a.prod_idau) 
        LEFT JOIN fe_prov AS o ON (o.idprov=yy.idprov), fe_gene AS g
        WHERE descri LIKE cbuscar AND prod_acti<>'I' AND sei>0 ORDER BY DESCRI;
end case;
END */$$
DELIMITER ;

/* Procedure structure for procedure `PROMUESTRAPRODUCTOSWEB` */

/*!50003 DROP PROCEDURE IF EXISTS  `PROMUESTRAPRODUCTOSWEB` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `PROMUESTRAPRODUCTOSWEB`(abuscar VARCHAR(80),nd DECIMAL(5,2),opt INTEGER,nid INTEGER)
BEGIN
DECLARE cbuscar VARCHAR(80);
IF opt=1 THEN
  SET cbuscar=CONCAT('%',TRIM(abuscar),+'%');
  ...
  IFNULL(ROUND(IF(tmon='S',premay,((a.prec*v.igv*nd)+b.prec)*prod_uti3),2),0) AS pre1,
  IFNULL(ROUND(IF(tmon='S',premen,((a.prec*v.igv*nd)+b.prec)*prod_uti2),2),0) AS pre2,
  IFNULL(ROUND(IF(tmon='S',pre3,((a.prec*v.igv*nd)+b.prec)*prod_uti1),2),0) AS pre3,v.igv,
  ROUND(IF(tmon='S',(a.prec*v.igv),(a.prec*v.igv*nd)),2) AS costosf,b.prec AS flete,ulfc,uno,dos,tre,cua,
  IFNULL(d.cost_cost,0) AS costor,IFNULL(d.cost_prec,0) AS precr,IFNULL(d.cost_mone,'')  AS moner,
  CAST(IFNULL(d.cost_idco,0) AS UNSIGNED) AS cost_idco,IFNULL(d.cost_flet,0)  AS fleter,IFNULL(d.cost_dola,0) AS dolar,
  peso,a.prec,tipro,idmar,a.idcat,cost,tmon,a.idflete,prod_uti1,prod_uti2,prod_uti3,prod_idus,prod_equi1,prod_equi2,
  prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,IFNULL(o.razo,'') AS proveedor,
  IFNULL(yy.ndoc,'') AS ndoc,IFNULL(yy.fech,'') AS fech,prod_cod1 AS cod2,prod_cod1,prod_acti,
  p.pres_desc,e.epta_prec
  FROM fe_art  AS a 
  INNER JOIN fe_fletes AS b ON(b.idflete=a.idflete)
  INNER JOIN fe_cat AS c ON(c.idcat=a.idcat) 
  LEFT JOIN fe_costos AS d ON(d.cost_idco=a.prod_idco)
  ...
  INNER JOIN `fe_presentaciones` p ON (e.epta_pres=p.pres_idpr),fe_gene AS v
  WHERE descri LIKE cbuscar AND prod_acti<>'I' and e.epta_acti='A' ORDER BY DESCRI;
ELSE
  ...
  INNER JOIN fe_epta e ON (a.idart=e.epta_idar)
  INNER JOIN `fe_presentaciones` p ON (e.epta_pres=p.pres_idpr),fe_gene AS v
  WHERE  CAST(idart AS CHAR)=cbuscar and e.epta_acti='A' AND prod_acti<>'I' ORDER BY DESCRI;
END IF;
END */$$
DELIMITER ;
...
set cbuscar=concat('%',trim(cbusca),+'%');
if opt=0 then
   select nomb,tipo,activo,idusua,0 as Uno,1 as uno,2 as dos,3 as tres,clave,idalma,usua_cont from fe_usua where nomb like cbuscar and activo='S' order by nomb;
end if;
if opt=1 then
   select nomb,tipo,activo,idusua,0 as Uno,1 as uno,2 as dos,3 as tres,clave,idalma,usua_cont from fe_usua where idusua=nid AND activo='S' order by nomb;
end if;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProRecibeTransferencia` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProRecibeTransferencia` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProRecibeTransferencia`(nidtra INTEGER,nmonto DECIMAL(12,2),dfecha DATE,cdeta varchar(100))
BEGIN
UPDATE fe_lcaja AS f SET lcaj_deud=f.lcaj_adebe,lcaj_acre=f.lcaj_ahaber,lcaj_adebe=0,lcaj_ahaber=0 WHERE lcaj_idtra=nidtra;
INSERT INTO fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_acre,lcaj_mone,
lcaj_idus,lcaj_form,lcaj_dcto,lcaj_tdoc,lcaj_idtra,lcaj_fope,lcaj_idct)VALUES
(dfecha,"",cdeta,nmonto,'S',5,'E',"RECIBIDO","",nidtra,LOCALTIME,0);
END */$$
DELIMITER ;

/* Procedure structure for procedure `ProRegistraHsalidaServicioCorte` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProRegistraHsalidaServicioCorte` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProRegistraHsalidaServicioCorte`(nidauto integer,dfs datetime,nidc integer)
begin
update fe_serviciocorte set serv_hsal=dfs,serv_idco=nidc where serv_idau=nidauto;
end */$$
DELIMITER ;

/* Procedure structure for procedure `ProRegistraServicioCorte` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProRegistraServicioCorte` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProRegistraServicioCorte`(nidauto integer,nidart integer,ncant decimal(10,2),
nprecio decimal(12,4),ncortador integer,ctipro char)
begin
insert into fe_serviciocorte(serv_idau,serv_idar,serv_cant,serv_prec,serv_feci,serv_idco,serv_tipro)
values(nidauto,nidart,ncant,nprecio,localtime,ncortador,ctipro);
end */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProSalidaServicioCorte` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProSalidaServicioCorte` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProSalidaServicioCorte`(nidauto integer)
begin
update fe_serviciocorte set serv_fecf=localtime where serv_idau=nidauto;
end */$$
DELIMITER ;
...
/*!50001 CREATE TABLE  `vmuestracotizaciones`(
 ...
 `razo` varchar(100) ,
 `nruc` varchar(11) ,
 `dped_posi` int(10) unsigned ,
 `equi` decimal(12,6) ,
 `dped_epta` int(10) unsigned ,
 `dire` varchar(100) ,
 `rped_mone` char(1) ,
 `ciud` varchar(100) ,
 `fono` varchar(15) ,
...
/*!50001 CREATE TABLE  `vmuestraventas`(
 ...
 `codv` int(11) ,
 `idauto` int(11) ,
 `kar_posi` int(10) unsigned ,
 `kar_equi` decimal(12,8) ,
 `kar_epta` int(10) unsigned ,
 `kar_comi` float ,
 `alma` int(11) ,
 `idcosto` int(11) ,
 `idkar` int(11) ,
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `rvendedores` AS select `fe_kar`.`idauto` AS `idauto`,`fe_kar`.`codv` AS `codv` from `fe_kar` group by `fe_kar`.`idauto`,`fe_kar`.`codv` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vmuestracompras` AS select `a`.`idauto` AS `idauto`,`a`.`alma` AS `alma`,`a`.`idkar` AS `idkar`,`a`.`kar_equi` AS `kar_equi`,`b`.`descri` AS `descri`,`b`.`peso` AS `peso`,`b`.`prod_idco` AS `prod_idco`,`b`.`unid` AS `unid`,`b`.`tipro` AS `tipro`,`a`.`idart` AS `idart`,`a`.`kar_chek` AS `kar_chek`,`a`.`incl` AS `incl`,`c`.`ndoc` AS `ndoc`,`c`.`valor` AS `valor`,`c`.`igv` AS `igv`,`c`.`impo` AS `impo`,`c`.`pimpo` AS `pimpo`,`a`.`cant` AS `cant`,`a`.`prec` AS `prec`,`c`.`fech` AS `fech`,`c`.`fecr` AS `fecr`,`c`.`form` AS `form`,`c`.`exon` AS `exon`,`c`.`ndo2` AS `ndo2`,`c`.`vigv` AS `vigv`,`c`.`idprov` AS `idprov`,`a`.`tipo` AS `tipo`,`c`.`tdoc` AS `tdoc`,`c`.`dolar` AS `dolar`,`c`.`mone` AS `mone`,`p`.`razo` AS `razo`,`p`.`dire` AS `dire`,`p`.`ciud` AS `ciud`,`p`.`nruc` AS `nruc`,ifnull(`x`.`idcaja`,0) AS `Idcaja`,`c`.`codt` AS `codt`,`a`.`dsnc` AS `dsnc`,`a`.`dsnd` AS `dsnd`,`a`.`gast` AS `gast`,`c`.`fusua` AS `fusua`,`w`.`nomb` AS `Usuario` from ((((((`fe_rcom` `c` left join `fe_kar` `a` on((`c`.`idauto` = `a`.`idauto`))) left join `fe_art` `b` on((`b`.`idart` = `a`.`idart`))) join `fe_prov` `p` on((`p`.`idprov` = `c`.`idprov`))) left join `fe_caja` `x` on((`x`.`idauto` = `c`.`idauto`))) join `fe_usua` `w` on((`w`.`idusua` = `c`.`idusua`))) join `fe_sucu` `s` on((`s`.`idalma` = `c`.`codt`))) where ((`c`.`acti` <> 'I') and (`a`.`acti` <> 'I')) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vmuestracotizaciones` AS select `a`.`idart` AS `idart`,`b`.`descri` AS `descri`,`b`.`unid` AS `unid`,`a`.`cant` AS `cant`,ifnull(`m`.`idven`,0) AS `idven`,ifnull(`m`.`nomv`,'') AS `Vendedor`,`a`.`prec` AS `prec`,`b`.`premay` AS `premay`,`b`.`premen` AS `premen`,`c`.`fech` AS `fech`,`c`.`idautop` AS `idautop`,`c`.`impo` AS `impo`,`c`.`ndoc` AS `ndoc`,`c`.`aten` AS `aten`,`c`.`forma` AS `forma`,`c`.`plazo` AS `plazo`,`c`.`validez` AS `validez`,`c`.`entrega` AS `entrega`,`c`.`detalle` AS `detalle`,ifnull(`d`.`idclie`,0) AS `idclie`,ifnull(`d`.`razo`,'') AS `razo`,ifnull(`d`.`nruc`,'') AS `nruc`,`a`.`dped_posi` AS `dped_posi`,`a`.`dped_equi` AS `equi`,`a`.`dped_epta` AS `dped_epta`,ifnull(`d`.`dire`,'') AS `dire`,`c`.`rped_mone` AS `rped_mone`,ifnull(`d`.`ciud`,'') AS `ciud`,`d`.`fono` AS `fono`,`d`.`fax` AS `fax`,`a`.`idped` AS `nreg` from ((((`fe_ped` `a` join `fe_rped` `c` on((`a`.`idautop` = `c`.`idautop`))) join `fe_art` `b` on((`b`.`idart` = `a`.`idart`))) left join `fe_clie` `d` on((`d`.`idclie` = `c`.`idclie`))) left join `fe_vend` `m` on((`m`.`idven` = `c`.`idven`))) where ((`a`.`acti` <> 'I') and (`c`.`acti` <> 'I')) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vmuestractasventas` AS select left(`p`.`nomb`,3) AS `tdoc`,`b`.`ndoc` AS `ndoc`,`b`.`fech` AS `fech`,`a`.`ncta` AS `ncta`,`c`.`razo` AS `razo`,(case `x`.`tipo` when 'D' then if((`b`.`mone` = 'S'),(`x`.`impo` + `x`.`ecta_total`),((`x`.`impo` + `x`.`ecta_total`) * `b`.`dolar`)) else 0 end) AS `Debe`,(case `x`.`tipo` when 'H' then if((`b`.`mone` = 'S'),(`x`.`impo` + `x`.`ecta_total`),((`x`.`impo` + `x`.`ecta_total`) * `b`.`dolar`)) else 0 end) AS `Haber`,`x`.`tipo` AS `tipo`,`a`.`idcta` AS `idcta`,`a`.`nomb` AS `nomb`,`b`.`idauto` AS `idrven`,`b`.`mone` AS `mone`,`x`.`idectas` AS `idectas`,`c`.`idclie` AS `idclie` from ((((`fe_ectas` `x` join `fe_plan` `a` on((`a`.`idcta` = `x`.`idcta`))) join `fe_rcom` `b` on((`b`.`idauto` = `x`.`idrven`))) join `fe_clie` `c` on((`c`.`idclie` = `b`.`idcliente`))) join `fe_tdoc` `p` on((`p`.`tdoc` = `b`.`tdoc`))) where ((`x`.`impo` <> 0) and (`b`.`acti` <> 'I') and (`p`.`dcto_acti` = 'A') and (`x`.`acti` = 'A')) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vmuestrapedidosvtas` AS select `b`.`prod_icbper` AS `prod_icbper`,`c`.`tdoc` AS `tdoc`,`c`.`form` AS `form`,`c`.`idclie` AS `idclie`,`a`.`idart` AS `idart`,`b`.`descri` AS `descri`,`x`.`pres_desc` AS `unid`,`a`.`cant` AS `cant`,`a`.`prec` AS `prec`,`b`.`premay` AS `premay`,`b`.`premen` AS `premen`,`c`.`fech` AS `fech`,`c`.`idautop` AS `idautop`,`c`.`impo` AS `impo`,`a`.`dped_epta` AS `dped_epta`,`c`.`ndoc` AS `ndoc`,`b`.`prod_idco` AS `idco`,`b`.`uno` AS `uno`,`b`.`dos` AS `dos`,`b`.`tre` AS `tre`,`b`.`cua` AS `cua`,`a`.`dped_equi` AS `dped_equi`,`n`.`nomb` AS `Usuario`,`c`.`rped_idus` AS `rped_idus`,`c`.`idpcped` AS `idpcped`,`c`.`fecho` AS `fecho`,`m`.`nomv` AS `nomv`,`c`.`idven` AS `idven`,`c`.`idtienda` AS `idtienda`,`c`.`facturado` AS `facturado`,`a`.`dped_posi` AS `dped_posi`,`c`.`aten` AS `aten`,`c`.`forma` AS `forma`,`c`.`plazo` AS `plazo`,`c`.`validez` AS `validez`,`b`.`tipro` AS `tipro`,`b`.`cost` AS `costo`,`b`.`premay` AS `pre1`,`b`.`premen` AS `pre2`,ifnull(`b`.`pre3`,0) AS `pre3`,`a`.`idped` AS `nreg`,`c`.`rped_serv` AS `rped_serv` from (((((((`fe_ped` `a` join `fe_rped` `c` on((`a`.`idautop` = `c`.`idautop`))) join `fe_art` `b` on((`b`.`idart` = `a`.`idart`))) join `fe_fletes` `p` on((`p`.`idflete` = `b`.`idflete`))) join `fe_vend` `m` on((`m`.`idven` = `c`.`idven`))) join `fe_usua` `n` on((`n`.`idusua` = `c`.`rped_idus`))) join `fe_epta` `v` on((`v`.`epta_idep` = `a`.`dped_epta`))) join `fe_presentaciones` `x` on((`x`.`pres_idpr` = `v`.`epta_pres`))) where ((`c`.`acti` = 'A') and (`a`.`acti` = 'A')) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vmuestraventas` AS select `c`.`rcom_icbper` AS `rcom_icbper`,`a`.`kar_icbper` AS `kar_icbper`,`c`.`rcom_mens` AS `rcom_mens`,`c`.`rcom_idtr` AS `rcom_idtr`,`a`.`codv` AS `codv`,`a`.`idauto` AS `idauto`,`a`.`kar_posi` AS `kar_posi`,`a`.`kar_equi` AS `kar_equi`,`a`.`kar_epta` AS `kar_epta`,`a`.`kar_comi` AS `kar_comi`,`c`.`codt` AS `alma`,`a`.`kar_idco` AS `idcosto`,`a`.`idkar` AS `idkar`,`a`.`idart` AS `Coda`,`a`.`cant` AS `cant`,`a`.`prec` AS `prec`,`c`.`valor` AS `valor`,`c`.`igv` AS `igv`,`c`.`impo` AS `impo`,`c`.`fech` AS `fech`,`c`.`fecr` AS `fecr`,`c`.`form` AS `form`,`c`.`deta` AS `deta`,`c`.`exon` AS `exon`,`c`.`ndo2` AS `ndo2`,`c`.`rcom_entr` AS `rcom_entr`,`c`.`idcliente` AS `idclie`,`d`.`razo` AS `razo`,`d`.`nruc` AS `nruc`,`d`.`dire` AS `dire`,`d`.`ciud` AS `ciud`,`d`.`ndni` AS `ndni`,`a`.`tipo` AS `tipo`,`c`.`tdoc` AS `tdoc`,`c`.`ndoc` AS `ndoc`,`c`.`dolar` AS `dolar`,`c`.`mone` AS `mone`,`b`.`descri` AS `descri`,ifnull(`x`.`idcaja`,0) AS `idcaja`,`b`.`unid` AS `unid`,`b`.`premay` AS `pre1`,`b`.`peso` AS `peso`,`b`.`premen` AS `pre2`,ifnull(`z`.`vend_idrv`,0) AS `nidrv`,`c`.`vigv` AS `vigv`,`a`.`dsnc` AS `dsnc`,`a`.`dsnd` AS `dsnd`,`a`.`gast` AS `gast`,`c`.`idcliente` AS `idcliente`,`c`.`codt` AS `codt`,`b`.`pre3` AS `pre3`,`b`.`cost` AS `costo`,`b`.`uno` AS `uno`,`b`.`dos` AS `dos`,(`b`.`uno` + `b`.`dos`) AS `TAlma`,`c`.`fusua` AS `fusua`,`p`.`nomv` AS `Vendedor`,`q`.`nomb` AS `Usuario` from (((((((`fe_art` `b` join `fe_kar` `a` on((`b`.`idart` = `a`.`idart`))) join `fe_rcom` `c` on((`a`.`idauto` = `c`.`idauto`))) left join `fe_caja` `x` on((`x`.`idauto` = `c`.`idauto`))) join `fe_clie` `d` on((`c`.`idcliente` = `d`.`idclie`))) left join `fe_vend` `p` on((`p`.`idven` = `a`.`codv`))) join `fe_usua` `q` on((`q`.`idusua` = `c`.`idusua`))) left join `fe_rvendedor` `z` on((`z`.`vend_idau` = `c`.`idauto`))) where ((`c`.`tipom` = 'V') and (`c`.`acti` <> 'I') and (`a`.`acti` <> 'I')) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vregcompras` AS select `x`.`fech` AS `fech`,`x`.`fecr` AS `fecr`,`x`.`tdoc` AS `tdoc`,`x`.`ndoc` AS `ndoc`,`x`.`idprov` AS `idprov`,`x`.`ndo2` AS `ndo2`,`x`.`mone` AS `mone`,`x`.`valor` AS `valor`,`x`.`igv` AS `igv`,`x`.`impo` AS `impo`,`x`.`codt` AS `codt`,`x`.`dolar` AS `dola`,`x`.`form` AS `form`,`x`.`idauto` AS `idauto`,`y`.`nomb` AS `usuario`,`x`.`fusua` AS `fusua`,`p`.`razo` AS `razo`,`p`.`nruc` AS `nruc`,`p`.`dire` AS `dire`,`p`.`ciud` AS `ciud`,`p`.`fono` AS `fono`,`s`.`nomb` AS `tienda` from (((`fe_rcom` `x` join `fe_usua` `y` on((`y`.`idusua` = `x`.`idusua`))) join `fe_prov` `p` on((`p`.`idprov` = `x`.`idprov`))) join `fe_sucu` `s` on((`s`.`idalma` = `x`.`codt`))) where (`x`.`acti` <> 'I') */;
