...
MySQL - 5.7.42-log : Database - eduar_bdvaltrac
*********************************************************************
*/
...
ncome float,tigv decimal(6,3),nutil1 float,nutil2 float,nutil3 float,nidusua integer,nsmin float,nsmax float,ccoda varchar(20),
ndolar float,cubi1 varchar(15)) RETURNS int(11)
BEGIN
declare nid integer default 0;
INSERT INTO fe_art(descri,unid,prec,cost,premay,premen,pre3,peso,idcat,idmar,tipro,idflete,tmon,fechc,idpc,prod_come,
prod_tigv,prod_uti1,prod_uti2,prod_uti3,prod_idus,prod_smin,prod_smax,prod_cod1,prod_dola,prod_ubi1)
VALUES (cdesc,cunid,nprec,ncosto,np1,np2,np3,npeso,ccat,cmar,ctipro,nflete,cm,localtime,
cidpc,ncome,tigv,nutil1,nutil2,nutil3,nidusua,nsmin,nsmax,ccoda,ndolar,cubi1);
...
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 decimal(12,6),vcom float) RETURNS int(11)
BEGIN
declare nidk integer default 0;
if ct='C' then
...
if opt=0 then
    ...
    IF ni>1 THEN
       ...
       UPDATE fe_rcom SET tdoc=ctdoc,form=cform,ndoc=cndoc,fech=dfecha,fecr=dfechar,deta=cdetalle,rcom_exon=nv,igv=nigv,impo=nt,ndo2=cndo2,
       mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt WHERE idauto=nidauto;
   end if;
   call ProDesactivaLcajaE(nidauto);
   if n1>0 and n2>0 and n3>0 then
...
IF @ufc<=dfe then
   ...
   if cmda<>'S' then
       SET ncostomn=npr*ndolar;
    else
      set ncostomn=npr;
   end if; 
   UPDATE fe_art SET prec=ncostomn,cost=npr*nigv,prod_idau=cnd,ulpc=idp,tmon='S',ulfc=dfe,
...
ncomc float,nidus integer,ncoda integer,nsmin float,nsmax float,tigv decimal(6,3),ndolar float,cestado char,cubi1 varchar(15))
BEGIN
...
prod_smin=nsmin,prod_tigv=tigv,prod_dola=ndolar,prod_acti=cestado,prod_ubi1=cubi1  WHERE idart=ncoda;
END */$$
DELIMITER ;
...
WHEN opt=0 THEN
    ...
    IFNULL(ROUND(IF(tmon='S',((a.prec*prod_tigv)+b.prec)*prod_uti1,((a.prec*prod_tigv*nd)+b.prec)*prod_uti1),0.5),0) AS pre1,
    IFNULL(ROUND(IF(tmon='S',((a.prec*prod_tigv)+b.prec)*prod_uti2,((a.prec*prod_tigv*nd)+b.prec)*prod_uti2),0.5),0) AS pre2,
    IFNULL(ROUND(IF(tmon='S',((a.prec*prod_tigv)+b.prec)*prod_uti3,((a.prec*prod_tigv*nd)+b.prec)*prod_uti3),0.5),0) AS pre3,
    ...
    u.nomb AS usuacreo,us.nomb AS usuamodifico,prod_ubi1
    FROM fe_art  AS a
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vmuestraventas` AS select `c`.`rcom_exon` AS `rcom_exon`,`c`.`idusua` AS `idusua`,`a`.`kar_comi` AS `kar_comi`,`a`.`codv` AS `codv`,`a`.`idauto` AS `idauto`,`c`.`codt` AS `alma`,`a`.`kar_idco` AS `idcosto`,`a`.`idkar` AS `idkar`,`a`.`idart` AS `Coda`,`a`.`cant` AS `cant`,`a`.`prec` AS `prec`,`c`.`valor` AS `valor`,`c`.`igv` AS `igv`,`c`.`impo` AS `impo`,`c`.`fech` AS `fech`,`c`.`fecr` AS `fecr`,`c`.`form` AS `form`,`c`.`deta` AS `deta`,`c`.`exon` AS `exon`,`c`.`ndo2` AS `ndo2`,`c`.`rcom_entr` AS `rcom_entr`,`c`.`idcliente` AS `idclie`,`d`.`razo` AS `razo`,`d`.`nruc` AS `nruc`,`d`.`dire` AS `dire`,`d`.`ciud` AS `ciud`,`d`.`ndni` AS `ndni`,`a`.`tipo` AS `tipo`,`c`.`tdoc` AS `tdoc`,`c`.`ndoc` AS `ndoc`,`c`.`dolar` AS `dolar`,`c`.`mone` AS `mone`,`b`.`descri` AS `descri`,cast(0 as decimal(10,0)) AS `idcaja`,`b`.`unid` AS `unid`,`b`.`premay` AS `pre1`,`b`.`peso` AS `peso`,`b`.`premen` AS `pre2`,ifnull(`z`.`vend_idrv`,0) AS `nidrv`,`c`.`vigv` AS `vigv`,`a`.`dsnc` AS `dsnc`,`a`.`dsnd` AS `dsnd`,`a`.`gast` AS `gast`,`c`.`idcliente` AS `idcliente`,`c`.`codt` AS `codt`,`b`.`pre3` AS `pre3`,`b`.`cost` AS `costo`,`b`.`uno` AS `uno`,`b`.`dos` AS `dos`,(`b`.`uno` + `b`.`dos`) AS `TAlma`,`c`.`fusua` AS `fusua`,`p`.`nomv` AS `Vendedor`,`q`.`nomb` AS `Usuario` from ((((((`eduar_bdvaltrac`.`fe_art` `b` join `eduar_bdvaltrac`.`fe_kar` `a` on((`a`.`idart` = `b`.`idart`))) join `eduar_bdvaltrac`.`fe_rcom` `c` on((`a`.`idauto` = `c`.`idauto`))) join `eduar_bdvaltrac`.`fe_clie` `d` on((`c`.`idcliente` = `d`.`idclie`))) join `eduar_bdvaltrac`.`fe_vend` `p` on((`p`.`idven` = `a`.`codv`))) join `eduar_bdvaltrac`.`fe_usua` `q` on((`q`.`idusua` = `c`.`idusua`))) left join (select `eduar_bdvaltrac`.`fe_rvendedor`.`vend_idau` AS `vend_idau`,`eduar_bdvaltrac`.`fe_rvendedor`.`vend_idrv` AS `vend_idrv` from `eduar_bdvaltrac`.`fe_rvendedor` where (`eduar_bdvaltrac`.`fe_rvendedor`.`vend_acti` = 'A')) `z` on((`z`.`vend_idau` = `c`.`idauto`))) where ((`c`.`idcliente` > 0) and (`c`.`acti` <> 'I') and (`a`.`acti` <> 'I')) */;
