...
MySQL - 8.0.37 : Database - syscom_bdferrpark
*********************************************************************
*/
...
nct float,cincl char,tmvto char,ccodv integer,calma integer,ncosto decimal(12,5),vcom float) RETURNS int
BEGIN
declare nidk integer default 0;
if ct='C' then
  ...
  VALUES (nid,cc,ct,npr,nct,tmvto,cincl,calma,ncosto,ccodv,0,vcom);
end if;
select last_insert_id() into nidk from fe_kar group by last_insert_id();
return nidk;
END */$$
...
/*!50003 CREATE PROCEDURE `CierraCaja`(in dfecha date,nidus integer)
BEGIN
...
BEGIN
SELECT * FROM fe_sucu  where idalma in(1) order by idalma;
END */$$
DELIMITER ;
