...
nimpo decimal(12,2),ni float,idven integer,nimpoo float,cusua integer,nidaval integer,ndscto float,
...
cform CHAR,cdcto VARCHAR(12),ctdoc VARCHAR(2),ncodt INTEGER,nturno INTEGER,ndscto DECIMAL(12,2),nidingreso INTEGER) RETURNS int
BEGIN
DECLARE nid INTEGER DEFAULT 0;
INSERT INTO fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
lcaj_idus,lcaj_clpr,lcaj_idau,lcaj_form,lcaj_dcto,lcaj_tdoc,lcaj_codt,lcaj_idtu,lcaj_dsct,lcaj_idle,lcaj_fope)VALUES
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidcp,nidauto,cform,cdcto,ctdoc,ncodt,nturno,ndscto,nidingreso,localtime);
...
CASE
	...
	FROM venta  AS v
        INNER JOIN fe_art AS a ON a.idart=v.`idgrade`
        WHERE estado=1 AND fecreg_inicio >=df AND pump in(1,2)  AND volume>0.05 ORDER BY fecreg_inicio DESC,nozzle ASC,pump ASC;
     WHEN nIsla = 2 THEN
	    SELECT gradename AS descri,amount AS monto,price AS precio,volume AS cantidad,
	    nozzle AS manguera,pump AS lado,estado,TotalVolume AS totalcantidad,
	    totalamount AS totalmonto,idjournal AS nId_Journal,fecreg_inicio,a.idart,a.unid
	    FROM venta AS v
            INNER JOIN fe_art AS a ON a.`idart`=v.`idgrade`
            WHERE estado=1 AND fecreg_inicio >=df AND pump IN(3,4)  AND volume>0.05  ORDER BY fecreg_inicio DESC,nozzle ASC,pump ASC;
      WHEN nIsla = 3 THEN
	    SELECT gradename AS descri,amount AS monto,price AS precio,volume AS cantidad,
	     nozzle AS manguera,pump AS lado,estado,TotalVolume AS totalcantidad,
	     totalamount AS totalmonto,idjournal AS nId_Journal,fecreg_inicio,a.idart,a.unid
	    FROM venta AS v
            INNER JOIN fe_art AS a ON a.`idart`=v.`idgrade`
            WHERE estado=1 AND fecreg_inicio >=df AND pump in(5,6) ORDER BY fecreg_inicio DESC,nozzle ASC,pump ASC;
       WHEN nIsla = 4 THEN
	    SELECT gradename AS descri,amount AS monto,price AS precio,volume AS cantidad,
	     nozzle AS manguera,pump AS lado,estado,TotalVolume AS totalcantidad,
	     totalamount AS totalmonto,idjournal AS nId_Journal,fecreg_inicio,a.idart,a.unid
	    FROM venta AS v
            INNER JOIN fe_art AS a ON a.`idart`=v.`idgrade`
            WHERE estado=1 AND fecreg_inicio >=df AND pump in(7,8) ORDER BY fecreg_inicio DESC,nozzle ASC,pump ASC;     
     END CASE;
END */$$
DELIMITER ;
...
CASE
	...
	WHEN nisla = 3 THEN
	    ...
	    FROM venta  AS v
            ...
            INNER JOIN fe_clie ON fe_clie.idclie=fe_rcom.idcliente
            WHERE fe_kar.acti='A' AND fe_rcom.acti='A' AND kar_idco>0 AND fe_rcom.fech BETWEEN dfi  AND dff GROUP BY idauto,kar_idco,ndoc,razo) AS k ON k.kar_idco=v.idjournal
            WHERE CAST(fecreg_inicio AS DATE)  BETWEEN dfi  AND dff  AND pump in(5,6) ORDER BY fecreg_inicio DESC,nozzle ASC,pump ASC;
       WHEN nisla = 4 THEN
	    SELECT gradename AS producto,amount AS monto,price AS precio,volume AS cantidad,
	    nozzle AS manguera,pump AS lado,estado,TotalVolume AS totalcantidad,
	    totalamount AS totalmonto,fecreg_inicio,dcto,cliente,impo AS importe,idjournal AS nId_Journal
	    FROM venta  AS v
            ...
            INNER JOIN fe_clie ON fe_clie.idclie=fe_rcom.idcliente
            WHERE fe_kar.acti='A' AND fe_rcom.acti='A' AND kar_idco>0 AND fe_rcom.fech BETWEEN dfi  AND dff GROUP BY idauto,kar_idco,ndoc,razo) AS k ON k.kar_idco=v.idjournal
            WHERE CAST(fecreg_inicio AS DATE)  BETWEEN dfi  AND dff  AND pump in(7,8) ORDER BY fecreg_inicio DESC,nozzle ASC,pump ASC;     
END CASE;
END */$$
DELIMITER ;
...
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,9 As codigo  FROM venta WHERE pump=1 AND idgrade=9 AND nozzle=1 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,9 as codigo FROM venta WHERE pump=2 AND idgrade=9  AND nozzle=1 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price AS precio,9 as codigo  FROM venta WHERE pump=3 AND idgrade=9 AND nozzle=1 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,9 as codigo FROM venta WHERE pump=4 AND idgrade=9  AND nozzle=1 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
...
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,4 as codigo FROM venta WHERE pump=6 AND idgrade=4  AND nozzle=2 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
...
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,1 as codigo FROM venta WHERE pump=8 AND idgrade=1  AND nozzle=3 ORDER BY fecreg_inicio DESC LIMIT 1)
union
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price AS precio,4 AS codigo FROM venta WHERE pump=7 AND idgrade=4  AND nozzle=2 ORDER BY fecreg_inicio DESC LIMIT 1)
union
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,4 as codigo  FROM venta WHERE pump=8 AND idgrade=4 AND nozzle=2 ORDER BY fecreg_inicio DESC LIMIT 1)
union
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price AS precio,5 AS codigo FROM venta WHERE pump=7 AND idgrade=5  AND nozzle=1 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price AS precio,5 AS codigo FROM venta WHERE pump=8 AND idgrade=5  AND nozzle=1 ORDER BY fecreg_inicio DESC LIMIT 1);
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProListarlecturasxisla` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProListarlecturasxisla` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProListarlecturasxisla`()
BEGIN
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,5 As codigo  FROM venta WHERE pump=1 AND idgrade=3 AND nozzle=1 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,5 as codigo FROM venta WHERE pump=2 AND idgrade=3  AND nozzle=4 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price AS precio,4 as codigo  FROM venta WHERE pump=1 AND idgrade=2 AND nozzle=2 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,4 as codigo FROM venta WHERE pump=2 AND idgrade=2  AND nozzle=3 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,2 as codigo  FROM venta WHERE pump=1 AND idgrade=1 AND nozzle=3 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,2 as codigo FROM venta WHERE pump=2 AND idgrade=2  AND nozzle=1 ORDER BY fecreg_inicio DESC LIMIT 1)
union
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,1 as codigo FROM venta WHERE pump=1 AND idgrade=4 AND nozzle=4 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
...
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,5 as codigo  FROM venta WHERE pump=3 AND idgrade=3 AND nozzle=1 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,5 as codigo FROM venta WHERE pump=4 AND idgrade=3  AND nozzle=4 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,4 as codigo  FROM venta WHERE pump=3 AND idgrade=2 AND nozzle=2 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,4 as codigo FROM venta WHERE pump=4 AND idgrade=2  AND nozzle=3 ORDER BY fecreg_inicio DESC LIMIT 1)
union
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price AS precio,2 AS codigo FROM venta WHERE pump=3 AND idgrade=1  AND nozzle=3 ORDER BY fecreg_inicio DESC LIMIT 1)
union
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,2 as codigo  FROM venta WHERE pump=4 AND idgrade=1 AND nozzle=2 ORDER BY fecreg_inicio DESC LIMIT 1)
union
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
...
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price AS precio,1 AS codigo FROM venta WHERE pump=4 AND idgrade=4  AND nozzle=1 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,9 As codigo  FROM venta WHERE pump=5 AND idgrade=5 AND nozzle=1 ORDER BY fecreg_inicio DESC LIMIT 1)
union
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price AS precio,9 AS codigo  FROM venta WHERE pump=6 AND idgrade=5 AND nozzle=1 ORDER BY fecreg_inicio DESC LIMIT 1);
END */$$
DELIMITER ;
...
WHEN opt='X' THEN
   ...
   INNER JOIN fe_cat AS c ON(c.idcat=a.idcat)
   INNER JOIN fe_mar AS s ON s.idmar=a.idmar
   INNER JOIN (SELECT cont_idar AS cont_idart,cont_idcon,cont_lado FROM fe_contometro WHERE cont_isla=nid GROUP BY cont_idar) AS X
   ON x.cont_idart=a.idart LEFT JOIN fe_costos AS d ON(d.cost_idco=a.prod_idco)
   LEFT JOIN fe_rcom AS yy ON (yy.idauto=a.prod_idau) LEFT JOIN fe_prov AS o ON (o.idprov=yy.idprov),fe_gene AS g
   WHERE prod_acti='A'  ORDER BY a.prod_orde,a.DESCRI;
END CASE;
END */$$
DELIMITER ;
...
CASE
WHEN nisla=1 THEN
     SELECT  CONCAT(CAST(lect_idtu AS CHAR),'-',CAST(lect_idin AS CHAR)) AS lectura,lect_idin AS idin FROM fe_lecturas WHERE lect_acti='A' AND lect_esta='A' AND lect_idco IN(1,2) GROUP BY lect_idin,lect_idtu;
when nisla=2 then
     SELECT  CONCAT(CAST(lect_idtu AS CHAR),'-',CAST(lect_idin AS CHAR)) AS lectura,lect_idin AS idin FROM fe_lecturas WHERE lect_acti='A' AND lect_esta='A' AND lect_idco IN(3,4) GROUP BY lect_idin,lect_idtu;
when nisla=3 then
    SELECT  CONCAT(CAST(lect_idtu AS CHAR),'-',CAST(lect_idin AS CHAR)) AS lectura,lect_idin AS idin FROM fe_lecturas WHERE lect_acti='A' AND lect_esta='A' AND lect_idco IN(5,6) GROUP BY lect_idin,lect_idtu;
else
    SELECT  CONCAT(CAST(lect_idtu AS CHAR),'-',CAST(lect_idin AS CHAR)) AS lectura,lect_idin AS idin FROM fe_lecturas WHERE lect_acti='A' AND lect_esta='A' AND lect_idco IN(7,8) GROUP BY lect_idin,lect_idtu;
END CASE;
END */$$
DELIMITER ;
