...
MySQL - 8.0.39 : Database - syscom_bddoslperu
*********************************************************************
*/
...
INSERT INTO syscom_bdverde2l.fe_clie(nruc,razo,dire,ciud,fono,fax,ndni,clie_tipo,clie_corr,clie_codv,clie_idus,idpcclie,
fechclie,celu,refe,clie_lcre,clie_rpm,clie_idzo,clie_dias,clie_conta,clie_dire1,clie_idse)
VALUES (cruc,crazo,cdire,cciud,cfono,cfax,cdni,ctipo,cemail,nidven,nidus,cpc,localtime,ccelu,crefe,linea,crpm,nidz,ndias,cconta,cdire1,nids);
return nid;
END */$$
DELIMITER ;
...
ncome float,ncomc float,nutil1 float,nutil2 float,nutil3 float,nidusua integer,nsmin float,nsmax float,nidcosto integer,ndolar float,nutil0 DECIMAL(8,5)) RETURNS int
BEGIN
declare nid integer default 0;
declare nidl1 integer default 0;
INSERT INTO syscom_bdverde2l.fe_art(descri,unid,prec,cost,premay,premen,pre3,peso,idcat,idmar,tipro,idflete,tmon,fechc,idpc,prod_come,
...
select last_insert_id() into nidl1 from syscom_bdverde2l.fe_art group by last_insert_id();
INSERT INTO fe_art(descri,unid,prec,cost,premay,premen,pre3,peso,idcat,idmar,tipro,idflete,tmon,fechc,idpc,prod_come,
prod_comc,prod_uti1,prod_uti2,prod_uti3,prod_idus,prod_smin,prod_smax,prod_idco,prod_dola,prod_enla)
VALUES (cdesc,cunid,nprec,ncosto,np1,np2,np3,npeso,ccat,cmar,ctipro,nflete,cm,localtime,
cidpc,ncome,ncomc,nutil1,nutil2,nutil3,nidusua,nsmin,nsmax,nidcosto,ndolar,nidl1);
...
INSERT INTO syscom_bdverde2l.fe_prov(nruc,razo,dire,ciud,fono,fax,prov_rpm,email,refe,celu,prov_idus,idpcprov,fechprov)
VALUES (cruc,crazo,cdire,cciud,cfono,cfax,crpm,correo,crefe,ccelu,nidus,cpc,localtime);
return nid;
END */$$
DELIMITER ;
...
where b.acti<>'I' group by  idart,alma) as a where (a.tcompras-a.tventas)<>0 order by a.idart;
...
crefe varchar(255),linea decimal(12,2),crpm varchar(10),nidz integer,ndias integer,cconta varchar(150),cdire1 varchar(150),nidseg integer)
BEGIN
update fe_clie set
...
clie_dias=ndias,clie_conta=cconta,clie_dire1=cdire1,clie_idse=nidseg where idclie=nid;
END */$$
DELIMITER ;
...
IF @ufc<=dfe then
   ...
   UPDATE syscom_bdverde2l.fe_art SET prec=npr,cost=npr*vigv,prod_idau=cnd,ulpc=idp,tmon=cmda,ulfc=dfe,
       premay=round(costor*@nutil1,2),premen=round(costor*@nutil2,2),pre3=if(@nutil3<>0,round(costor*@nutil3,2),0),
...
ncomc FLOAT,nidus INTEGER,ncoda INTEGER,nsmin FLOAT,nsmax FLOAT,nidcosto INTEGER,ndolar FLOAT,cestado CHAR,nutil0 DECIMAL(8,4))
BEGIN
...
prod_smin=nsmin,prod_idco=nidcosto,prod_dola=ndolar,prod_acti=cestado,prod_uti0=nutil0  WHERE idart=ncoda;
END */$$
DELIMITER ;
...
when opt=0 then
   ...
   clie_dias,clie_conta,clie_dire1,clie_idvi,clie_auto,clie_rete,u.nomb AS usuacreo,
   uu.nomb AS usuamodifico,clie_idse
   from fe_clie as a 
...
when opt=2 then
   ...
   LEFT JOIN fe_usua AS u  ON u.idusua=a.clie_idus
   LEFT JOIN fe_usua AS uu ON uu.idusua=a.clie_actu    
   where  clie_acti<>'I' and  ndni like cbuscar order by razo;
when opt=3 then
   select idclie,nruc,razo,ndni,dire,ciud,ifnull(fono,'0') as fono,ifnull(fax,'0') as fax,
   ifnull(celu,'0') as celu,refe,clie_tipo,clie_codv,clie_lcre,clie_corr,
   clie_rpm,clie_idus,clie_actu,fechclie,clie_feac,ifnull(c.zona_nomb,'') as Zona,ifnull(a.clie_idzo,0) as clie_idzo,
...
/* Procedure structure for procedure `ProMuestraProductos1old` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProMuestraProductos1old` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProMuestraProductos1old`(abuscar VARCHAR(80), nd FLOAT,opt INTEGER,nid INTEGER)
BEGIN
DECLARE cbuscar VARCHAR(80);
case
when opt=1 THEN
...
/*!50003 CREATE PROCEDURE `ProMuestraProductosprueba`(abuscar VARCHAR(80), nd FLOAT,opt INTEGER,nid INTEGER)
BEGIN
DECLARE cbuscar VARCHAR(80);
case
when opt=1 THEN
   SET cbuscar=CONCAT('%',TRIM(abuscar),+'%');
   SELECT idart,descri,unid,uno,dos,tre,cua,cin,sei,
   IFNULL(ROUND(IF(tmon='S',((a.prec*v.igv))*prod_uti1,((a.prec*v.igv*IF(prod_dola>nd,prod_dola,nd)))*prod_uti1)+b.prec,2),0) AS pre1,
   IFNULL(ROUND(IF(tmon='S',((a.prec*v.igv))*prod_uti2,((a.prec*v.igv*IF(prod_dola>nd,prod_dola,nd)))*prod_uti2)+b.prec,2),0) AS pre2,
   IFNULL(ROUND(IF(tmon='S',((a.prec*v.igv))*prod_uti3,((a.prec*v.igv*IF(prod_dola>nd,prod_dola,nd)))*prod_uti3)+b.prec,2),0) AS pre3,
   IFNULL(ROUND(IF(tmon='S',((a.prec*v.igv))*prod_uti0,((a.prec*v.igv*nd))*prod_uti0)+IF(prod_uti0>0,b.prec,0),2),0) AS pre0,   
   ROUND(IF(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*nd)+b.prec),2) AS costo,c.idgrupo,c.dcat,prod_dola,
   ROUND(IF(tmon='S',(a.prec*v.igv),(a.prec*v.igv*nd)),2) AS costosf,b.prec AS flete,
   IFNULL(d.cost_cost,0) AS costor,IFNULL(d.cost_prec,0) AS precr,IFNULL(d.cost_mone,'')  AS moner,
   CAST(IFNULL(d.cost_idco,0) AS UNSIGNED) AS cost_idco,IFNULL(d.cost_flet,0)  AS fleter,IFNULL(d.cost_dola,0) AS dolar,
