...
MySQL - 8.0.39 : Database - syscom_bdhuancar
*********************************************************************
*/
...
if opt=0 then
   ...
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idprov,tipom,fusua,idusua,codt,rcom_nitem,rcom_tipo,pimpo)
   VALUES (ctdoc,cform,cndoc,dfecha,dfechar,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,localtime,nus,nidcodt,nitem,ctipo,npvta);
  ...
  /* INSERT INTO fe_correvtas(auto_corr)VALUES(CONCAT(ctdoc,cndoc));*/
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,rcom_nitem,rcom_tipo)
   VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,localtime,nus,nidcodt,nitem,ctipo);
   select last_insert_id() into nid from fe_rcom group by last_insert_id();
end if;
return nid;
END */$$
...
WHERE b.acti<>'I' GROUP BY  idart,alma) AS a INNER JOIN fe_art AS z ON z.idart=a.idart;
...
if opt=0 then
   ...
   mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idprov=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,rcom_nitem=nitem,pimpo=npvta where idauto=nidauto;
  else
   update fe_rcom set tdoc=ctdoc,form=cform,ndoc=cndoc,fech=dfecha,fecr=dfechar,deta=cdetalle,valor=nv,igv=nigv,impo=nt,ndo2=cndo2,
   mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,rcom_nitem=nitem where idauto=nidauto;
end if;
END */$$
DELIMITER ;
...
set cbuscar=concat('%',trim(abuscar),+'%');
	...
	p.pres_desc,e.epta_prec,IF(e.epta_mone='S',ROUND(e.epta_cost/((100-g.pmvtas)/100),2),ROUND((e.epta_cost*g.dola)/((100-g.pmvtas)/100),2)) AS precio1,
        IF(e.epta_mone='S',e.epta_cost,ROUND(e.epta_cost*g.dola,2)) AS costo,e.epta_cant,e.epta_idar,e.epta_idep,e.epta_esti
	FROM fe_art  as a 
...
if opt=0 then
	...
	peso,a.prec,tipro,idmar,a.idcat,cost,tmon,a.idflete,prod_uti1,prod_uti2,prod_uti3,prod_idus,prod_equi1,prod_equi2,
	prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,'' as proveedor,
	'' as ndoc,'' as fech,a.descri as clave,prod_acti as estado,'' as usuacreo,'' as usuamodifico
	FROM fe_art  as a 
	inner join fe_fletes as b on(b.idflete=a.idflete)
	inner join fe_cat as c on(c.idcat=a.idcat),fe_gene AS g 
	WHERE descri LIKE cbuscar and prod_acti<>'I' ORDER BY DESCRI;
else
	SELECT a.idart,left(concat(trim(c.dcat),' ',substr(a.descri,instr(a.descri,',')+1),' ',substr(a.descri,1,instr(a.descri,',')-1)),150) as descri,unid,prod_unid1,
	cast(IF(uno>0,IF(MOD(uno,prod_equi1)=0,uno/prod_equi1,
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vguiasventas` AS select `b`.`guia_idgui` AS `idguia`,`a`.`idart` AS `coda`,`a`.`descri` AS `descri`,`a`.`unid` AS `unid`,`b`.`guia_ndoc` AS `ndoc`,`b`.`guia_fech` AS `fech`,`b`.`guia_fect` AS `fect`,`b`.`guia_ptoll` AS `ptoll`,`b`.`guia_deta` AS `detalle`,`x`.`entr_cant` AS `cant`,`y`.`placa` AS `placa`,`y`.`razon` AS `Transportista`,`y`.`ructr` AS `ructr`,`y`.`nombr` AS `Chofer`,`y`.`breve` AS `Brevete`,`y`.`cons` AS `Constancia`,`y`.`marca` AS `marca`,`y`.`dirtr` AS `Direccion`,`p`.`nomb` AS `usuario`,`d`.`razo` AS `cliente`,`d`.`idclie` AS `idcliente`,`c`.`ndoc` AS `refe`,`c`.`tdoc` AS `tdoc` from (((((((`fe_guias` `b` join `fe_ent` `x` on((`x`.`entr_idgu` = `b`.`guia_idgui`))) join `fe_usua` `p` on((`p`.`idusua` = `b`.`guia_idus`))) join `fe_rcom` `c` on((`c`.`idauto` = `b`.`guia_idau`))) join `fe_clie` `d` on((`d`.`idclie` = `c`.`idcliente`))) join `fe_kar` `s` on((`s`.`idauto` = `c`.`idauto`))) join `fe_art` `a` on((`a`.`idart` = `s`.`idart`))) left join `fe_tra` `y` on((`y`.`idtra` = `b`.`guia_idtr`))) where (`b`.`guia_acti` = 'A') */;
