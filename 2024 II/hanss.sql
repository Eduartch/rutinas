...
MySQL - 8.0.39 : Database - syscom_bdhanss
*********************************************************************
*/
...
declare idve,nidctaper integer;
    update fe_rcom set tdoc=ctdoc,form=cform,ndoc=cndoc,fech=dfecha,fecr=dfechar,deta=cdetalle,valor=nv,igv=nigv,impo=nt,ndo2=cndo2,
   mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,rcom_nitem=nitems,
...
if(b.epta_mone='S',if(b.epta_esti='M',round((b.epta_cost+f.prec)*((b.epta_marg/100)+1),2),round((b.epta_cost+f.prec)*((b.epta_marg/100)+1),2)),
if(b.epta_esti='M',round(((b.epta_cost*nd)+f.prec)*((b.epta_marg/100)+1),2),round(((b.epta_cost*nd)+f.prec)*((b.epta_marg/100)+1),2))),b.epta_prec) as epta_prec,
if(b.epta_mone='S',round(b.epta_cost/((100-v.igv)/100),2),round((b.epta_cost*nd)/((100-v.igv)/100),2)) as precio1,
if(b.epta_mone='S',b.epta_cost+f.prec,round(b.epta_cost*nd,2)+f.prec) as costo,
b.epta_cant,b.epta_idar,b.epta_idep,b.epta_esti,epta_comi FROM fe_epta  as b 
inner join fe_presentaciones as a on a.pres_idpr=b.epta_pres 
inner join fe_art as p on p.idart=b.epta_idar 
inner join fe_fletes as f on f.idflete=p.idflete ,fe_gene as v
...
FROM fe_art  as a inner join fe_fletes as b on(b.idflete=a.idflete)
inner join fe_cat as c on(c.idcat=a.idcat) left join fe_costos as d on(d.cost_idco=a.prod_idco)
left join fe_rcom as yy on (yy.idauto=a.prod_idau) left join fe_prov as o on (o.idprov=yy.idprov)
...
IF opt=1 THEN
  ...
  aa.pres_desc,IF(bb.epta_cost>0,
  IF(bb.epta_mone='S',IF(bb.epta_esti='M',ROUND((bb.epta_cost+b.prec)*((bb.epta_marg/100)+1),2),ROUND((bb.epta_cost+b.prec)*((bb.epta_marg/100)+1),2)),
  IF(bb.epta_esti='M',ROUND(((bb.epta_cost*nd)+b.prec)*((bb.epta_marg/100)+1),2),ROUND(((bb.epta_cost*nd)+b.prec)*((bb.epta_marg/100)+1),2))),bb.epta_prec) AS epta_prec,
  IF(bb.epta_mone='S',ROUND(bb.epta_cost/((100-v.igv)/100),2),ROUND((bb.epta_cost*nd)/((100-v.igv)/100),2)) AS precio1,
  IF(bb.epta_mone='S',bb.epta_cost+b.prec,ROUND(bb.epta_cost*nd,2)+b.prec) AS costo,
  bb.epta_cant,bb.epta_idep,bb.epta_esti,epta_comi
  FROM fe_art  AS a 
  INNER JOIN fe_fletes AS b ON(b.idflete=a.idflete)
  INNER JOIN fe_cat AS c ON(c.idcat=a.idcat) 
  inner join fe_epta  AS bb on bb.epta_idar=a.idart
  INNER JOIN fe_presentaciones AS aa ON aa.pres_idpr=bb.epta_pres 
  LEFT JOIN fe_costos AS d ON(d.cost_idco=a.prod_idco)
  LEFT JOIN fe_rcom AS yy ON (yy.idauto=a.prod_idau) 
