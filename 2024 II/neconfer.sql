...
MySQL - 8.0.39 : Database - syscom_bdneconfer
*********************************************************************
*/
...
cplaca1 varchar(10),cconstancia1 varchar(20),cbrevete varchar(10)) RETURNS int
BEGIN
DECLARE id INTEGER;
...
guia_ubi2,guia_idv1,guia_idv2,guia_idre,guia_idde,guia_tdocr,guia_dctor,guia_cond,guia_brev,guia_pla2,guia_con1)
...
cconductor,cbrevete,cplaca1,cconstancia1);
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vmuestravtas` AS select `c`.`idusua` AS `idusua`,`a`.`kar_comi` AS `kar_comi`,`a`.`codv` AS `codv`,`c`.`idauto` AS `idauto`,`c`.`codt` AS `alma`,`a`.`kar_idco` AS `idcosto`,`a`.`idkar` AS `idkar`,`a`.`idart` AS `Coda`,`a`.`cant` AS `cant`,`a`.`prec` AS `prec`,`c`.`valor` AS `valor`,`c`.`igv` AS `igv`,`c`.`impo` AS `impo`,`c`.`fech` AS `fech`,`c`.`fecr` AS `fecr`,`c`.`form` AS `form`,`c`.`deta` AS `deta`,`c`.`exon` AS `exon`,`c`.`ndo2` AS `ndo2`,`c`.`idcliente` AS `idclie`,`d`.`razo` AS `razo`,`d`.`nruc` AS `nruc`,`d`.`dire` AS `dire`,`d`.`ciud` AS `ciud`,`d`.`ndni` AS `ndni`,`a`.`tipo` AS `tipo`,`c`.`tdoc` AS `tdoc`,`c`.`ndoc` AS `ndoc`,`c`.`dolar` AS `dolar`,`c`.`mone` AS `mone`,`c`.`vigv` AS `vigv`,`a`.`dsnc` AS `dsnc`,`a`.`dsnd` AS `dsnd`,`a`.`gast` AS `gast`,`c`.`idcliente` AS `idcliente`,`c`.`codt` AS `codt`,`c`.`fusua` AS `fusua`,ifnull(`b`.`descri`,`c`.`deta`) AS `descri`,ifnull(`b`.`unid`,'') AS `unid`,`g`.`nomb` AS `usuario` from ((((`fe_rcom` `c` join `fe_clie` `d` on((`d`.`idclie` = `c`.`idcliente`))) join `fe_usua` `g` on((`g`.`idusua` = `c`.`idusua`))) left join `fe_kar` `a` on((`a`.`idauto` = `c`.`idauto`))) left join `fe_art` `b` on((`b`.`idart` = `a`.`idart`))) where ((`c`.`acti` <> 'I') and ((`a`.`acti` is null) or (`a`.`acti` = 'A'))) */;
