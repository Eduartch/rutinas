...
MySQL - 8.0.39 : Database - syscom_bdgrmotupe
*********************************************************************
*/
...
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,2 as codigo FROM venta WHERE pump=2 AND idgrade=1  AND nozzle=2 ORDER BY fecreg_inicio DESC LIMIT 1)
union
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
...
union
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price AS precio,1 AS codigo FROM venta WHERE pump=3 AND idgrade=4  AND nozzle=4 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
...
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price as precio,9 As codigo  FROM venta WHERE pump=5 AND idgrade=5 AND nozzle=1 AND totalvolume>0 ORDER BY fecreg_inicio DESC LIMIT 1)
union
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price AS precio,9 AS codigo  FROM venta WHERE pump=6 AND idgrade=5 AND nozzle=1  AND totalvolume>0 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price AS precio,9 AS codigo  FROM venta WHERE pump=7 AND idgrade=5 AND nozzle=1 AND totalvolume>0 ORDER BY fecreg_inicio DESC LIMIT 1)
UNION
(SELECT idgrade,gradename AS producto,pump AS surtidor,nozzle AS lado,totalvolume AS lectura,totalamount AS monto,
fecreg_inicio AS inicio,fecreg_fin AS fin,price AS precio,9 AS codigo  FROM venta WHERE pump=8 AND idgrade=5 AND nozzle=1 and totalvolume>0 ORDER BY fecreg_inicio DESC LIMIT 1);
END */$$
DELIMITER ;
...
WHEN opt='O' THEN
   ...
   INNER JOIN fe_mar AS s ON s.idmar=a.idmar 
   LEFT JOIN fe_costos AS d ON(d.cost_idco=a.prod_idco)
   LEFT JOIN fe_rcom AS yy ON (y.idauto=a.prod_idau) 
   LEFT JOIN fe_prov AS o ON (o.idprov=yy.idprov),fe_gene as v
   WHERE descri LIKE cbuscar AND prod_acti<>'I' AND tipro<>'C' ORDER BY a.prod_orde,a.DESCRI;
WHEN opt='T' THEN
   SET cbuscar=CONCAT('%',TRIM(abuscar),+'%');
...
set cbuscar=concat('%',trim(cbusca),+'%');
if opt=0 then
   select nomb,tipo,activo,idusua,0 as Uno,1 as uno,2 as dos,3 as tres,clave,idalma from fe_usua where nomb like cbuscar  AND activo='S' order by nomb;
end if;
if opt=1 then
   select nomb,tipo,activo,idusua,0 as Uno,1 as uno,2 as dos,3 as tres,clave,idalma from fe_usua where idusua=nid  AND activo='S' order by nomb;
end if;
if opt=2 then
   select nomb,tipo,activo,idusua,0 as Uno,1 as uno,2 as dos,3 as tres,clave,idalma from fe_usua where 
