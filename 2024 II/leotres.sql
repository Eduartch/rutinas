...
MySQL - 8.0.39 : Database - syscom_bdleotres
*********************************************************************
*/
...
if new.acti='I' then
   ...
   if old.idusua1<>new.idusua1 then   
...
when trim(ct)="Proveedores" then
     if nid=0 then
   	 ...
   	 Select idtra into vdvto From fe_tra Where Trim(razon)=cb And idtra<>nid And tran_acti<>'I';
     end if;
...
cmarca VARCHAR(20),ccons VARCHAR(30),nidus INTEGER,cplaca1 VARCHAR(10),ctipot VARCHAR(2)) RETURNS int
BEGIN
DECLARE nid INTEGER DEFAULT 0;
INSERT  INTO fe_tra(placa,razon,dirtr,ructr,nombr,breve,marca,cons,tran_idus,placa1,tran_tipo)
VALUES(cplaca,crazo,cdir,nruc,chofe,cbreve,cmarca,ccons,nidus,cplaca1,ctipot);
...
VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,LOCALTIME,nus,1,nidautop,npvta,'C');
...
BEGIN
declare id integer;
insert into fe_guias(guia_fech,guia_ptop,guia_ptoll,guia_idau,guia_fect,guia_idus,guia_fope,guia_deta,guia_idtr,guia_ndoc,guia_codt)
values(dfecha,cptop,cptoll,nidauto,dfechat,nidus,localtime,cdeta,nidtr,cndoc,nidtda);
...
guia_idtr,guia_ndoc,guia_moti,guia_codt)
...
BEGIN
   if ctipo="C" then
      ...
      if nalma=4 then
          UPDATE fe_art SET cua=cua+ccan,prod_acti='A' WHERE idart=coda;
     end if;
   end if;
   if ctipo="V" then
      if nalma=0 then
...
nid INTEGER,cplaca1 VARCHAR(11),ctipot VARCHAR(2))
BEGIN
UPDATE fe_tra SET ructr=cruc,razon=crazo,nombr=cchofer,marca=cmarca,placa=cplaca,dirtr=cdire,
breve=cbreve,cons=ccons,placa1=cplaca1,tran_tipo=ctipot WHERE idtra=nid;
END */$$
DELIMITER ;
...
IF opt=1 THEN
    SET cbuscar=CONCAT('%',TRIM(abuscar),+'%');
    SELECT idart,descri,unid,uno,dos,tre,cua,cero,
    ROUND(prod_uti1*ROUND(IF(tmon='S',(a.prec*vigv)+b.prec,(a.prec*vigv*IF(prod_dola>nd,prod_dola,nd))+b.prec),2),2) AS pre1,
    ROUND(prod_uti2*ROUND(IF(tmon='S',(a.prec*vigv)+b.prec,(a.prec*vigv*IF(prod_dola>nd,prod_dola,nd))+b.prec),2),2) AS pre2,
    ROUND(prod_uti3*ROUND(IF(tmon='S',(a.prec*vigv)+b.prec,(a.prec*vigv*IF(prod_dola>nd,prod_dola,nd))+b.prec),2),2) AS pre3,
    ROUND(IF(tmon='S',(a.prec*vigv)+b.prec,(a.prec*vigv*IF(prod_dola>nd,prod_dola,nd))+b.prec),2) AS costo,c.idgrupo,c.dcat,prod_dola,
    ROUND(IF(tmon='S',(a.prec*vigv),(a.prec*vigv*nd)),2) AS costosf,b.prec AS flete,m.dmar,
    MID(descri,1,LENGTH(TRIM(descri))-LENGTH(TRIM(m.dmar))) AS descri1,prod_cod1,
    IFNULL(d.cost_cost,0) AS costor,IFNULL(d.cost_prec,0) AS precr,IFNULL(d.cost_mone,'')  AS moner,
    CAST(IFNULL(d.cost_idco,0) AS UNSIGNED) AS cost_idco,IFNULL(d.cost_flet,0)  AS fleter,IFNULL(d.cost_dola,0) AS dolar,
    peso,a.prec,tipro,a.idmar,a.idcat,cost,tmon,a.idflete,prod_uti1,prod_uti2,prod_uti3,
    prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,IFNULL(o.razo,'') AS proveedor,
    IFNULL(y.ndoc,'') AS ndoc,IFNULL(y.fech,'') AS fech,ulfc,prod_ent1,prod_ent2,prod_icbper,prod_acti
    FROM fe_art  AS a INNER JOIN fe_fletes AS b ON(b.idflete=a.idflete)
    INNER JOIN fe_mar AS m ON m.idmar=a.idmar
    INNER JOIN fe_cat AS c ON(c.idcat=a.idcat) LEFT JOIN fe_costos AS d ON(d.cost_idco=a.prod_idco)
    LEFT JOIN fe_rcom AS Y ON (y.idauto=a.prod_idau) LEFT JOIN fe_prov AS o ON (o.idprov=y.idprov)
...
/*!50001 CREATE TABLE  `vlistaprecios`(
 ...
 `tmon` varchar(1) ,
 `idflete` int ,
 `prod_uti1` decimal(10,7) ,
 `prod_uti2` decimal(10,7) ,
 `prod_uti3` decimal(10,7) ,
 `prod_come` float ,
 `prod_comc` float ,
 `ulpc` int ,
 `prod_idus` int ,
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vguiasdevolucion` AS select `b`.`guia_idgui` AS `idguia`,`a`.`idart` AS `coda`,`a`.`descri` AS `descri`,`a`.`unid` AS `unid`,`b`.`guia_ndoc` AS `ndoc`,`b`.`guia_fech` AS `fech`,`b`.`guia_fect` AS `fect`,`b`.`guia_ptoll` AS `ptoll`,`b`.`guia_deta` AS `detalle`,`x`.`entr_cant` AS `cant`,`y`.`placa` AS `placa`,ifnull(`y`.`razon`,'') AS `Transportista`,`y`.`ructr` AS `ructr`,`y`.`nombr` AS `Chofer`,`y`.`breve` AS `Brevete`,`y`.`cons` AS `Constancia`,`y`.`marca` AS `marca`,`y`.`dirtr` AS `Direccion`,`p`.`nomb` AS `usuario`,`d`.`razo` AS `cliente`,`d`.`idprov` AS `idprov`,`c`.`ndoc` AS `refe`,`c`.`tdoc` AS `tdoc`,`b`.`guia_mens` AS `guia_mens`,`b`.`guia_arch` AS `guia_arch`,`d`.`email` AS `email`,`b`.`guia_hash` AS `guia_hash`,`b`.`guia_feen` AS `guia_feen`,`b`.`guia_codt` AS `guia_codt` from (((((((`fe_guias` `b` join `fe_ent` `x` on((`x`.`entr_idgu` = `b`.`guia_idgui`))) left join `fe_tra` `y` on((`y`.`idtra` = `b`.`guia_idtr`))) join `fe_kar` `s` on((`s`.`idkar` = `x`.`entr_idkar`))) join `fe_art` `a` on((`a`.`idart` = `s`.`idart`))) join `fe_usua` `p` on((`p`.`idusua` = `b`.`guia_idus`))) join `fe_rcom` `c` on((`c`.`idauto` = `b`.`guia_idau`))) join `fe_prov` `d` on((`d`.`idprov` = `c`.`idprov`))) where (`b`.`guia_acti` <> 'I') */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vguiasventas` AS select `b`.`guia_arch` AS `guia_arch`,`d`.`clie_corr` AS `clie_corr`,`b`.`guia_idgui` AS `idguia`,`a`.`idart` AS `coda`,`a`.`descri` AS `descri`,`a`.`unid` AS `unid`,`b`.`guia_ndoc` AS `ndoc`,`b`.`guia_fech` AS `fech`,`b`.`guia_fect` AS `fect`,`b`.`guia_ptoll` AS `ptoll`,`b`.`guia_deta` AS `detalle`,`x`.`entr_cant` AS `cant`,`y`.`placa` AS `placa`,ifnull(`y`.`razon`,'') AS `Transportista`,`y`.`ructr` AS `ructr`,`y`.`nombr` AS `Chofer`,`y`.`breve` AS `Brevete`,`y`.`cons` AS `Constancia`,`y`.`marca` AS `marca`,`y`.`dirtr` AS `Direccion`,`p`.`nomb` AS `usuario`,`d`.`razo` AS `cliente`,`d`.`idclie` AS `idcliente`,`c`.`ndoc` AS `refe`,`c`.`tdoc` AS `tdoc`,`b`.`guia_mens` AS `guia_mens` from (((((((`fe_guias` `b` join `fe_ent` `x` on((`x`.`entr_idgu` = `b`.`guia_idgui`))) left join `fe_tra` `y` on((`y`.`idtra` = `b`.`guia_idtr`))) join `fe_kar` `s` on((`s`.`idkar` = `x`.`entr_idkar`))) join `fe_art` `a` on((`a`.`idart` = `s`.`idart`))) join `fe_usua` `p` on((`p`.`idusua` = `b`.`guia_idus`))) join `fe_rcom` `c` on((`c`.`idauto` = `b`.`guia_idau`))) join `fe_clie` `d` on((`d`.`idclie` = `c`.`idcliente`))) where (`b`.`guia_acti` <> 'I') */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vkardexc` AS select `a`.`idart` AS `idart`,`a`.`tipo` AS `tipo`,`a`.`cant` AS `cant` from (`fe_kar` `a` join `fe_rcom` `b` on((`b`.`idauto` = `a`.`idauto`))) where ((`b`.`acti` = 'A') and (`a`.`acti` = 'A') and (`b`.`rcom_tipo` = 'C')) order by `a`.`idart` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vpdtespago` AS select `a`.`ndoc` AS `ndoc`,`a`.`fech` AS `fech`,`a`.`dola` AS `dola`,`b`.`rdeu_idct` AS `rdeu_idct`,`a`.`nrou` AS `nrou`,`a`.`banc` AS `banc`,`a`.`iddeu` AS `iddeu`,max(`a`.`fevto`) AS `fevto`,round(sum((`a`.`impo` - `a`.`acta`)),2) AS `saldo`,`b`.`rdeu_idpr` AS `Idpr`,`b`.`rdeu_impc` AS `ImporteC`,'C' AS `situa`,`b`.`rdeu_idau` AS `Idauto`,`a`.`ncontrol` AS `ncontrol`,`a`.`tipo` AS `tipo`,`a`.`banco` AS `banco`,ifnull(`c`.`ndoc`,'0') AS `docd`,ifnull(`c`.`tdoc`,'0') AS `tdoc`,`b`.`rdeu_mone` AS `Moneda`,`b`.`rdeu_codt` AS `Codt`,`b`.`rdeu_idrd` AS `Idrd` from ((`fe_deu` `a` join `fe_rdeu` `b` on((`b`.`rdeu_idrd` = `a`.`deud_idrd`))) left join `fe_rcom` `c` on((`c`.`idauto` = `b`.`rdeu_idau`))) where ((`b`.`rdeu_Acti` <> 'I') and (`a`.`acti` <> 'I')) group by `a`.`ncontrol` having (round(sum((`a`.`impo` - `a`.`acta`)),2) > 0) order by max(`a`.`fevto`) */;
