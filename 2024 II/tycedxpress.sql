...
MySQL - 8.0.43 : Database - syscom_bdtexpressc
*********************************************************************
*/
...
BEGIN
DECLARE nidauto,nidautor INTEGER DEFAULT 0;
SELECT guia_idau INTO nidauto FROM fe_guias WHERE guia_idgui=na LIMIT 1;
IF nidauto>0 THEN
     select idauto into nidautor from fe_rcom where idauto=nidauto and tdoc='09' and acti='A' limit 1;   
     if nidautor>0 then 
       UPDATE fe_rcom SET acti='I' WHERE idauto=nidautor;
     end if;  
END IF;
...
when opt=0 then
    ...
    CAST(ROUND(IF(tmon='S',((a.prec*prod_tigv)+b.prec)*prod_uti1,((a.prec*prod_tigv*nd)+b.prec)*prod_uti1),3) AS DECIMAL(12,3)) AS pre1,
    CAST(ROUND(IF(tmon='S',((a.prec*prod_tigv)+b.prec)*prod_uti2,((a.prec*prod_tigv*nd)+b.prec)*prod_uti2),3) AS DECIMAL(12,3)) AS pre2,
    CAST(ROUND(IF(tmon='S',((a.prec*prod_tigv)+b.prec)*prod_uti3,((a.prec*prod_tigv*nd)+b.prec)*prod_uti3),3) AS DECIMAL(12,3)) AS pre3,
    cast(ROUND(IF(tmon='S',(a.prec*prod_tigv)+b.prec,(a.prec*prod_tigv*nd)+b.prec),3) as decimal(12,3)) AS costo,c.idgrupo,c.dcat,prod_dola,prod_cod1 AS cod1,
