...
MySQL - 8.0.42 : Database - syscom_bdnietohag
*********************************************************************
*/
...
ncome float,tigv decimal(6,3),nutil1 float,nutil2 float,nutil3 float,nidusua integer,nsmin float,nsmax float,ccoda varchar(20),ndolar float) RETURNS int
BEGIN
declare nid integer default 0;
...
VALUES (nid,cc,'V',npr,nct,tmvto,cincl,calma,nidcosto1,ccodv,0,vcom);
SELECT LAST_INSERT_ID() INTO nidk FROM fe_kar GROUP BY LAST_INSERT_ID();
RETURN nidk;
END */$$
DELIMITER ;
...
SET nid=0;
INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,
dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,rcom_tipo,pimpo,rcom_fech,rcom_icbper,rcom_exon)
VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,
ctg,ccodp,cmvto,LOCALTIME,nus,nidcodt,ctipo,npvta,dfecha,nicbper,nv);
SELECT LAST_INSERT_ID() INTO nid FROM fe_rcom GROUP BY LAST_INSERT_ID();
IF n1>0 AND n2>0 AND n3>0 THEN
   CALL IngresaCuentasV(nv,nigv,nt,n1,n2,n3,"H","H","D",nid);
END IF;
...
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 decimal(12,6),vcom float) RETURNS int
BEGIN
declare nidk integer default 0;
if ct='C' then
  INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_cost,codv,kar_alma1)
...
nct FLOAT,cincl CHAR,tmvto CHAR,ccodv INTEGER,calma INTEGER,nidcosto1 DECIMAL(12,4),nidkar INTEGER,op INTEGER,xcom FLOAT)
BEGIN
IF op=0 THEN
  UPDATE fe_kar SET Acti='I'  WHERE idkar=nidkar;
 ELSE
  UPDATE fe_kar SET
  idauto=nid,idart=cc,prec=npr,cant=nct,ttip=tmvto,incl=cincl,
...
if opt=0 then
    if ni>1 then
      update fe_rcom set tdoc=ctdoc,form=cform,ndoc=cndoc,fech=dfecha,fecr=dfechar,deta=cdetalle,valor=nv,igv=nigv,impo=nt,ndo2=cndo2,
      mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idprov=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,rcom_exon=nt where idauto=nidauto;
      call ProDesactivaLcajaE(nidauto);
      if n1>0 and n2>0 and n3>0 then
...
mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,rcom_icbper=nicbper,rcom_exon=nv WHERE idauto=nidauto;
CALL ProDesactivaLcajaE(nidauto);
IF n1>0 AND n2>0 AND n3>0 THEN
