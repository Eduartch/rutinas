...
MySQL - 8.0.42 : Database - syscom_bdfrancks
*********************************************************************
*/
...
ndolar float,ni float,ctg char,ccodp integer,cmvto char,nus integer,opt integer,nidcodt integer,
n1 integer,n2 integer,n3 integer,nturno integer,npvta float) RETURNS int
BEGIN
declare nid integer;
declare ctipo char;
set nid=0;
...
if opt=0 then
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,
   dolar,vigv,tcom,idprov,tipom,fusua,idusua,codt,rcom_tipo,pimpo,rcom_idtr)
   VALUES (ctdoc,cform,cndoc,dfecha,dfechar,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,
   ni,ctg,ccodp,cmvto,localtime,nus,nidcodt,ctipo,npvta,nturno);
   select last_insert_id() into nid from fe_rcom group by last_insert_id();
    if n1>0 and n2>0 and n3>0 then
   ...
   VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,
   ctg,ccodp,cmvto,localtime,nus,nidcodt,ctipo,npvta,npvta);
   select last_insert_id() into nid from fe_rcom group by last_insert_id();
   if n1>0 and n2>0 and n3>0 then
      call IngresaCuentasV(nv,nigv,nt,n1,n2,n3,"H","H","D",nid);
   end if;
...
/*!50003 CREATE FUNCTION `FunIngresaGuiasXdCompras`(dfecha DATETIME,cptop VARCHAR(100),cptoll VARCHAR(150),nidauto INTEGER,
dfechat DATETIME,nidus INTEGER,cdeta VARCHAR(150),nidtr INTEGER,cndoc VARCHAR(12),nidtda INTEGER) RETURNS int
BEGIN
DECLARE id INTEGER;
...
dfechat DATE,nidus INTEGER,cdeta VARCHAR(150),cndoc VARCHAR(12),nidtda INTEGER,cubigeo1 VARCHAR(8),
cubigeo2 VARCHAR(8),nidv1 INTEGER,nidv2 INTEGER,nidr INTEGER,nidd INTEGER,ctdocr VARCHAR(2),cdctor VARCHAR(12),cconductor VARCHAR(120),
cplaca1 VARCHAR(10),cconstancia1 VARCHAR(20),cbrevete VARCHAR(10),nidtr INTEGER,tdoc CHAR(2),motivo CHAR(1),cmarca VARCHAR(50),nidauto INTEGER) RETURNS int
BEGIN
DECLARE id INTEGER;
...
guia_tdoc,guia_moti,guia_marca,guia_idau)
VALUES(dfecha,cptop,cptoll,dfechat,nidus,LOCALTIME,cdeta,cndoc,nidtda,cubigeo1,cubigeo2,nidv1,nidv2,nidr,nidd,ctdocr,cdctor,
cconductor,cbrevete,cplaca1,cconstancia1,nidtr,tdoc,motivo,cmarca,nidauto);
SELECT LAST_INSERT_ID() INTO id FROM  fe_guias GROUP BY LAST_INSERT_ID();
RETURN id;
END */$$
DELIMITER ;
...
nv decimal(12,2),nigv decimal(12,2),nt decimal(12,2),cndo2 varchar(10),cm char,
ndolar decimal(6,4),ni decimal(6,4),ctg char,ccodp integer,nidturno integer ,nus integer,nidcodt integer,
n1 integer,n2 integer,n3 integer,tgratuitas decimal(12,2),idtr decimal(10,2),nexon decimal(12,2),ndscto decimal(12,2),ndire integer) RETURNS int
BEGIN
declare nid integer;
set nid=0;
 INSERT INTO fe_correvtas(auto_corr)VALUES(CONCAT(ctdoc,cndoc));
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idcliente,
   tipom,fusua,idusua,codt,pimpo,rcom_exon,rcom_dsct,rcom_otro,rcom_idtr,alma)
   VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,'V',localtime,nus,nidcodt,
   idtr,nexon,ndscto,tgratuitas,nidturno,ndire);
   select last_insert_id() into nid from fe_rcom group by last_insert_id();
    if n1>0 and n2>0 and n3>0 then
      call IngresaCuentasV(nv,nigv,nt,n1,n2,n3,"H","H","D",nid);
 End if;
...
/*!50003 CREATE PROCEDURE `astock`(coda integer,nalma integer,ccant decimal(10,2),ctipo char(1))
BEGIN
   if ctipo="C" then
      if nalma=0 then
...
if opt=0 then
   ...
   mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idprov=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,rcom_nitem=nitems,rcom_tipo=ctipo where idauto=nidauto;
...
ndolar float,ni float,ctg char,ccodp integer,cmvto char,nus integer,opt integer,nidcodt integer,
n1 integer,n2 integer,n3 integer,ndire integer,npvta float,nidauto integer)
BEGIN
declare idce,idve,nidctaper integer;
declare ctipo char(1);
if ctdoc='01' or ctdoc='09' or ctdoc='II' or ctdoc='07' or ctdoc='08' or ctdoc='03' then
   ...
   select gene_idve into idve from fe_gene where idgene=1;
   update fe_rcom set tdoc=ctdoc,form=cform,ndoc=cndoc,fech=dfecha,fecr=dfechar,deta=cdetalle,valor=nv,igv=nigv,impo=nt,ndo2=cndo2,
   mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,alma=ndire where idauto=nidauto;
   call ProDesactivaLcajaE(nidauto);
   if n1>0 and n2>0 and n3>0 then
      select @i1:=sum(xx.nid1),@i2:=sum(xx.nid2),@i3:=sum(xx.nid3)
      from (select case a.nitem when 1 then idectas else 0 end as nid1,
...
when opt='X' then
   SELECT idart,descri,unid,uno,dos,tre,cua,premay AS pre1,premay AS pre2, premay AS pre3,
   ...
   peso,a.prec,tipro,a.idmar,a.idcat,cost,tmon,a.idflete,prod_uti1,prod_uti2,prod_uti3,prod_idus,
   prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,IFNULL(o.razo,'') AS proveedor,
   IFNULL(yy.ndoc,'') AS ndoc,IFNULL(yy.fech,'') AS fech,ulfc,s.dmar,prod_acti,prod_stoc,prod_orde,a.coda1,prod_dmin,prod_dmax
   FROM fe_art  AS a 
   INNER JOIN fe_fletes AS b ON(b.idflete=a.idflete)
   INNER JOIN fe_cat AS c ON(c.idcat=a.idcat)
   INNER JOIN fe_mar AS s ON s.idmar=a.idmar
   INNER JOIN 
   (SELECT cont_idar AS cont_idart,max(cont_idcon) as maxidcon,max(cont_lado) as cont_lado FROM fe_contometro WHERE cont_isla=nid GROUP BY cont_idar) AS xx
   ON xx.cont_idart=a.idart 
   LEFT JOIN fe_costos AS d ON(d.cost_idco=a.prod_idco)
   LEFT JOIN fe_rcom AS yy ON (yy.idauto=a.prod_idau) 
   LEFT JOIN fe_prov AS o ON (o.idprov=a.ulpc),fe_gene AS g
   WHERE prod_acti<>'I'  AND tipro in('C');
...
case
when opt=0 then
   select idprov,nruc,razo,ndni,dire,ciud,fono,fax,email,celu,refe,prov_rpm,prov_idus,prov_actu,fechprov,prov_feac,ubig
...
WHEN opt=0 THEN
   ...
   IFNULL(celu,'0') AS celu,refe,clie_tipo,clie_codv,clie_lcre,clie_corr,clie_rpm,clie_idus,clie_actu,fechclie,clie_feac,IFNULL(c.zona_nomb,'') AS Zona,
   IFNULL(a.clie_idzo,0) AS clie_idzo,ubig
   FROM fe_clie AS a LEFT JOIN fe_zona AS c ON c.zona_idzo=a.clie_idzo 
   WHERE  clie_acti<>'I' AND razo LIKE cbuscar ORDER BY razo;
WHEN  opt=1 THEN
   SELECT idclie,nruc,razo,ndni,dire,ciud,IFNULL(fono,'0') AS fono,IFNULL(fax,'0') AS fax,
   IFNULL(celu,'0') AS celu,refe,clie_tipo,clie_codv,clie_lcre,clie_corr,clie_rpm,clie_idus,clie_actu,fechclie,clie_feac,IFNULL(c.zona_nomb,'') AS Zona,
   IFNULL(a.clie_idzo,0) AS clie_idzo,ubig
...
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidcp,nidauto,cform,cdcto,ctdoc,ncodt,LOCALTIME);
END */$$
DELIMITER ;
...
nprecio DECIMAL(12,5),ncant DECIMAL(12,4),cunid VARCHAR(4))
BEGIN
   INSERT INTO fe_detallevta(detv_desc,detv_item,detv_ite1,detv_ite2,detv_idau,detv_prec,detv_cant,detv_unid)
   VALUES(cdesc,nitem,nitem1,nitem2,nidauto,nprecio,ncant,cunid);
END */$$
...
nv DECIMAL(12,2),nigv DECIMAL(12,2),nt DECIMAL(12,2),cndo2 VARCHAR(10),cm CHAR,
ndolar FLOAT,ni FLOAT,ctg CHAR,ccodp INTEGER,cmvto CHAR,nus INTEGER,opt INTEGER,nidcodt INTEGER,
n1 INTEGER,n2 INTEGER,n3 INTEGER,nexonerado DECIMAL(12,2),npvta FLOAT,ndetraccion DECIMAL(10,2),nidauto INTEGER,cdetra varchar(10))
BEGIN
UPDATE fe_rcom SET tdoc=ctdoc,form=cform,ndoc=cndoc,fech=dfecha,fecr=dfecha,deta=cdetalle,valor=nv,igv=nigv,impo=nt,ndo2=cndo2,
mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,rcom_exon=nexonerado,rcom_mdet=ndetraccion,rcom_detr=cdetra
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vguiasrcompras` AS select `b`.`guia_idgui` AS `idguia`,`a`.`idart` AS `coda`,`a`.`descri` AS `descri`,`a`.`unid` AS `unid`,`b`.`guia_ndoc` AS `ndoc`,`b`.`guia_fech` AS `fech`,`b`.`guia_fect` AS `fect`,`b`.`guia_ptoll` AS `ptoll`,`b`.`guia_deta` AS `detalle`,`x`.`entr_cant` AS `cant`,`y`.`placa` AS `placa`,ifnull(`y`.`razon`,'') AS `Transportista`,`y`.`ructr` AS `ructr`,`y`.`nombr` AS `Chofer`,`y`.`breve` AS `Brevete`,`y`.`cons` AS `Constancia`,`y`.`marca` AS `marca`,`y`.`dirtr` AS `Direccion`,`p`.`nomb` AS `usuario`,`d`.`empresa` AS `cliente`,0 AS `idprov`,`b`.`guia_ndoc` AS `refe`,'09' AS `tdoc`,`b`.`guia_mens` AS `guia_mens`,`b`.`guia_arch` AS `guia_arch`,`d`.`correo` AS `email`,`b`.`guia_hash` AS `guia_hash`,`b`.`guia_feen` AS `guia_feen`,`b`.`guia_codt` AS `guia_codt`,`b`.`guia_tick` AS `guia_tick` from (((((`fe_guias` `b` join `fe_ent` `x` on((`x`.`entr_idgu` = `b`.`guia_idgui`))) left join `fe_tra` `y` on((`y`.`idtra` = `b`.`guia_idtr`))) join `fe_art` `a` on((`a`.`idart` = `x`.`entr_idar`))) join `fe_usua` `p` on((`p`.`idusua` = `b`.`guia_idus`))) join `fe_gene` `d`) where ((`b`.`guia_acti` <> 'I') and (`x`.`entr_acti` = 'A') and (`b`.`guia_moti` = 'C')) */;
