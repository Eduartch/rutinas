...
MySQL - 10.3.39-MariaDB : Database - eduar_bdmegabat
*********************************************************************
*/
...
/*!50003 CREATE FUNCTION `FunBuscaCodigo`(ccoda varchar(15)) RETURNS int(11)
begin
declare id integer;
...
cdire varchar(100),cciud varchar(100),cfono varchar(15),cfax varchar(15),cdni varchar (11),
ctipo char,cemail varchar(45),nidven integer,nidus integer,cpc varchar(45),ccelu varchar(15),
crefe varchar(255),linea float,crpm varchar(10),nidz integer,nidop integer,cdist varchar(100)) RETURNS int(11)
BEGIN
declare nid integer default 0;
INSERT INTO fe_clie(nruc,razo,dire,ciud,fono,fax,ndni,clie_tipo,clie_corr,clie_codv,clie_idus,idpcclie,
...
ncome float,ncomc float,nutil1 float,nutil2 float,nutil3 float,nidusua integer,nsmin float,nsmax float,nidcosto integer,ndolar float) RETURNS varchar(15) CHARSET latin1 COLLATE latin1_swedish_ci
BEGIN
...
/*!50003 CREATE FUNCTION `FunCuentaActiva`(nid integer) RETURNS int(11)
begin
declare id integer;
...
nidauto integer,nidart varchar(15),nflete float,nprec float,cmone char,ndola float,dfecha date) RETURNS int(11)
BEGIN
declare nid integer;
...
sacreedor decimal(12,2),cmone char,ndolar decimal(5,3),nidus integer,nidcp integer,nidauto integer,cform char,cdcto char(15),nidtda integer) RETURNS int(11)
begin
declare nid integer;
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
...
nidus integer,nidcp integer,nidauto integer,cform char,cdcto char(15),nidcaja integer,nidtda integer) RETURNS int(11)
begin
declare nid integer;
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
...
nidpr integer,cmone char) RETURNS int(11)
BEGIN
...
/*!50003 CREATE FUNCTION `FuningresaDCotizacion`(ncoda varchar(15),ncant float,nprec float,nidauto integer) RETURNS int(11)
BEGIN
declare id integer default 0;
...
`cgarantia` VARCHAR(50), `nus` INTEGER, `nidcodt` INTEGER, `n1` INTEGER, `n2` INTEGER, `n3` INTEGER, `nvend` INTEGER, `idtr` DECIMAL(10,2), `nexon` DECIMAL(12,2), `ndscto` DECIMAL(12,2)) RETURNS int(11)
BEGIN
DECLARE nid INTEGER;
SET nid=0;
...
/*!50003 CREATE FUNCTION `FunIngresaDPedidos`(ncoda varchar(10),ncant float,nprec float,nidauto integer) RETURNS int(11)
BEGIN
declare id integer default 0;
...
dfechat DATETIME,nidus INTEGER,cdeta VARCHAR(150),nidtr INTEGER,cndoc VARCHAR(12),nidtda INTEGER,cubigeo VARCHAR(8),nidclie INTEGER) RETURNS int(11)
BEGIN
DECLARE id INTEGER;
...
nct float,cincl char,ct1 char,cdeta varchar(50),nidtda integer,nidtda1 integer,na1 integer) RETURNS int(11)
BEGIN
declare nidk integer default 0;
...
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 integer,vcom float,codi integer) RETURNS int(11)
BEGIN
declare nidk integer default 0;
if ct='C' then
...
/*!50003 CREATE FUNCTION `FUNINGRESANOTASCREDITOventas1`(nid0 INTEGER,nid1 INTEGER,nidcr INTEGER,nimpo DECIMAL(12,2)) RETURNS int(11)
BEGIN
DECLARE id INTEGER DEFAULT 0;
INSERT INTO fe_ncven(ncre_idan,ncre_idau,ncre_idcr,ncre_impo)VALUES(nid0,nid1,nidcr,nimpo);
SELECT LAST_INSERT_ID() INTO id FROM fe_ncven GROUP BY LAST_INSERT_ID();
RETURN id;
END */$$
DELIMITER ;
...
ctipo CHAR,nimpo DECIMAL(12,2),nidalma INTEGER,deta varchar(100)) RETURNS int(11)
BEGIN
DECLARE nid INTEGER DEFAULT 0;
INSERT INTO fe_rcomu(rcom_fech,rcom_ndoc,rcom_fope,rcom_tipo,rcom_impo,rcom_codt,rcom_deta)
VALUES(dfecha,cndoc,LOCALTIME,ctipo,nimpo,nidalma,deta);
...
cmon char,dfecha date,nimpoo decimal(12,2),nidus integer,nidtda integer,cpc varchar(45)) RETURNS int(11)
BEGIN
declare id integer default 0;
...
SELECT karu_prec INTO nid FROM fe_karu AS a 
INNER JOIN fe_rcomu AS b ON b.rcom_idau=a.karu_idau 
WHERE karu_idar=nidart  AND a.karu_Acti='A' AND b.rcom_acti='A' ORDER BY rcom_fech DESC LIMIT 1;
RETURN nid;
END */$$
DELIMITER ;
...
/*!50003 CREATE FUNCTION `FunValidaProductos`(nid varchar(15)) RETURNS int(11)
BEGIN
declare sw integer;
DECLARE CONTINUE HANDLER FOR NOT FOUND SET sw=0;
...
prod_smin=nsmin,prod_idco=nidcosto,prod_dola=ndolar  WHERE trim(idart)=trim(ncoda);
END */$$
DELIMITER ;
...
ctipo CHAR,opt INTEGER,nimpo DECIMAL(12,2),nidalma INTEGER,nid INTEGER,cdeta varchar(100))
BEGIN
IF opt=0 THEN
   UPDATE fe_rcomu  SET rcom_acti='I' WHERE rcom_idau=nid;
ELSE
  UPDATE fe_rcomu  SET rcom_fech=dfecha,rcom_ndoc=cndoc,
  rcom_tipo=ctipo,rcom_impo=nimpo,rcom_codt=nidalma,rcom_deta=cdeta WHERE rcom_idau=nid;
END IF;
END */$$
DELIMITER ;
...
VALUES(dfecha,cdeta,cesta,nimpo,ctipo,nidauto,ncodt);
END */$$
DELIMITER ;
...
BEGIN
SELECT nomb,idalma,dire,ciud,sucuidserie,sucu_idus FROM fe_sucu  where idalma in(1) order by idalma;
END */$$
DELIMITER ;
...
/*!50001 CREATE TABLE  `vexporta`(
 ...
 `deta` varchar(80) ,
 `exon` varchar(1) ,
 `rcom_idin` int(10) unsigned ,
 `ndo2` varchar(10) ,
 `idclie` int(11) ,
 `NombreCliente` varchar(100) ,
 `clie_codv` int(10) unsigned ,
 `clie_idus` int(10) unsigned ,
 `clie_idzo` int(10) unsigned ,
