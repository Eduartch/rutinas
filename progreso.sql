...
MySQL - 8.0.37 : Database - sysven_bdprogreso
*********************************************************************
*/
...
/*!50003 CREATE FUNCTION `FunBuscaCodigo`(ccod  varchar(15),nid integer) RETURNS int
BEGIN
declare vdvto integer default 0;
DECLARE CONTINUE HANDLER FOR NOT FOUND SET vdvto=0;
if nid=0 then
    select idart into vdvto from fe_art where trim(prod_coda)=trim(ccod) and prod_Acti='A';
else
    select idart into vdvto from fe_art where trim(prod_coda)=trim(ccod) and prod_Acti='A' and idart<>nid;
end if;
if vdvto>0 then
   return 0;
else
...
when trim(ct)="Colores" then
     if nid=0 then
         Select colr_idco into vdvto From fe_colores Where Trim(colr_desc)=cb And colr_acti<>'I';
     else
   	    Select colr_idco into vdvto From fe_colores Where Trim(colr_desc)=cb And colr_idco<>nid And colr_acti<>'I';
     end if;
...
/* Function  structure for function  `FuncreaColores` */

/*!50003 DROP FUNCTION IF EXISTS `FuncreaColores` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FuncreaColores`(cdesc varchar(120),codigo varchar(3)) RETURNS int
begin
decLARE nid integer;
declare ccoda varchar(3);
insert into fe_colores(colr_desc,colr_coda)values(cdesc,codigo);
select last_insert_id() into nid from fe_colores group by last_insert_id();
set ccoda=cast(nid as char(3));
update fe_colores set colr_coda=ccoda where colr_idco=nid;
return nid;
end */$$
DELIMITER ;
...
/* Function  structure for function  `FuncreaModelos` */

/*!50003 DROP FUNCTION IF EXISTS `FuncreaModelos` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FuncreaModelos`(cdesc varchar(120),codigo char(3)) RETURNS int
begin
decLARE nid integer;
insert into fe_modelos(mode_desc,mode_coda)values(cdesc,codigo);
select last_insert_id() into nid from fe_modelos group by last_insert_id();
return nid;
end */$$
DELIMITER ;
...
/*!50003 CREATE FUNCTION `FuncreaProductos`(cdesc varchar(100),ctipo varchar(10),ccoda varchar(15),cnume varchar(5),
npm decimal(10,2),
npn decimal(10,2),ccodp integer,cmone char,nutil1 decimal(6,2),nutil2 decimal(6,2),ncomi decimal(5,2),cmode varchar(80),
smin decimal(4),smax decimal(4),nidus integer,idpc varchar(50),npeso decimal(8,2)) RETURNS int
BEGIN
declare nid integer default 0;
INSERT INTO fe_art(descri,prod_tipo,prod_coda,prod_nume,premay,premen,prod_codp,tmon,prod_uti1,
prod_uti2,fechc,prod_come,prod_des1,prod_smin,prod_smax,idmar,idcat,idflete,prod_idus,idpc,peso)
VALUES (cdesc,ctipo,ccoda,cnume,npm,npn,ccodp,cmone,nutil1,nutil2,localtime,ncomi,cmode,smin,smax,1,1,1,nidus,idpc,npeso);
...
/* Function  structure for function  `FunIngresaCabeceraCanjeVTas` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaCabeceraCanjeVTas` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaCabeceraCanjeVTas`(
...
set nid=0;
   ...
   dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,rcom_nitem,rcom_tipo,pimpo,rcom_fech,rcom_ccaj)
   VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,
   ctg,ccodp,cmvto,localtime,nus,nidcodt,nitem,ctipo,npvta,dfecha,'C');
...
nv float,nigv float,nt float,cndo2 varchar(10),cm char,
ndolar float,ni float,ctg char,ccodp integer,cmvto char,nus integer,opt integer,nidcodt integer,
n1 integer,n2 integer,n3 integer,nitem decimal(12,2),npvta float) RETURNS int
BEGIN
declare nid,ntdoc,idce,idve,nctaotros integer default 0;
declare ctipo,ctipo1 char;
set ntdoc=0;
set nid=0;
set nctaotros=0;
if opt=0 then
   ...
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,
   dolar,vigv,tcom,idprov,tipom,fusua,idusua,codt,rcom_tipo,pimpo,rcom_fech,rcom_otro)
   VALUES (ctdoc,cform,cndoc,dfecha,dfechar,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,
   ni,ctg,ccodp,cmvto,localtime,nus,nidcodt,ctipo,npvta,dfecha,nitem);
   select last_insert_id() into nid from fe_rcom group by last_insert_id();
   if n1>0 and n2>0 and n3>0 then
      if nitem>0 then
         set nctaotros=138;
      ...
      call IngresaCuentas(nv,0,0,0,nigv,0,nitem,nt,n1,0,0,0,n2,0,nctaotros,n3,"D","","","","D","",ctipo1,"H",nid);
    end if;
  else
   select gene_idve into idve from fe_gene where idgene=1;
   if ctdoc='20' then
...
nv FLOAT,nigv FLOAT,nt FLOAT,cndo2 VARCHAR(10),cm CHAR,
ndolar FLOAT,ni FLOAT,ctg CHAR,ccodp INTEGER,cvtar CHAR,nus INTEGER,nidcodt INTEGER,
...
END IF;
INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,
dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,rcom_tipo,pimpo,rcom_fech,rcom_icbper,rcom_vtar)
VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,
ctg,ccodp,'V',localtime,nus,nidcodt,ctipo,npvta,dfecha,nicbper,cvtar);
SELECT LAST_INSERT_ID() INTO nid FROM fe_rcom GROUP BY LAST_INSERT_ID();
IF n1>0 AND n2>0 AND n3>0 THEN
   CALL IngresaCuentasV(nv,nigv,nt,n1,n2,n3,"H","H","D",nid);
END IF;
...
/* Function  structure for function  `FunIngresaKardexCompras` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaKardexCompras` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaKardexCompras`(nid integer,cc integer,ct char,npr float,
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 integer,nprel decimal(12,6),nd1 decimal(6,2),nd2 decimal(6,2),nd3 decimal(6,2)) RETURNS int
BEGIN
declare nidk integer default 0;
 INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_idco,kar_prel,kar_alma1,kar_dct1,kar_dct2,kar_dct3)
  VALUES (nid,cc,ct,npr,nct,tmvto,cincl,calma,nidcosto1,nprel,calma,nd1,nd2,nd3);
select last_insert_id() into nidk from fe_kar group by last_insert_id();
return nidk;
END */$$
DELIMITER ;
...
nct FLOAT,cincl CHAR,tmvto CHAR,ccodv INTEGER,calma INTEGER,nidcosto1 INTEGER,vcom FLOAT) RETURNS int
BEGIN
DECLARE nidk INTEGER DEFAULT 1;
...
/*!50003 CREATE FUNCTION `FunIngresaKardexTraspaso`(nid integer,cc integer,ct char,npr float,
nct float,cincl char,ccodv integer,ct1 char,cdeta varchar(50),nidtda integer,nidtda1 integer,na1 integer,nidk1 integer) RETURNS int
BEGIN
declare nidk integer default 0;
INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,incl,codv,ttip,alma,kar_alma1,kar_idtr)
VALUES (nid,cc,ct,npr,nct,cincl,ccodv,ct1,nidtda,nidtda,nidk1);
select last_insert_id() into nidk from fe_kar group by last_insert_id();
insert into fe_traspaso(tras_idka,tras_idau,tras_refe,tras_codt,tras_codt1,tras_idau1)values(nidk,nid,cdeta,nidtda,nidtda1,na1);
return nidk;
END */$$
...
select rdeu_idrd into xi from fe_rdeu  where rdeu_acti<>'I' and rdeu_idau=nid group by rdeu_idrd;
...
/* Function  structure for function  `FunVerificaFechaReposicion` */

/*!50003 DROP FUNCTION IF EXISTS `FunVerificaFechaReposicion` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunVerificaFechaReposicion`(dfecha date) RETURNS int
begin
declare vdvto integer default 0;
DECLARE CONTINUE HANDLER FOR NOT FOUND SET vdvto=0;
select repo_idre into vdvto from fe_repone where repo_fech=dfecha and repo_acti='A' group by repo_fech;
   if vdvto>0 then
      return 0;
     else
      return 1;
...
nv decimal(12,2),nigv decimal(12,2),nt decimal(12,2),cndo2 varchar(10),cm char,
ndolar float,ni float,ctg char,ccodp integer,cmvto char,nus integer,opt integer,nidcodt integer,
n1 integer,n2 integer,n3 integer,nitems decimal(10,2),npvta float,nidauto integer)
BEGIN
declare idce,idve,nidctaper,nctaotros integer;
declare ctipo1 char;
set nctaotros=0;
if opt=0 then
   select gene_idce into idce from fe_gene where idgene=1;
   ...
   mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idprov=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,rcom_otro=nitems where idauto=nidauto;
   ...
   if n1>0 and n2>0 and n3>0 then
      ...
      select @id1:=sum(x.nid1),@id5:=sum(x.nid2),@id6:=sum(x.nid3),@id7:=sum(x.nid7),@id8:=sum(x.nid4)
      ...
      case nitem when 7 then idectas else 0 end as nid7,
      case nitem when 8 then idectas else 0 end as nid4,idrcon
      from fe_ectasc as a where idrcon=nidauto) as x group by idrcon;
      if @id1>0 then
        call ProActualizaCuentasc(nv,0,0,0,nigv,0,nitems,nt,n1,0,0,0,n2,0,nctaotros,n3,@id1,@id5,@id6,@id7,@id8,"D","D","D",ctipo1,"H");
      else
           call IngresaCuentas(nv,0,0,0,nigv,0,nitems,nt,n1,0,0,0,n2,0,nctaotros,n3,"D","","","","D","",ctipo1,"H",nidauto);
      end if;
    end if;
  else
   select gene_idve into idve from fe_gene where idgene=1;
   update fe_rcom set tdoc=ctdoc,form=cform,ndoc=cndoc,fech=dfecha,fecr=dfechar,deta=cdetalle,valor=nv,igv=nigv,impo=nt,ndo2=cndo2,
   mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom="V",idusua1=nus,codt=nidcodt,rcom_vtar="T" where idauto=nidauto;
   call ProDesactivaLcajaE(nidauto);
   if n1>0 and n2>0 and n3>0 then
...
ctdoc VARCHAR(2),cform CHAR,cndoc VARCHAR(12),dfecha DATE,cdetalle VARCHAR(120),
nv DECIMAL(12,2),nigv DECIMAL(12,2),nt DECIMAL(12,2),cndo2 VARCHAR(10),cm CHAR,
ndolar FLOAT,ni FLOAT,ctg CHAR,ccodp INTEGER,cvtar CHAR,
...
mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom='V',idusua1=nus,codt=nidcodt,rcom_icbper=nicbper,rcom_tipo=ctipo,rcom_vtar=cvtar WHERE idauto=nidauto;
CALL ProDesactivaLcajaE(nidauto);
IF n1>0 AND n2>0 AND n3>0 THEN
...
idv7 integer,idv4 integer,ct1 char,ct2 char,ct3 char,ct7 char,ct4 char)
BEGIN
update fe_ectasc set impo=nv1,idcta=nid1,ecta_tipo=ct1 where idectas=idv1;
...
/* Procedure structure for procedure `ProActualizaEstadoReposicion` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizaEstadoReposicion` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizaEstadoReposicion`(nidre integer)
begin
   update fe_repone set repo_esta='E' where repo_idre=nidre;
end */$$
DELIMITER ;

/* Procedure structure for procedure `ProActualizaestadoVenta` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizaestadoVenta` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizaestadoVenta`(nidauto integer)
BEGIN
update fe_rcom set rcom_entr='F',deta='Canjeado' where idauto=nidauto;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProActualizaKardexCompras` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizaKardexCompras` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizaKardexCompras`(nid integer,cc integer,ct char,npr float,
nct float,cincl char,tmvto char,nprel decimal(12,6),calma integer,nidcosto1 integer,nidkar integer,op integer,xcom float,
nd1 decimal(6,2),nd2 decimal(6,2),nd3 decimal(6,2))
BEGIN
if op=0 then
  Update fe_kar set Acti='I'  where idkar=nidkar;
 else
  Update fe_kar set
  idauto=nid,idart=cc,tipo=ct,prec=npr,cant=nct,ttip=tmvto,incl=cincl,
  alma=calma,kar_idco=nidcosto1,kar_prel=nprel,kar_comi=xcom,kar_dct1=nd1,kar_dct2=nd2,kar_dct3=nd3 where idkar=nidkar;
end if;
END */$$
DELIMITER ;
...
cc integer,dfe date,npr float,cnd integer,idp integer,cmda char,ni float,ndolar decimal(8,4),nidcosto integer,nd1 decimal(6,2),nd2 decimal(6,2), nd3 decimal(6,2))
BEGIN
...
SELECT ulfc into @ufc FROM fe_art WHERE idart=cc;
   ...
   prod_dct1=nd1,prod_dct2=nd2,prod_dct3=nd3,ulfc=dfe,prod_dola=ndolar WHERE idart=cc;
END */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProActualizaProductos`(cdesc varchar(100),ctipo varchar(10),ccoda varchar(15),cnume varchar(5),
npm decimal(10,2),
npn decimal(10,2),ccodp varchar(20),cmone char,nutil1 decimal(6,2),nutil2 decimal(6,2),ncomi decimal(5,2),cmode varchar(80),
smin decimal(4),smax decimal(4),nidus integer,idpc varchar(50),ncoda integer,npeso decimal(8,2))
BEGIN
UPDATE fe_art SET descri=cdesc,
prod_tipo=ctipo,prod_coda=ccoda,prod_nume=cnume,premay=npm,premen=npn,prod_codp=ccodp,tmon=cmone,prod_uti1=nutil1,
prod_uti2=nutil2,prod_come=ncomi,prod_des1=cmode,prod_smin=smin,prod_smax=smax,prod_idus=nidus,prod_fact=localtime,peso=npeso
WHERE idart=ncoda;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProActualizaReposicion` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizaReposicion` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizaReposicion`(dfecha date,nidart integer,ncant decimal(12),cndoc varchar(12),nidua integer,nidre integer,opt integer)
begin
if opt=0 then
   update fe_repone set repo_acti='I' where repo_idre=nidre;
else
   update fe_repone set repo_idar=nidart,repo_fech=dfecha,repo_cant=ncant,repo_ndoc=cndoc,repo_idua=nidua where repo_idre=nidre;
end if;
end */$$
DELIMITER ;
...
BEGIN
declare nidauto integer default 0;
SELECT guia_idau INTO nidauto FROM fe_guias WHERE guia_idgui=na LIMIT 1;
if nidauto>0 then
   update fe_rcom set acti='I' where idauto=nidauto and tdoc='09';
end if;
...
/*!50003 CREATE PROCEDURE `ProAnulaTransacciones1`(OUT estado varchar(500),in ctdoc varchar(2),in cndoc varchar(10),
...
if nid>0 then
   ...
   if ctipo='V' then
      ...
      if sw='S' then
         ...
         SELECT @nidconcepto:=idcon FROM fe_con WHERE tdoc=cconcepto AND tipo='I' AND conc_acti<>'I' GROUP BY idcon;
         call PROingresa_anulada1(@df,@cndoc,@ct,nu,@nidconcepto,0);
      end if;
    else
...
BEGIN
select uno+dos as uno,dos,tre,cua from fe_art where idart=nidart;
END */$$
DELIMITER ;
...
begin
if nalma=1 then
  select uno+dos as saldo from fe_art where idart=nidart;
else
  if nalma=2 then
     select dos as saldo FROM fe_art WHERE idart=nidart;
  else   
    select tre as saldo from fe_art where idart=nidart;
  end if;  
End if;
end */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProIngresaDatosLcajaEefectivo`(dfecha date,cndoc varchar(10),cdeta varchar(100),idcta integer,sdeudor decimal(12,2),
sacreedor decimal(12,2),cmone char,ndolar decimal(5,3),nidus integer,nidcp integer,nidauto integer,cform char,cdcto varchar(12),ctdoc varchar(2))
begin
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
lcaj_idus,lcaj_clpr,lcaj_idau,lcaj_form,lcaj_dcto,lcaj_tdoc)values
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidcp,nidauto,cform,cdcto,ctdoc);
end */$$
DELIMITER ;

/* Procedure structure for procedure `ProIngresaDatosLcajaEfectivo` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProIngresaDatosLcajaEfectivo` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProIngresaDatosLcajaEfectivo`(dfecha date,cndoc varchar(10),cdeta varchar(100),idcta integer,sdeudor decimal(12,2),
sacreedor decimal(12,2),cmone char,ndolar decimal(5,3),nidus integer,nidcp integer,nidauto integer,cform char,cdcto varchar(12),ctdoc varchar(2))
begin
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
lcaj_idus,lcaj_clpr,lcaj_idau,lcaj_form,lcaj_dcto,lcaj_tdoc)values
...
/* Procedure structure for procedure `ProIngresaReposicion` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProIngresaReposicion` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProIngresaReposicion`(dfecha date,nidart integer,ncant decimal(12),cndoc varchar(12),nidus integer)
begin
  insert into fe_repone(repo_fech,repo_idar,repo_cant,repo_ndoc,repo_idus)values(dfecha,nidart,ncant,cndoc,nidus);
end */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProMuestraColores` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProMuestraColores` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProMuestraColores`(abuscar varchar(120),opt integer)
BEGIN
declare cbusca varchar(20);
set cbusca=concat('%',trim(abuscar),+'%');
if opt=1 then
  SELECT a.colr_coda,a.colr_desc,colr_idco,ifnull(count(b.idart),0) as TotalProductos FROM
  fe_colores as a left join (select idart,prod_colr from fe_art where prod_acti='A') as b on b.prod_colr=a.colr_idco
  WHERE colr_coda LIKE cbusca and colr_acti='A'group by a.colr_idco ORDER BY colr_idco;
else
   SELECT a.colr_coda,a.colr_desc,colr_idco,ifnull(count(b.idart),0) as TotalProductos FROM
  fe_colores as a left join (select idart,prod_colr from fe_art where prod_acti='A') as b on b.prod_colr=a.colr_idco
  WHERE colr_desc LIKE cbusca and colr_acti='A'group by a.colr_idco ORDER BY colr_desc;
end if;
END */$$
DELIMITER ;

/* Procedure structure for procedure `ProMuestraColoresx` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProMuestraColoresx` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProMuestraColoresx`(abuscar varchar(120),opt integer)
BEGIN
declare cbusca varchar(20);
set cbusca=concat('%',trim(abuscar),+'%');
if opt=1 then
  SELECT a.colr_coda,a.colr_desc,colr_idco FROM
  fe_colores as a  WHERE colr_coda LIKE cbusca and colr_acti='A' ORDER BY colr_idco;
else
   SELECT a.colr_coda,a.colr_desc,colr_idco FROM
  fe_colores as a WHERE colr_desc LIKE cbusca and colr_acti='A'  ORDER BY colr_desc;
end if;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProMuestraModelos` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProMuestraModelos` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProMuestraModelos`(abuscar varchar(120))
BEGIN
declare cbusca varchar(20);
set cbusca=concat('%',trim(abuscar),+'%');
SELECT a.mode_coda,a.mode_desc,ifnull(count(b.idart),0) as TotalProductos,mode_idmo FROM
fe_modelos as a left join (select idart,prod_mode from fe_art where prod_acti='A') as b on b.prod_mode=a.mode_idmo
WHERE mode_desc LIKE cbusca and mode_acti='A'group by a.mode_idmo ORDER BY mode_desc;
END */$$
DELIMITER ;

/* Procedure structure for procedure `ProMuestraModelosx` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProMuestraModelosx` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProMuestraModelosx`(abuscar varchar(120))
BEGIN
declare cbusca varchar(20);
set cbusca=concat('%',trim(abuscar),+'%');
SELECT a.mode_coda,a.mode_desc,mode_idmo FROM
fe_modelos as a wHERE mode_desc LIKE cbusca and mode_acti='A' ORDER BY mode_desc;
END */$$
DELIMITER ;
...
IF opt=1 THEN
    ...
    premen  AS pre2,
    premen  AS pre3,
    ROUND(prec*vigv,2) AS costo,prod_dola,
    ROUND(prec*vigv,2) AS costosf,prod_des1 AS descri1,prod_coda,MID(prod_coda,6,3) AS color,
    peso,a.prec,tipro,a.idmar,a.idcat,cost,tmon,a.idflete,prod_uti1,prod_uti2,prod_uti3,
    prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,prod_idau,
    ulfc,prod_nume,prod_tipo,prod_dct1,prod_dct2,prod_dct3,prod_codp,'n' AS modi,uldc,prod_icbper
    FROM fe_art  AS a  WHERE descri LIKE cbuscar AND prod_acti<>'I' ORDER BY prod_des1,prod_coda,prod_nume;
  ELSE
    SET cbuscar=CONCAT('%',TRIM(abuscar),'%');
    SELECT idart,descri,unid,uno,dos,tre,cua,cero,
    premay  AS pre1,
    ...
    prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,prod_idau,
    ulfc,prod_nume,prod_tipo,prod_dct1,prod_dct2,prod_dct3,prod_codp,'n' AS modi,uldc,prod_icbper
    FROM fe_art  AS a WHERE prod_coda LIKE cbuscar AND prod_acti<>'I' ORDER BY prod_des1,prod_coda,prod_nume;
END IF;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProMuestratallas` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProMuestratallas` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProMuestratallas`()
BEGIN
  SELECT a.tall_tall,tall_idta from fe_talla as a  ORDER BY tall_tall;
END */$$
DELIMITER ;
...
/*!50001 CREATE TABLE  `vguiasdevolucion`(
 ...
 `guia_arch` varchar(30) ,
 `email` varchar(45) ,
 `guia_hash` varchar(15) ,
 `guia_feen` datetime ,
...
/*!50001 CREATE TABLE  `vlistaprecios`(
 ...
 `tmon` varchar(1) ,
 `idflete` int ,
 `prod_uti1` decimal(9,6) ,
 `prod_uti2` decimal(9,6) ,
 `prod_uti3` decimal(9,6) ,
 `prod_come` float ,
 `prod_comc` float ,
 `ulpc` int ,
 `prod_idus` int ,
...
/*!50001 CREATE TABLE  `vmuestracompras`(
 `idauto` int ,
 `kar_prel` decimal(12,6) ,
 `kar_dct1` decimal(6,2) ,
 `kar_dct2` decimal(6,2) ,
 `kar_dct3` decimal(6,2) ,
 `alma` int ,
 `idkar` int ,
 `descri` varchar(100) ,
 `peso` float ,
 `prod_coda` varchar(15) ,
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `rvendedores` AS select `a`.`idauto` AS `idauto`,max(`a`.`codv`) AS `codv` from `fe_kar` `a` where ((`a`.`acti` = 'A') and (`a`.`tipo` = 'V')) group by `a`.`idauto` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vmuestracompras` AS select `a`.`idauto` AS `idauto`,`a`.`kar_prel` AS `kar_prel`,`a`.`kar_dct1` AS `kar_dct1`,`a`.`kar_dct2` AS `kar_dct2`,`a`.`kar_dct3` AS `kar_dct3`,`a`.`alma` AS `alma`,`a`.`idkar` AS `idkar`,`b`.`descri` AS `descri`,`b`.`peso` AS `peso`,`b`.`prod_coda` AS `prod_coda`,`b`.`prod_idco` AS `prod_idco`,`b`.`unid` AS `unid`,`b`.`tipro` AS `tipro`,`a`.`idart` AS `idart`,`a`.`incl` AS `incl`,`c`.`ndoc` AS `ndoc`,`c`.`valor` AS `valor`,`c`.`igv` AS `igv`,`c`.`impo` AS `impo`,`c`.`pimpo` AS `pimpo`,`a`.`cant` AS `cant`,`a`.`prec` AS `prec`,`c`.`fech` AS `fech`,`c`.`fecr` AS `fecr`,`c`.`form` AS `form`,`c`.`exon` AS `exon`,`c`.`ndo2` AS `ndo2`,`c`.`vigv` AS `vigv`,`c`.`idprov` AS `idprov`,`a`.`tipo` AS `tipo`,`c`.`tdoc` AS `tdoc`,`c`.`dolar` AS `dolar`,`c`.`mone` AS `mone`,`p`.`razo` AS `razo`,`p`.`dire` AS `dire`,`p`.`ciud` AS `ciud`,`p`.`nruc` AS `nruc`,`c`.`codt` AS `codt`,`a`.`dsnc` AS `dsnc`,`a`.`dsnd` AS `dsnd`,`a`.`gast` AS `gast`,`c`.`fusua` AS `fusua`,`c`.`idusua` AS `idusua`,`w`.`nomb` AS `Usuario`,`c`.`rcom_otro` AS `rcom_otro` from ((((`fe_rcom` `c` left join `fe_kar` `a` on((`c`.`idauto` = `a`.`idauto`))) left join `fe_art` `b` on((`b`.`idart` = `a`.`idart`))) join `fe_prov` `p` on((`p`.`idprov` = `c`.`idprov`))) join `fe_usua` `w` on((`w`.`idusua` = `c`.`idusua`))) where ((`c`.`acti` <> 'I') and (`a`.`acti` <> 'I')) */;
