...
MySQL - 8.0.45 : Database - eduar_bdcesar
*********************************************************************
*/
...
nidus INTEGER,cpc VARCHAR(45),cdni VARCHAR(10),cubigeo VARCHAR(10),distrito VARCHAR(100),provincia VARCHAR(100),dpto VARCHAR(100)) RETURNS int
BEGIN
DECLARE nid INTEGER DEFAULT 0;
INSERT INTO fe_prov(nruc,razo,dire,ciud,fono,fax,prov_rpm,email,refe,celu,prov_idus,idpcprov,fechprov,ndni,prov_ubig,prov_dist,prov_prov,prov_dpto)
VALUES (cruc,crazo,cdire,cciud,cfono,cfax,crpm,correo,crefe,ccelu,nidus,cpc,LOCALTIME,cdni,cubigeo,distrito,provincia,dpto);
...
nidcta5 INTEGER,nt5 DECIMAL(12,2),nidcta6 INTEGER,nt6 DECIMAL(12,2)) RETURNS int
BEGIN
DECLARE nid1,nmes INTEGER;
DECLARE cauto VARCHAR(20);
DECLARE nimpo1,nimpo2,nimpo3,ndd,nddd,nimpo4,nimpo5,nimpo6 DECIMAL(12,2);
SET nid1=0;
SET nddd=0;
SET ndd=0;
...
SET nimpo6=IF(cm='D',nt6*nddd,nt6);
CALL ProIngresaCuentasV(nimpo1,nimpo2,nimpo3,nidcta1,nidcta2,nidcta3,nid1,nimpo4,nidcta4,nimpo5,nidcta5,nimpo6,nidcta6);
RETURN nid1;
END */$$
DELIMITER ;
...
/*!50003 CREATE FUNCTION `FunTraspasoDatosLcajaE`(dfecha DATE,cndoc VARCHAR(12),cdeta VARCHAR(100),idcta INTEGER,sdeudor DECIMAL(12,2),
sacreedor DECIMAL(12,2),cmone CHAR,ndolar DECIMAL(5,3),nidus INTEGER,nidcp INTEGER,nidalma INTEGER) RETURNS int
BEGIN
DECLARE id INTEGER;
INSERT INTO fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,lcaj_idus,lcaj_clpr,
lcaj_tran,lcaj_codt,lcaj_fope)VALUES
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidcp,'T',nidalma,localtime);
SELECT LAST_INSERT_ID() INTO id FROM fe_lcaja GROUP BY LAST_INSERT_ID();
RETURN id;
END */$$
DELIMITER ;
...
cdeta VARCHAR(120),nidus INTEGER,ndolar DECIMAL(6,3),cndoc VARCHAR(20),idcta INTEGER,debe DECIMAL(12,2),
haber DECIMAL(12,2),norden INTEGER,nidcaja INTEGER) RETURNS int
BEGIN
DECLARE id INTEGER;
INSERT INTO fe_cbancos(cban_idba,cban_nume,cban_fech,cban_idmp,cban_deta,cban_ndoc,cban_idct,
cban_debe,cban_haber,cban_orde,cban_idca,cban_fope,cban_dola)
VALUES(idb,nop,dfecha,idmp,cdeta,cndoc,idcta,debe,haber,norden,nidcaja,LOCALTIME,ndolar);
SELECT LAST_INSERT_ID() INTO id FROM fe_cbancos GROUP BY LAST_INSERT_ID();
RETURN id;
END */$$
DELIMITER ;
...
nv5 DECIMAL(12,2),nid5 INTEGER,idv5 INTEGER,ct5 CHAR,nv6 DECIMAL(12,2),nid6 INTEGER,idv6 INTEGER,ct6 CHAR)
BEGIN
...
UPDATE fe_ectas SET impo=nv6,idcta=nid6,tipo=ct6 WHERE idectas=idv6;
END */$$
DELIMITER ;
...
cemail VARCHAR(45),nidus INTEGER,ccelu VARCHAR(15),crefe VARCHAR(200),crpm VARCHAR(10),cdni VARCHAR(10),
cubigeo VARCHAR(10),distrito VARCHAR(100),provincia VARCHAR(100),dpto VARCHAR(100))
BEGIN
UPDATE fe_prov SET
...
prov_dist=distrito,prov_prov=provincia,prov_dpto=dpto WHERE idprov=nid;
END */$$
DELIMITER ;
...
nid INTEGER,tper DECIMAL(12,2),ctaper INTEGER,nv5 DECIMAL(12,2),nid5 INTEGER,nv6 DECIMAL(12,2),nid6 INTEGER)
BEGIN
INSERT INTO fe_ectas(idrven,impo,idcta,nitem,tipo)
VALUES(nid,nv1,nid1,1,"H");
INSERT INTO fe_ectas(idrven,impo,idcta,nitem,tipo)
