...
MySQL - 8.0.37 : Database - sysven_bdopari
*********************************************************************
*/
...
BEGIN
SELECT nomb,idalma,dire,ciud,sucuidserie FROM fe_sucu where idalma in(1) order by idalma;
END */$$
DELIMITER ;
...
if opt=1 then
  ...
  inner join fe_cat as c on(c.idcat=a.idcat) 
  left join fe_rcom as yy on (yy.idauto=a.prod_idau) 
  left join fe_prov as o on (o.idprov=a.ulpc)
  WHERE descri LIKE cbuscar and prod_acti<>'I' ORDER BY DESCRI;
else
  set cbuscar=concat('%',trim(abuscar),+'%');
  SELECT idart,descri,unid,prod_unid1,
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vpdtespagoc` AS select `b`.`rcre_idcl` AS `idclie`,`a`.`ndoc` AS `ndoc`,`s`.`saldo` AS `importe`,`a`.`mone` AS `mone`,`a`.`banc` AS `banc`,`b`.`rcre_fech` AS `fech`,`x`.`razo` AS `razo`,`x`.`fono` AS `fono`,`x`.`dire` AS `dire`,`x`.`ciud` AS `ciud`,`s`.`fevto` AS `fevto`,`a`.`tipo` AS `tipo`,`a`.`dola` AS `dola`,ifnull(`c`.`ndoc`,'') AS `docd`,`a`.`nrou` AS `nrou`,`a`.`banco` AS `banco`,`a`.`idcred` AS `idcred`,`b`.`rcre_idau` AS `idauto`,`d`.`nomv` AS `nomv`,`a`.`ncontrol` AS `ncontrol`,`b`.`rcre_codv` AS `rcre_codv`,`a`.`estd` AS `estd` from (((((`vpdtesventas` `s` join `fe_clie` `x` on((`x`.`idclie` = `s`.`rcre_idcl`))) join `fe_cred` `a` on((`a`.`idcred` = `s`.`ncontrol`))) join `fe_rcred` `b` on((`b`.`rcre_idrc` = `a`.`cred_idrc`))) left join `fe_rcom` `c` on((`c`.`idauto` = `b`.`rcre_idau`))) join `fe_vend` `d` on((`d`.`idven` = `b`.`rcre_codv`))) order by `s`.`fevto`,`c`.`ndoc` */;
