...
MySQL - 8.0.37 : Database - sysven_bdpacodi
*********************************************************************
*/
...
ncome float,ncomc float,nutil1 float,nutil2 float,nutil3 float,nidusua integer,nsmin float,nsmax float,tigv decimal(6,2),ndolar float) RETURNS int
BEGIN
declare nid integer default 0;
INSERT INTO fe_art(descri,unid,prec,cost,premay,premen,pre3,peso,idcat,idmar,tipro,idflete,tmon,fechc,idpc,prod_come,
prod_comc,prod_uti1,prod_uti2,prod_uti3,prod_idus,prod_smin,prod_smax,prod_tigv,prod_dola)
VALUES (cdesc,cunid,nprec,ncosto,np1,np2,np3,npeso,ccat,cmar,ctipro,nflete,cm,localtime,
cidpc,ncome,ncomc,nutil1,nutil2,nutil3,nidusua,nsmin,nsmax,tigv,ndolar);
...
/*!50003 CREATE FUNCTION `FunIngresaDPedidos`(ncoda integer,ncant decimal(12,2),nprec decimal(12,4),
nidauto integer,nflete decimal(8,2)) RETURNS int
BEGIN
declare id integer default 0;
INSERT INTO fe_ped(idart,cant,prec,idautop,pedi_flet)
VALUES(ncoda,ncant,nprec,nidauto,nflete);
select last_insert_id() into id from fe_ped group by last_insert_id();
return id;
END */$$
DELIMITER ;
...
nr integer,ctipoa char,nflete decimal(8,2))
BEGIN
if ctipoa='A' then
     UPDATE fe_ped SET acti='I' WHERE idped=nr;
   else
    UPDATE fe_ped SET idart=ncoda,cant=ncant,prec=nprec,pedi_flet=nflete WHERE idped=nr;
end if;
END */$$
DELIMITER ;
...
ncomc float,nidus integer,ncoda integer,nsmin float,nsmax float,tigv decimal(6,2),ndolar float,cestado char)
BEGIN
UPDATE fe_art SET descri=cdesc,unid=cunid,cost=ncosto,premay=np1,premen=np2,
pre3=np3,peso=npeso,idcat=ccat,idmar=cmar,tipro=ctipro,idflete=nflete,tmon=cm,
...
begin
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
lcaj_idus,lcaj_clpr,lcaj_idau,lcaj_form,lcaj_fope,lcaj_dcto,lcaj_tdoc,lcaj_codt,lcaj_fope)values
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidcp,nidauto,cform,localtime,cdcto,ctdoc,1,localtime);
end */$$
DELIMITER ;
...
nprecio decimal(14,6),ncant decimal(10,2),cunidad varchar(15))
begin
...
if opt=1 then
   ...
   prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,ifnull(o.razo,'') as proveedor,
   ifnull(yy.ndoc,'') as ndoc,ifnull(yy.fech,'') as fech,ulfc,s.dmar,prod_acti,prod_tigv,
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vpdtesentrega` AS select `p`.`pdte_idar` AS `idart`,sum(`p`.`Pedido`) AS `Pedido`,sum(`p`.`Entregado`) AS `Entregado`,(sum(`p`.`Pedido`) - sum(`p`.`Entregado`)) AS `saldo`,`p`.`idin` AS `idin`,`p`.`pdte_idau` AS `pdte_idau` from `vsaldos` `p` group by `p`.`idin`,`p`.`pdte_idar`,`p`.`pdte_idau` having (sum((`p`.`Pedido` - `p`.`Entregado`)) > 0) */;
