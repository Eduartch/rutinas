...
MySQL - 8.0.41 : Database - sysven_bddistrisym
*********************************************************************
*/
...
WHEN TRIM(ct)="Transportistas" THEN
     IF nid=0 THEN
         SELECT idtra INTO vdvto FROM fe_tra WHERE TRIM(razon)=cb AND tran_acti<>'I';
     ELSE
   	    SELECT iodtra INTO vdvto FROM fe_tra WHERE TRIM(razon)=cb AND idtra>nid AND tran_acti<>'I';
     END IF;     
end case;
if vdvto>0 then
   return 0;
...
crefe varchar(255),linea float,crpm varchar(10),nidz integer,nids integer ) RETURNS int
BEGIN
declare nid integer default 0;
INSERT INTO sysven_bdsafety.fe_clie(nruc,razo,dire,ciud,fono,fax,ndni,clie_tipo,clie_corr,clie_codv,clie_idus,idpcclie,
fechclie,celu,refe,clie_lcre,clie_rpm,clie_idzo,clie_idse)
VALUES (cruc,crazo,cdire,cciud,cfono,cfax,cdni,ctipo,cemail,nidven,nidus,cpc,LOCALTIME,ccelu,crefe,linea,crpm,nidz,nids);
...
BEGIN
declare nid integer default 0;
INSERT INTO sysven_bdsafety.fe_mar(dmar,fechcm,marc_idus,idpcm)VALUES (cdescri,LOCALTIME(),nidus,cidpc);
...
prod_comc,prod_uti1,prod_uti2,prod_uti3,prod_idus,prod_smin,prod_smax,prod_cod1,prod_dola,prod_enla)
VALUES (cdesc,cunid,nprec,ncosto,np1,np2,np3,npeso,ccat,cmar,ctipro,nflete,cm,localtime,
cidpc,ncome,ncomc,nutil1,nutil2,nutil3,nidusua,nsmin,nsmax,ccodigo1,ndolar,nid1);
...
BEGIN
declare nid integer default 0;
INSERT INTO sysven_bdsafety.fe_prov(nruc,razo,dire,ciud,fono,fax,prov_rpm,email,refe,celu,prov_idus,idpcprov,fechprov)
...
/* Function  structure for function  `FunIngresaDatosLcajaE13` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaDatosLcajaE13` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaDatosLcajaE13`(dfecha datetime,cndoc varchar(10),cdeta varchar(100),idcta integer,sdeudor decimal(12,2),
sacreedor decimal(12,2),cmone char,ndolar decimal(5,3),nidus integer,nidtda integer,ctipogasto char ) RETURNS int
begin
declare id integer;
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
lcaj_idus,lcaj_codt,lcaj_fope,lcaj_tgas)values
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidtda,localtime,ctipogasto);
select last_insert_id() into id from fe_lcaja group by last_insert_id();
return id;
end */$$
DELIMITER ;
...
nhaber decimal(12,2),cglosa varchar(180),ct char(1),cnume varchar(12),nidcta integer,ccond char,nit integer,ncomp varchar(15),
...
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 decimal(10,4),vcom float) RETURNS int
BEGIN
declare nidk integer default 0;
if ct='C' then
  INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_idco,codv,kar_alma1)
  VALUES (nid,cc,ct,npr,nct,tmvto,cincl,calma,0,ccodv,calma);
 else
...
declare sw integer default 0;
set nid1=(select pdte_idin from fe_ipdtes where pdte_idau=nid and pdte_acti='A' group by pdte_idin);
if nid1>0 then
...
/*!50003 CREATE FUNCTION `FunCreaCtasBancos`(cta varchar(100),idb1 integer,cmone char,
cdeta varchar(100),nidctap integer,ncodt integer) RETURNS int
BEGIN
declare idb integer;
insert into fe_ctasb(ctas_ctas,ctas_idba,ctas_mone,ctas_deta,ctas_ncta,ctas_codt)
values(cta,idb1,cmone,cdeta,nidctap,ncodt);
...
nidpr integer,cmone char,ctran char,nimtd decimal (12,2),nimth decimal(12,2),ncodt integer) RETURNS int
BEGIN
declare iddiario integer default 0;
insert into fe_ldiario(ldia_fech,ldia_debe,ldia_haber,ldia_glosa,ldia_tipo,
ldia_nume,ldia_idcta,ldia_cond,ldia_item,ldia_comp,ldia_idcv,ldia_idcc,ldia_mone,ldia_tran,ldia_itrd,ldia_itrh,ldia_codt)
values(dfech,ndebe,nhaber,cglosa,ct,cnume,nidcta,ccond,nit,ncomp,nidcl,nidpr,cmone,ctran,nimtd,nimth,ncodt);
select last_insert_id() into iddiario from fe_ldiario group by last_insert_id();
return iddiario;
END */$$
DELIMITER ;
...
cdire varchar(100),cciud varchar(100),cfono varchar(15),cfax varchar(15),cdni varchar (11),
ctipo char,cemail varchar(45),nidven integer,nidus integer,ccelu varchar(15),crefe varchar(255),linea float,crpm varchar(10),
nidz integer,nids integer)
BEGIN
update fe_clie set
nruc=cruc,razo=crazo,dire=cdire,ciud=cciud,fono=cfono,fax=cfax,ndni=cdni,clie_tipo=ctipo,clie_corr=cemail,
clie_codv=nidven,clie_actu=nidus,clie_feac=localtime,
celu=ccelu,refe=crefe,clie_lcre=linea,clie_rpm=crpm,clie_idzo=nidz,clie_idse=nids where idclie=nid;
END */$$
DELIMITER ;
...
if opt=0 then
   ...
   ldia_idcta=nidcta,ldia_cond=ccond,ldia_tran=ctran,ldia_inic='I',
...
/*!50003 CREATE PROCEDURE `ProEditaAlmacen`(cnomb varchar(50),cdire varchar(100),cciud varchar(100),nser integer,nidus integer,nid integer)
begin
...
when opt=0 then
   ...
   clie_feac,ifnull(c.zona_nomb,'') as Zona,ifnull(a.clie_idzo,0) as clie_idzo,clie_idse,ifnull(u.nomb,'') as usuacrea,
   ifnull(uu.nomb,'') as usuamodi
   from fe_clie as a 
   left JOIN fe_usua  AS u ON u.idusua=a.clie_idus
   left JOIN fe_usua AS uu  ON uu.idusua=a.clie_actu
   left join fe_zona as c on c.zona_idzo=a.clie_idzo where  clie_acti<>'I' and razo like cbuscar order by razo;
when opt=1 then
   select idclie,nruc,razo,ndni,dire,ciud,ifnull(fono,'0') as fono,ifnull(fax,'0') as fax,
   ifnull(celu,'0') as celu,refe,clie_tipo,clie_codv,clie_lcre,clie_corr,clie_rpm,clie_idus,clie_actu,fechclie,
   clie_feac,ifnull(c.zona_nomb,'') as Zona,ifnull(a.clie_idzo,0) as clie_idzo,clie_idse,IFNULL(u.nomb,'') AS usuacrea,
   ...
   left JOIN fe_usua  AS u ON u.idusua=a.clie_idus
   left JOIN fe_usua AS uu  ON uu.idusua=a.clie_actu
   left join fe_zona as c on c.zona_idzo=a.clie_idzo where  clie_acti<>'I' and nruc like cbuscar order by razo;
when opt=2 then
   select idclie,nruc,razo,ndni,dire,ciud,ifnull(fono,'0') as fono,ifnull(fax,'0') as fax,
   ifnull(celu,'0') as celu,refe,clie_tipo,clie_codv,clie_lcre,clie_corr,clie_rpm,clie_idus,clie_actu,fechclie,
   clie_feac,ifnull(c.zona_nomb,'') as Zona,ifnull(a.clie_idzo,0) as clie_idzo,clie_idse,IFNULL(u.nomb,'') AS usuacrea,
   ...
   left JOIN fe_usua  AS u ON u.idusua=a.clie_idus
   left JOIN fe_usua AS uu  ON uu.idusua=a.clie_actu
   left join fe_zona as c on c.zona_idzo=a.clie_idzo where  clie_acti<>'I' and  ndni like cbuscar order by razo;
when  opt=3 then
   select idclie,nruc,razo,ndni,dire,ciud,ifnull(fono,'0') as fono,ifnull(fax,'0') as fax,
   ifnull(celu,'0') as celu,refe,clie_tipo,clie_codv,clie_lcre,clie_corr,clie_rpm,clie_idus,clie_actu,fechclie,
   clie_feac,ifnull(c.zona_nomb,'') as Zona,ifnull(a.clie_idzo,0) as clie_idzo,clie_idse,u.nomb AS usuacrea,IFNULL(u.nomb,'') AS usuacrea,
   IFNULL(uu.nomb,'') AS usuamodi
   from fe_clie as a 
   LEFT JOIN fe_usua  AS u ON u.idusua=a.clie_idus
   LEFT JOIN fe_usua AS uu  ON uu.idusua=a.clie_actu
   left join fe_zona as c on c.zona_idzo=a.clie_idzo where  clie_acti<>'I' and idclie =nid order by razo;
when opt=4 then
   select idclie,nruc,razo,ndni,dire,ciud,ifnull(fono,'0') as fono,ifnull(fax,'0') as fax,
   ifnull(celu,'0') as celu,refe,clie_tipo,clie_codv,clie_lcre,clie_corr,clie_rpm,clie_idus,clie_actu,fechclie,
   clie_feac,ifnull(c.zona_nomb,'') as Zona,ifnull(a.clie_idzo,0) as clie_idzo,clie_idse,IFNULL(u.nomb,'') AS usuacrea,
   ...
   left join fe_usua  as u on u.idusua=a.clie_idus
   left join fe_usua as uu  on uu.idusua=a.clie_actu
   left join fe_zona as c on c.zona_idzo=a.clie_idzo where  clie_acti<>'I' and ciud like cbuscar order by razo;
end case;
END */$$
DELIMITER ;
...
BEGIN
select ncta,nomb,idcta,cdestinod,cdestinoh,tipocta,plan_oper from fe_plan where idcta=nid;
END */$$
DELIMITER ;
...
if opt=1 then
    set cbuscar=concat('%',trim(abuscar),+'%');
    SELECT idart,descri,unid,uno,dos,tre,cua,cero,
    if(prod_uti1>1,round(prod_uti1*round(if(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*if(prod_dola>nd,prod_dola,nd))+b.prec),2),2),cast(0 as decimal(10,2))) as pre1,
    if(prod_uti2>1,round(prod_uti2*round(if(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*if(prod_dola>nd,prod_dola,nd))+b.prec),2),2),CAST(0 AS DECIMAL(10,2))) as pre2,
    if(prod_uti3>1,round(prod_uti3*round(if(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*if(prod_dola>nd,prod_dola,nd))+b.prec),2),2),CAST(0 AS DECIMAL(10,2))) as pre3,
    ...
    prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,ifnull(o.razo,'') as proveedor,
    ifnull(y.ndoc,'') as ndoc,ifnull(y.fech,'') as fech,ulfc,prod_ent1,prod_ent2,prod_icbper,g.desgrupo as grupo,b.desflete,
...
BEGIN
SELECT * FROM fe_sucu  where idalma in(1,2,3) order by idalma;
END */$$
DELIMITER ;
...
begin
 DECLARE ncodt INTEGER DEFAULT 1;
 CASE   
 WHEN SUBSTR(cndoc,4,1)='4' THEN
    SET ncodt=3;
 WHEN   SUBSTR(cndoc,4,1)='3' THEN 
    SET ncodt=2;
ELSE 
   SET ncodt=1; 
 END CASE;   
...
begin
 ...
 values(@nc,dfecha,dfecha,cndoc,ctdoc,'V','','','K','E','S','N',localtime,nidus,0,ncodt);
 SELECT @na:=LAST_INSERT_ID() FROM fe_rcom group by LAST_INSERT_ID();
Call ProIngresaDatosLcajaEefectivo(dfecha,"","*** ANULADA ***",0,0,0,'S',0,nidus,0,@na,'E',cndoc,ctdoc);
END */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProIngresaDatosLibroDiarioPle55`(dfech date,ndebe decimal(12,2),
nhaber decimal(12,2),cglosa varchar(180),ct char(1),cnume varchar(12),nidcta integer,ccond char,nit integer,ncomp varchar(15),
nidcl integer,nidpr integer,cmone char,ctran char,nimtd decimal (12,2),nimth decimal(12,2),ctdoc varchar(2),ncodt integer)
BEGIN
insert into fe_ldiario(ldia_fech,ldia_debe,ldia_haber,ldia_glosa,ldia_tipo,
ldia_nume,ldia_idcta,ldia_cond,ldia_item,ldia_comp,ldia_idcv,ldia_idcc,ldia_mone,ldia_tran,ldia_itrd,ldia_itrh,ldia_tdoc,ldia_codt)
values(dfech,ndebe,nhaber,cglosa,ct,cnume,nidcta,ccond,nit,ncomp,nidcl,nidpr,cmone,ctran,nimtd,nimth,ctdoc,ncodt);
END */$$
DELIMITER ;
...
opt integer,ccond char,nit integer,ncomp varchar(15),nidcl integer,nidpr integer,cmone char,ctran char,ncodt integer)
BEGIN
if opt=0 then
   update fe_ldiario set ldia_acti='I' where ldia_idld=idd;
 else
   ...
   ldia_item=nit,ldia_comp=ncomp,ldia_idcv=nidcl,ldia_idcc=nidpr,ldia_mone=cmone,ldia_codt=ncodt where ldia_idld=idd;
end if;
END */$$
DELIMITER ;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vpdtesx` AS select sum(ifnull(`f`.`entr_cant`,0)) AS `entregado`,(`b`.`cant` - sum(ifnull(`f`.`entr_cant`,0))) AS `saldo`,`a`.`idauto` AS `idauto`,`b`.`idkar` AS `idkar`,`b`.`idart` AS `idart` from ((((select `fe_kar`.`idauto` AS `idauto`,`fe_kar`.`idkar` AS `idkar`,`fe_kar`.`idart` AS `idart`,`fe_kar`.`cant` AS `cant` from `fe_kar` where (`fe_kar`.`acti` = 'A')) `b` join `fe_rcom` `a` on((`a`.`idauto` = `b`.`idauto`))) left join `fe_ent` `f` on((`f`.`entr_idkar` = `b`.`idkar`))) left join `fe_guias` `w` on((`w`.`guia_idgui` = `f`.`entr_idgu`))) where (((`a`.`acti` = 'A') and (`a`.`idcliente` > 0)) or (`f`.`entr_acti` = 'A') or (`f`.`entr_acti` is null)) group by `b`.`idkar`,`a`.`idauto`,`b`.`idart` */;
