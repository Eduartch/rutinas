...
MySQL - 8.0.41 : Database - sysven_bdactiva
*********************************************************************
*/
...
/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `AnulaPagosCajal` */$$

/*!50003 CREATE */ /*!50003 TRIGGER `AnulaPagosCajal` AFTER UPDATE ON `fe_lcaja` FOR EACH ROW begin
...
if opt=0 then
   ...
   update fe_rcom set form=cform where idauto=nidauto;
end if;
END */$$
DELIMITER ;
...
DECLARE idve INTEGER;
   ...
   rcom_exon=nexonerado,rcom_mdet=ndetraccion,rcom_detr=cdetra   WHERE idauto=nidauto;
   IF n1>0 AND n2>0 AND n3>0 THEN
      SELECT @i1:=SUM(xx.nid1),@i2:=SUM(xx.nid2),@i3:=SUM(xx.nid3)
      FROM (SELECT CASE a.nitem WHEN 1 THEN idectas ELSE 0 END AS nid1,
      CASE a.nitem WHEN 2 THEN idectas ELSE 0 END AS nid2,
