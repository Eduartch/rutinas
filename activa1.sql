...
MySQL - 8.0.43 : Database - sysven_bdactiva
*********************************************************************
*/
...
/* Trigger structure for table `fe_caja1` */

DELIMITER $$

/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `ActualizaCajaChica` */$$

/*!50003 CREATE */ /*!50003 TRIGGER `ActualizaCajaChica` AFTER UPDATE ON `fe_caja1` FOR EACH ROW begin
insert into fe_alogcajach(logc_idca,logc_fope)values(old.caja_idca,localtime);
end */$$
...
/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `ActualizaBancosConPagos` */$$

/*!50003 CREATE */ /*!50003 TRIGGER `ActualizaBancosConPagos` AFTER UPDATE ON `fe_cbancos` FOR EACH ROW begin
...
/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `Anuladetalleguia` */$$

/*!50003 CREATE */ /*!50003 TRIGGER `Anuladetalleguia` AFTER UPDATE ON `fe_guias` FOR EACH ROW BEGIN
...
if new.acti='I' then
    ...
    logk_ida1,logk_idk1,logk_fech,logk_idco)values('Modificado ',old.idart,old.cant,old.prec,old.idauto,
...
/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `AnulaPagosCaja1` */$$

/*!50003 CREATE */ /*!50003 TRIGGER `AnulaPagosCaja1` AFTER UPDATE ON `fe_lcaja` FOR EACH ROW BEGIN
...
/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `AnulaPagosDiario` */$$

/*!50003 CREATE */ /*!50003 TRIGGER `AnulaPagosDiario` AFTER UPDATE ON `fe_ldiario` FOR EACH ROW begin
...
if new.acti='I' then
   ...
   IF old.tdoc='09' THEN
      UPDATE fe_guias SET guia_acti='I' WHERE guia_idau=old.idauto;
   END IF;
...
/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `ActivaKardex` */$$

/*!50003 CREATE */ /*!50003 TRIGGER `ActivaKardex` AFTER UPDATE ON `fe_rcom` FOR EACH ROW BEGIN
...
/* Trigger structure for table `fe_rcomu` */

DELIMITER $$

/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `AnulaCajaChicaUx` */$$

/*!50003 CREATE */ /*!50003 TRIGGER `AnulaCajaChicaUx` AFTER UPDATE ON `fe_rcomu` FOR EACH ROW begin
if new.rcom_acti='I' then
    update fe_caja1 set caja_Acti='I' where caja_idau=old.rcom_idau;
end if;
end */$$
...
/*!50003 CREATE FUNCTION `FunCreaActivos`(cdesc varchar(180),nvalor decimal(12,2),cdeta varchar(150)) RETURNS int
BEGIN
...
declare ctipo char;
set nid=0;
    INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,rcom_otro,ndo2,mone,dolar,vigv,tcom,idcliente,tipom,fusua,idusua,codt,pimpo)
   VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,cndo2,cm,ndolar,ni,ctg,ccodp,cmvto,localtime,nus,nidcodt,idtr);
   select last_insert_id() into nid from fe_rcom group by last_insert_id();
return nid;
END */$$
DELIMITER ;
...
/* Function  structure for function  `FunIngresaCabeceraReclamos` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaCabeceraReclamos` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaCabeceraReclamos`(
ctdoc varchar(2),cform char,cndoc varchar(10),dfecha date,dfechar date,cdetalle varchar(200),
nv decimal(12,2),nigv decimal(12,2),nt decimal(12,2),cndo2 varchar(10),cm char,
ndolar float,ni float,ctg char,ccodp integer,cmvto char,nus integer,opt integer,nidcodt integer,
n1 integer,n2 integer,n3 integer,nid1 integer,alma1 integer) RETURNS int
BEGIN
declare nid integer;
INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,tipom,fusua,idusua,codt,alma,idautop,idcliente)
VALUES (ctdoc,cform,cndoc,dfecha,dfechar,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,cmvto,localtime,nus,nidcodt,alma1,nid1,ccodp);
select last_insert_id() into nid from fe_rcom group by last_insert_id();
return nid;
END */$$
DELIMITER ;
...
na integer,dfecha date,nt1 float,cmvtoc char,cform char,cm1 char,cndoc varchar(12),nidcon integer,
cu integer,cdetalle varchar(120),cor varchar(2),nimp1 float,cm2 char,tcvta float,nidcodt integer,cajas char,nidcredito int,ide integer,fp char) RETURNS int
BEGIN
declare nid integer;
set nid=0;
if cajas='A' then
   INSERT INTO fe_caja(idauto,fech,caja_nimp,tipo,forma,tmon,ndoc,idcon,idusua,fechao,deta,origen,nimpo,mone,dola,codt,idcred,caja_idem.caja_form)
   VALUES (na,dfecha,nt1,cmvtoc,cform,cm1,cndoc,nidcon,cu,localtime,cdetalle,cor,nimp1,cm2,tcvta,nidcodt,nidcredito,ide,fp);
 else
   INSERT INTO fe_caja(idauto,fech,impo,tipo,forma,tmon,ndoc,idcon,idusua,fechao,deta,origen,nimpo,mone,dola,codt,idcred,caja_idem,caja_form)
   VALUES (na,dfecha,nt1,cmvtoc,cform,cm1,cndoc,nidcon,cu,localtime,cdetalle,cor,nimp1,cm2,tcvta,nidcodt,nidcredito,ide,fp);
end if;
select last_insert_id() into nid from fe_caja group by last_insert_id();
return nid;
...
/* Function  structure for function  `FunIngresaCajaCambio` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaCajaCambio` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaCajaCambio`(
na integer,dfecha date,nt1 float,cmvtoc char,cform char,cm1 char,cndoc varchar(10),nidcon integer,
cu integer,cdetalle varchar(120),cor varchar(2),nimp1 float,cm2 char,tcvta float,nidcodt integer,
cajas char,nidcredito int,ncambio decimal(12,2)) RETURNS int
BEGIN
declare nid integer;
set nid=0;
INSERT INTO fe_caja(idauto,fech,impo,tipo,forma,tmon,ndoc,idcon,idusua,fechao,deta,origen,nimpo,mone,dola,codt,idcred,anula)
VALUES (na,dfecha,nt1,cmvtoc,cform,cm1,cndoc,nidcon,cu,localtime,cdetalle,cor,nimp1,cm2,tcvta,nidcodt,nidcredito,ncambio);
select last_insert_id() into nid from fe_caja group by last_insert_id();
return nid;
END */$$
DELIMITER ;
...
na integer,dfecha date,nt1 decimal(12,2),cmvtoc char,cform char,cm1 char,cndoc varchar(10),nidcon integer,
cu integer,cdetalle varchar(120),cor varchar(2),nimp1 float,cm2 char,tcvta float,nidcodt integer,cajas char,nidcredito int,ide integer) RETURNS int
BEGIN
declare nid integer;
set nid=0;
...
/* Function  structure for function  `FunIngresaCajaTarjetas` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaCajaTarjetas` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaCajaTarjetas`(
na integer,dfecha date,nt1 decimal(12,2),cmvtoc char,cform char,cm1 char,cndoc varchar(12),nidcon integer,
cu integer,cdetalle varchar(120),cor varchar(2),nimp1 decimal(12,2),cm2 char,tcvta float,nidcodt integer,
cajas char,nidcredito int,ide integer,ntarjetas decimal(12,2),ctipot char,btarjeta varchar(15),creferencia varchar(10)) RETURNS int
BEGIN
declare nid integer;
set nid=0;
INSERT INTO fe_caja(idauto,fech,impo,tipo,forma,tmon,ndoc,idcon,idusua,fechao,deta,origen,nimpo,mone,dola,codt,idcred,
caja_tarj,caja_ttar,caja_refe,caja_btar)
VALUES (na,dfecha,nt1,cmvtoc,cform,cm1,cndoc,nidcon,cu,localtime,cdetalle,cor,nimp1,cm2,tcvta,nidcodt,nidcredito,ntarjetas,
ctipot,creferencia,btarjeta);
select last_insert_id() into nid from fe_caja group by last_insert_id();
return nid;
END */$$
DELIMITER ;
...
n1 integer,n2 integer,n3 integer,nretencion decimal(8,2),idtr decimal(10,2),nexon decimal(12,2),ndscto decimal(12,2)) RETURNS int
BEGIN
...
/*!50003 CREATE FUNCTION `FunIngresaKardex2`(nid integer,cc varchar(15),ct char,npr float,
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 integer,nalma integer,codi integer) RETURNS int
BEGIN
declare nidk integer default 0;
if ct='C' then
  INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_idco,codv,kar_alma1,kar_codi)
  VALUES (nid,cc,ct,npr,nct,tmvto,cincl,calma,nidcosto1,ccodv,nalma,codi);
 else
  INSERT INTO fe_kar(idauto,idart,tipo,prec,cant,ttip,incl,alma,kar_idco,codv,kar_alma1,kar_codi)
  VALUES (nid,cc,ct,npr,nct,tmvto,cincl,calma,nidcosto1,ccodv,nalma,codi);
end if;
...
/*!50003 CREATE FUNCTION `FunIngresaKardexGratuito`(nid integer,cc varchar(15),ct char,npr float,
nct float,cincl char,tmvto char,ccodv integer,calma integer,nidcosto1 decimal(12,2),vcom float) RETURNS int
BEGIN
declare nidk integer default 0;
...
BEGIN
declare id integer default 0;
insert into fe_ncven(ncre_idan,ncre_idau,ncre_idcr)values(nid0,nid1,nidpagos);
select last_insert_id() into id from fe_ncven group by last_insert_id();
return id;
END */$$
DELIMITER ;
...
/* Function  structure for function  `FunIngresaRActivos` */

/*!50003 DROP FUNCTION IF EXISTS `FunIngresaRActivos` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FunIngresaRActivos`(dfecha date,cndoc varchar(10),
ctipo char,nimpo decimal(12,2),nidalma integer,nidus integer,cdetalle varchar(200)) RETURNS int
begin
declare nid integer default 0;
insert into fe_ract(rcom_fech,rcom_ndoc,rcom_fope,rcom_tipo,rcom_impo,rcom_codt,rcom_idus,rcom_deta)
values(dfecha,cndoc,localtime,ctipo,nimpo,nidalma,nidus,cdetalle);
...
ctipo char,cdocp varchar(10),nimpo float,ninic float,
...
sacreedor decimal(12,2),cmone char,ndolar decimal(5,3),nidus integer,nidcp integer,corigen varchar(2),nidtda integer,cfp char) RETURNS int
begin
declare id integer;
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,lcaj_idus,lcaj_clpr,
lcaj_tran,lcaj_orig,lcaj_codt,caja_form,lcaj_fope)values
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidcp,'T',corigen,nidtda,cfp,localtime);
...
sacreedor decimal(12,2),cmone char,ndolar decimal(5,3),nidus integer,nidcp integer,cforma char,nidtda integer) RETURNS int
begin
declare id integer;
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,
lcaj_dola,lcaj_idus,lcaj_clpr,lcaj_form,lcaj_codt,lcaj_fope,lcaj_orig)values
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidcp,cforma,nidtda,localtime,'Ca');
select last_insert_id() into id from fe_lcaja group by last_insert_id();
return id;
end */$$
DELIMITER ;
...
ctdoc VARCHAR(2),cform CHAR,cndoc VARCHAR(12),dfecha DATE,coddetra VARCHAR(6),cdetra VARCHAR(6),cdetalle VARCHAR(120),
nv DECIMAL(12,2),nigv DECIMAL(12,2),nt DECIMAL(12,2),cndo2 VARCHAR(10),cm CHAR,
ndolar FLOAT,ni FLOAT,ctg CHAR,ccodp INTEGER,cmvto CHAR,nus INTEGER,opt INTEGER,nidcodt INTEGER,
n1 INTEGER,n2 INTEGER,n3 INTEGER,nexonerada DECIMAL(12,2),ndetraccion DECIMAL(10,2)) RETURNS int
BEGIN
DECLARE nid,idve INTEGER;
SET nid=0;
   SELECT gene_idve INTO idve FROM fe_gene WHERE idgene=1;
...
select round(sum(lcaj_deud),2) into ing from fe_lcaja where lcaj_fech<dfecha and lcaj_deud<>0 and lcaj_acti='A' and lcaj_form='E' and lcaj_codt=nidt;
select round(ifnull(sum(lcaj_deud),0),2) into eft from fe_lcaja where lcaj_fech<dfecha and lcaj_deud>0 and lcaj_acti='A' and lcaj_form='T' and lcaj_deud>0
and lcaj_tarj>0 and lcaj_codt=nidt;
select ifnull(sum(round(lcaj_acre,2)),0) into egr from fe_lcaja where lcaj_fech<dfecha and lcaj_acre>0  and lcaj_acti='A' and lcaj_form='E' and lcaj_codt=nidt;
return (ing+eft-egr);
end */$$
DELIMITER ;
...
nv decimal(12,2),nigv decimal(12,2),nt decimal(12,2),cndo2 varchar(10),cm char,
ndolar float,ni float,ctg char,ccodp integer,cmvto char,nus integer,opt integer,nidcodt integer,
n1 integer,n2 integer,n3 integer,nretencion decimal(8,2),npvta float,nidauto integer)
BEGIN
...
if opt=0 then
   ...
   select gene_idve into idve from fe_gene where idgene=1;
   update fe_rcom set tdoc=ctdoc,form=cform,ndoc=cndoc,fech=dfecha,fecr=dfechar,deta=cdetalle,valor=nv,igv=nigv,impo=nt,ndo2=cndo2,
   mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,rcom_mret=nretencion where idauto=nidauto;
   if n1>0 and n2>0 and n3>0 then
...
igv=nigv,impo=nt,ndo2=cndo2,mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,tipom=cmvto,codt=nidcodt WHERE idauto=nauto;
END */$$
DELIMITER ;
...
ctdoc VARCHAR(2),cform CHAR,cndoc VARCHAR(12),dfecha DATE,cdetra VARCHAR(5),cdetalle VARCHAR(120),
nv DECIMAL(12,2),nigv DECIMAL(12,2),nt DECIMAL(12,2),cndo2 VARCHAR(10),cm CHAR,
ndolar FLOAT,ni FLOAT,ctg CHAR,ccodp INTEGER,cmvto CHAR,nus INTEGER,opt INTEGER,nidcodt INTEGER,
n1 INTEGER,n2 INTEGER,n3 INTEGER,nexonerado DECIMAL(12,2),npvta FLOAT,ndetraccion DECIMAL(10,2),nidauto INTEGER)
BEGIN
DECLARE idve INTEGER;
   ...
   rcom_exon=nexonerado,rcom_mdet=ndetraccion,rcom_detra=cdetra   WHERE idauto=nidauto;
   IF n1>0 AND n2>0 AND n3>0 THEN
      SELECT @i1:=SUM(xx.nid1),@i2:=SUM(xx.nid2),@i3:=SUM(xx.nid3)
      FROM (SELECT CASE a.nitem WHEN 1 THEN idectas ELSE 0 END AS nid1,
      CASE a.nitem WHEN 2 THEN idectas ELSE 0 END AS nid2,
   ...
   CALL ProDesactivaLcajaE(nidauto);
   IF idve>0 AND cform='E' THEN
      CALL ProIngresaDatosLcajaE1(dfecha,"",CONCAT("Venta Dcto ",cndoc),idve,nt,0,cm,ndolar,nus,ccodp,nidauto);
   END IF;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProActualizaCajaTarjetas` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizaCajaTarjetas` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizaCajaTarjetas`(na integer,dfecha date,
nt1 float,cmvtoc char,cform char,cm1 char,cndoc varchar(12),nidcon integer,
cu integer,cdetalle varchar(120),cor varchar(2),nimp1 float,cm2 char,tcvta float,nidcodt integer,nidcaja integer,
ntarjeta decimal(12,2),ctipot char,btarjeta varchar(15),creferencia varchar(10))
BEGIN
Update fe_caja set fech=dfecha,impo=nt1,tipo=cmvtoc,forma=cform,tmon=cm1,ndoc=cndoc,idcon=nidcon,idusua=cu,deta=cdetalle,origen=cor,
nimpo=nimp1,mone=cm2,dola=tcvta,codt=nidcodt,caja_tarj=ntarjeta,caja_ttar=ctipot,caja_refe=creferencia,caja_btar=btarjeta where idcaja=nidcaja;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProActualizaDActivos` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizaDActivos` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizaDActivos`(nidart integer,ncant decimal(12,2),
nprecio decimal(8,2),nidauto integer,opt integer,nidk integer,dfecha date)
begin
if opt=0 then
   update fe_kact  set kact_acti='I' where kact_idka=nidk;
else
  update fe_kact  set kact_idar=nidart,kact_cant=ncant,kact_prec=nprecio,kact_idau=nidauto,kact_fcom=dfecha where kact_idka=nidk;
end if;
end */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProActualizaDcotizacion`(ncoda varchar(15),ncant float,nprec float,nr integer,opt integer)
BEGIN
if opt=0 then
...
/*!50003 CREATE PROCEDURE `ProActualizaDetallePedidos`(ncoda VARCHAR(15),ncant float,nprec float,nr integer,ctipoa char)
BEGIN
if ctipoa='A' then
     UPDATE fe_ped SET acti='I' WHERE idped=nr;
   else
...
/* Procedure structure for procedure `ProActualizaMvtoscontarjeta` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizaMvtoscontarjeta` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizaMvtoscontarjeta`(nidcaja integer,com1 decimal(8,2),
com2 decimal(12,2),com3 decimal(12,2),nigv decimal(12,2),portes decimal(8,2),deposito decimal(10,2),dfecha date)
BEGIN
update fe_caja
set caja_com1=com1,caja_com2=com2,caja_com3=com3,caja_igv=nigv,
caja_port=portes,caja_depo=deposito,caja_feab=dfecha where idcaja=nidcaja;
END */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProActualizaOCompra`(nid integer,opt char,nidart varchar(15),ncant float,nprec float)
BEGIN
if opt='E' then
...
np3 float,npeso float,ccat integer,cmar integer,ctipro char,nflete integer,cm char,nprecio float,
nidgrupo integer,nutil1 float,nutil2 float,nutil3 float,ncome float,
ncomc float,nidus integer,ncoda varchar(15),nsmin float,nsmax float,nidcosto integer,ndolar float,ce char)
BEGIN
UPDATE fe_art SET descri=cdesc,unid=cunid,cost=ncosto,premay=np1,premen=np2,pre3=np3,peso=npeso,idcat=ccat,idmar=cmar,tipro=ctipro,idflete=nflete,tmon=cm,
prec=nprecio,prod_uti1=nutil1,prod_uti2=nutil2,prod_uti3=nutil3,
prod_come=ncome,prod_comc=ncomc,prod_uact=nidus,prod_fact=localtime,prod_smax=nsmax,
prod_smin=nsmin,prod_idco=nidcosto,prod_dola=ndolar,prod_acti=ce  WHERE idart=ncoda;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProActualizaRActivos` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizaRActivos` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizaRActivos`(dfecha date,cndoc varchar(10),
ctipo char,opt integer,nimpo decimal(12,2),nidalma integer,nid integer,nidus integer,cdetalle varchar(200))
begin
if opt=0 then
   update fe_ract  set rcom_acti='I',rcom_idua=nidus where rcom_idau=nid;
else
  update fe_ract  set rcom_fech=dfecha,rcom_ndoc=cndoc,
  rcom_tipo=ctipo,rcom_impo=nimpo,rcom_codt=nidalma,rcom_idua=nidus,rcom_deta=cdetalle where rcom_idau=nid;
end if;
end */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProActualizaTbcos` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizaTbcos` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizaTbcos`(nidcta integer,cndoc varchar(15),dfecha date,cdeta varchar(150),cesta char,
nimporte decimal(12,2),opt integer,nidb integer)
begin
if opt=0 then
   update fe_tbcos set banc_acti='I' where banc_idba=nidb;
  else
   update fe_tbcos set banc_icta=nidcta,banc_ndoc=cndoc,banc_deta=cdeta,banc_fech=dfecha,banc_esta=cesta,banc_impo=nimporte
    where banc_idba=nidb;
end if;
end */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProActualizaVtasProductos` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProActualizaVtasProductos` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProActualizaVtasProductos`(nidart varchar(15),npreciol decimal(12,2),cmoneda char,
ndscto decimal(5,2),nprec decimal(8,2),npr1 decimal(8,2),npr2 decimal(8,2),npr3 decimal(8,2),
dfecha date,nidusua integer,nuti1 decimal(8,4),nuti2 decimal(8,4),nuti3 decimal(8,4))
BEGIN
update fe_art set prod_list=npreciol,prec=nprec,tmon=cmoneda,prod_dsct=ndscto,prod_fact=dfecha,prod_uact=nidusua,
prod_uti1=nuti1,prod_uti2=nuti2,prod_uti3=nuti3,premay=npr1,premen=npr2,pre3=npr3 where idart=nidart;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProAnulaCambio` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProAnulaCambio` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProAnulaCambio`(nid integer,nu integer,dfecha date,nu1 integer,
nidtda integer,nidcaja integer,nimpo decimal(12,2),cndoc varchar(10),ctdoc varchar(2),cdetalle varchar(120))
BEGIN
   ...
   set cconcepto:=concat(trim(ctdoc),'E');
   ...
   values(@nc,dfecha,dfecha,cndoc,ctdoc,'V',cdetalle,'','K','E','S','N',localtime,nu,nidtda);
   SELECT @na:=LAST_INSERT_ID() FROM fe_rcom group by LAST_INSERT_ID();
   INSERT INTO fe_caja(idauto,fech,impo,tipo,forma,tmon,ndoc,idcon,idusua,fechao,deta,origen,codt,anula)
   VALUES (@na,dfecha,nimpo,"I","E","S",cndoc,@nidconcepto,nu,localtime,cdetalle,"CK",nidtda,nimpo);
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProAnulaIngresoCargasBaterias` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProAnulaIngresoCargasBaterias` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProAnulaIngresoCargasBaterias`(nid integer)
begin
update fe_cargasb set carg_acti='I' where carg_idca=nid;
end */$$
DELIMITER ;
...
in ctipo char,IN nidauto integer,in nu integer, in sw char,dfecha date,in nu1 integer,nidtda integer)
BEGIN
declare nid integer;
declare cconcepto varchar(3);
DECLARE EXIT HANDLER FOR SQLEXCEPTION,SQLWARNING,NOT FOUND
...
if nid>0 then
   ...
   if ctipo='V' then
      ...
      if sw='S' then
        ...
        SELECT @nidconcepto:=idcon FROM fe_con WHERE tdoc=cconcepto AND tipo='I' AND conc_acti<>'I' GROUP BY idcon;
        call PROingresa_anulada(@df,@cn1,@ct,nu,@nidconcepto,nidtda);
      end if;
    else
...
BEGIN
update fe_lcaja set lcaj_acti='I' where lcaj_idca=nid;
update fe_cbancos set cban_acti='I' where cban_idca=nid;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProDesactivaDetalleVTa` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProDesactivaDetalleVTa` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProDesactivaDetalleVTa`(nid integer)
BEGIN
update  fe_detvta set detv_acti='I' where detv_idau=nid;
...
/*!50003 CREATE PROCEDURE `PRODSTOCKS`(nidart varchar(15))
BEGIN
select uno,dos,tre,cua,cin,sei,sie,och,nue,die from fe_art where idart=nidart;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProEntregaCargasBaterias` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProEntregaCargasBaterias` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProEntregaCargasBaterias`(nid integer,saldo decimal(8,2),dfee date)
begin
update fe_cargasb set carg_tipo='E',carg_acta=saldo,carg_fece=dfee where carg_idca=nid;
end */$$
DELIMITER ;
...
nimpo decimal(12,2),ctipo char,nidauto integer,ncodt integer)
begin
...
/* Procedure structure for procedure `ProIngresaCargasBaterias` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProIngresaCargasBaterias` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProIngresaCargasBaterias`(dfecha date,cdetalle varchar(200),
nimpo decimal(10,2),crefe varchar(200),cndoc varchar(10),nidalma integer,ncantidad integer(3),nacta decimal(10,2))
begin
insert into fe_cargasb(carg_fech,carg_deta,carg_impo,carg_ndoc,carg_refe,carg_tipo,carg_codt,carg_cant,carg_inic)
values(dfecha,cdetalle,nimpo,cndoc,crefe,'I',nidalma,ncantidad,nacta);
end */$$
DELIMITER ;
...
idcta integer,sdeudor decimal(12,2),sacreedor decimal(12,2),cmone char,ndolar decimal(5,3),nidus integer,nidcp integer,nidauto integer,
cform char,cdcto char(15),ctdoc varchar(2),nidt integer,corigen varchar(2),
ntarjetas DECIMAL(12,2),ctipot CHAR,btarjeta VARCHAR(15),creferencia VARCHAR(10))
begin
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
lcaj_idus,lcaj_clpr,lcaj_idau,lcaj_form,lcaj_fope,lcaj_dcto,lcaj_tdoc,lcaj_codt,lcaj_orig,lcaj_tarj,lcaj_btar,lcaj_ttar,lcaj_refe)values
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidcp,nidauto,cform,localtime,cdcto,ctdoc,nidt,corigen,
ntarjetas,btarjeta,ctipot,creferencia);
end */$$
DELIMITER ;

/* Procedure structure for procedure `ProIngresaDatosLcajaEefectivo1` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProIngresaDatosLcajaEefectivo1` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProIngresaDatosLcajaEefectivo1`(dfecha date,cndoc varchar(12),cdeta varchar(120),
idcta integer,sdeudor decimal(12,2),sacreedor decimal(12,2),cmone char,ndolar decimal(5,3),nidus integer,nidcp integer,nidauto integer,
cform char,cdcto char(15),ctdoc varchar(2),nidt integer,corigen varchar(2),
ntarjetas DECIMAL(12,2),ctipot CHAR,btarjeta VARCHAR(15),creferencia VARCHAR(10),cfo char)
begin
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
lcaj_idus,lcaj_clpr,lcaj_idau,lcaj_form,lcaj_fope,lcaj_dcto,lcaj_tdoc,lcaj_codt,lcaj_orig,lcaj_tarj,lcaj_btar,lcaj_ttar,lcaj_refe,caja_form)values
(dfecha,cndoc,cdeta,idcta,sdeudor,sacreedor,cmone,ndolar,nidus,nidcp,nidauto,cform,localtime,cdcto,ctdoc,nidt,corigen,
ntarjetas,btarjeta,ctipot,creferencia,cfo);
end */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProIngresaDetalleOCompra`(nid integer,nidart varchar(15),ncant float,nprec float)
BEGIN
...
/* Procedure structure for procedure `ProIngresaKardexActivos` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProIngresaKardexActivos` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProIngresaKardexActivos`(nidart integer,ncant decimal(12,2),
nprecio decimal(8,2),nidauto integer,dfecha date)
begin
insert into fe_kact(kact_idar,kact_cant,kact_prec,kact_idau,kact_fcom)
values(nidart,ncant,nprecio,nidauto,dfecha);
end */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProIngresaTbcos` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProIngresaTbcos` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProIngresaTbcos`(nidcta integer,cndoc varchar(15),dfecha date,cdeta varchar(150),cesta char,nimporte decimal(12,2))
begin
insert into fe_tbcos(banc_icta,banc_ndoc,banc_fech,banc_deta,banc_esta,banc_impo)values(nidcta,cndoc,dfecha,cdeta,cesta,nimporte);
end */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `Proingresa_anulada`(in dfecha Datetime,in cndoc varchar(12),in ctdoc varchar(2),
in nidus integer,in nidcon integer,in nidtda integer)
begin
 select @nc:=idclie from fe_clie where nruc='***********';
 insert into fe_rcom(idcliente,fech,fecr,ndoc,tdoc,tipom,deta,ndo2,tcom,form,mone,exon,fusua,idusua,codt)
 values(@nc,dfecha,dfecha,cndoc,ctdoc,'V','','','K','E','S','N',localtime,nidus,nidtda);
 SELECT @na:=LAST_INSERT_ID() FROM fe_rcom group by LAST_INSERT_ID();
 INSERT INTO fe_lcaja(lcaj_idau,lcaj_fech,lcaj_deud,lcaj_form,lcaj_dcto,lcaj_idus,lcaj_fope,lcaj_deta,lcaj_orig,lcaj_codt)
 VALUES (@na,dfecha,0,"E",cndoc,nidus,localtime,"*** ANULADA ***","CK",nidtda);
END */$$
DELIMITER ;
...
begin
 SELECT idart,descri,unid,uno,dos,tre,cua,cin,sei,sie,och,nue,die,onc,prod_dct1,prod_dct2,prod_dct3,
  premay AS  pre1,
  premen AS pre2,pre3,
  ROUND(IF(tmon='S',(a.prec*v.igv)+b.prec,(a.prec*v.igv*v.dola)+b.prec),2) AS costo,c.idgrupo,c.dcat,
  ROUND(IF(tmon='S',(a.prec*v.igv),(a.prec*v.igv*v.dola)),2) AS costosf,b.prec AS flete,a.ulfc,
  IFNULL(d.cost_cost,0) AS costor,IFNULL(d.cost_prec,0) AS precr,''  AS moner,
  CAST(0 AS UNSIGNED) AS cost_idco,IFNULL(d.cost_flet,0)  AS fleter,v.dola AS dolar,
  ...
  WHERE trim(idart)=TRIM(nid) AND prod_acti<>'I' ORDER BY DESCRI;
end */$$
DELIMITER ;
...
BEGIN
SELECT nomb,idalma,dire,ciud,sucuidserie,ubigeo FROM fe_sucu  where idalma in (1,2,6,7,8,10) order by idalma;
END */$$
DELIMITER ;
...
case
 when opt=0 then
   ...
   clie_idpt,ifnull(dpto_nomb,'') as dpto,clie_dist as distrito,clie_rete
...
/*!50003 CREATE PROCEDURE `PROMuestraClientes1`(cbusca varchar(80),opt integer,nid integer)
BEGIN
declare cbuscar varchar(80);
set cbuscar=concat('%',trim(cbusca),+'%');
case
...
set cbuscar=concat('%',trim(abuscar),+'%');
if opt=1 then
  SELECT idart,descri,unid,uno,dos,tre,cua,cin,sei,sie,och,nue,die,
  premay as pre1,premen as pre2,pre3,
...
if sw=1 then
if opt=1 then
  SELECT idart,descri,unid,uno,dos,tre,cua,cin,sei,sie,och,nue,die,onc,
...
if op=0 then
  ...
  Update fe_kar set idauto=nid,idart=cc,tipo=ct,prec=npr,cant=nct,ttip=tmvto,incl=cincl,
...
/*!50001 CREATE TABLE  `vguiasdevolucion`(
 ...
 `guia_arch` varchar(20) ,
 `email` varchar(45) ,
 `guia_hash` varchar(10) ,
 `guia_feen` datetime ,
 `guia_codt` int ,
...
/*!50001 CREATE TABLE  `vstock`(
 `idart` varchar(15) ,
 `codi_desc` varchar(5) ,
 `tcompras` decimal(32,2) ,
 `tventas` decimal(32,2) ,
 `alma` int ,
 `fech` date ,
 `kar_codi` int unsigned 
)*/;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vmuestracompras` AS select `a`.`idauto` AS `idauto`,`a`.`alma` AS `alma`,`a`.`idkar` AS `idkar`,`b`.`descri` AS `descri`,`b`.`peso` AS `peso`,`b`.`prod_idco` AS `prod_idco`,`b`.`unid` AS `unid`,`b`.`tipro` AS `tipro`,`a`.`idart` AS `idart`,`a`.`incl` AS `incl`,`c`.`ndoc` AS `ndoc`,`c`.`valor` AS `valor`,`c`.`igv` AS `igv`,`c`.`impo` AS `impo`,`c`.`pimpo` AS `pimpo`,`a`.`cant` AS `cant`,`a`.`prec` AS `prec`,`c`.`fech` AS `fech`,`a`.`kar_codi` AS `kar_codi`,`q`.`codi_desc` AS `codi_desc`,`c`.`fecr` AS `fecr`,`c`.`form` AS `form`,`c`.`exon` AS `exon`,`c`.`ndo2` AS `ndo2`,`c`.`vigv` AS `vigv`,`c`.`idprov` AS `idprov`,`a`.`tipo` AS `tipo`,`c`.`tdoc` AS `tdoc`,`c`.`dolar` AS `dolar`,`c`.`mone` AS `mone`,`p`.`razo` AS `razo`,`p`.`dire` AS `dire`,`p`.`ciud` AS `ciud`,`p`.`nruc` AS `nruc`,ifnull(`x`.`idcaja`,0) AS `Idcaja`,`c`.`codt` AS `codt`,`a`.`dsnc` AS `dsnc`,`a`.`dsnd` AS `dsnd`,`a`.`gast` AS `gast`,`c`.`fusua` AS `fusua`,`w`.`nomb` AS `Usuario` from ((((((`fe_rcom` `c` left join `fe_kar` `a` on((`c`.`idauto` = `a`.`idauto`))) left join `fe_art` `b` on((`b`.`idart` = `a`.`idart`))) join `fe_prov` `p` on((`p`.`idprov` = `c`.`idprov`))) join `fe_cingreso` `q` on((`q`.`codi_idco` = `a`.`kar_codi`))) left join `fe_caja` `x` on((`x`.`idauto` = `c`.`idauto`))) join `fe_usua` `w` on((`w`.`idusua` = `c`.`idusua`))) where ((`c`.`acti` <> 'I') and (`a`.`acti` <> 'I')) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vmuestraventas` AS select `a`.`kar_comi` AS `kar_comi`,`a`.`codv` AS `codv`,`a`.`idauto` AS `idauto`,`c`.`codt` AS `alma`,`a`.`kar_idco` AS `idcosto`,`a`.`idkar` AS `idkar`,`a`.`idart` AS `Coda`,`a`.`cant` AS `cant`,`a`.`prec` AS `prec`,`c`.`valor` AS `valor`,`c`.`igv` AS `igv`,`c`.`impo` AS `impo`,`c`.`fech` AS `fech`,`c`.`fecr` AS `fecr`,`c`.`rcom_dsct` AS `rcom_dsct`,`c`.`rcom_mens` AS `rcom_mens`,`c`.`form` AS `form`,`c`.`deta` AS `deta`,`c`.`exon` AS `exon`,`c`.`ndo2` AS `ndo2`,`c`.`rcom_entr` AS `rcom_entr`,`c`.`idcliente` AS `idclie`,`d`.`razo` AS `razo`,`d`.`nruc` AS `nruc`,`d`.`dire` AS `dire`,`d`.`ciud` AS `ciud`,`d`.`ndni` AS `ndni`,`a`.`tipo` AS `tipo`,`c`.`tdoc` AS `tdoc`,`c`.`ndoc` AS `ndoc`,`c`.`dolar` AS `dolar`,`c`.`mone` AS `mone`,`b`.`descri` AS `descri`,ifnull(`x`.`idcaja`,0) AS `idcaja`,`b`.`unid` AS `unid`,`b`.`premay` AS `pre1`,`b`.`peso` AS `peso`,`b`.`premen` AS `pre2`,ifnull(`z`.`vend_idrv`,0) AS `nidrv`,`c`.`vigv` AS `vigv`,`a`.`dsnc` AS `dsnc`,`a`.`dsnd` AS `dsnd`,`a`.`gast` AS `gast`,`c`.`idcliente` AS `idcliente`,`c`.`codt` AS `codt`,`b`.`pre3` AS `pre3`,`b`.`cost` AS `costo`,`b`.`uno` AS `uno`,`b`.`dos` AS `dos`,`b`.`tre` AS `tre`,(((((`b`.`uno` + `b`.`dos`) + `b`.`sei`) + `b`.`cin`) + `b`.`cua`) + `b`.`nue`) AS `TAlma`,`b`.`sei` AS `sei`,`b`.`cua` AS `cua`,`b`.`cin` AS `cin`,`b`.`sie` AS `sie`,`b`.`och` AS `och`,`b`.`nue` AS `nue`,`b`.`die` AS `die`,`a`.`kar_codi` AS `kar_codi`,`c`.`fusua` AS `fusua`,`p`.`nomv` AS `Vendedor`,`q`.`nomb` AS `Usuario`,`b`.`tipro` AS `tipro`,`c`.`rcom_mret` AS `rcom_mret`,`c`.`rcom_mdet` AS `rcom_mdet` from (((((((`fe_art` `b` join `fe_kar` `a` on((`b`.`idart` = `a`.`idart`))) join `fe_rcom` `c` on((`a`.`idauto` = `c`.`idauto`))) left join `fe_caja` `x` on((`x`.`idauto` = `c`.`idauto`))) join `fe_clie` `d` on((`c`.`idcliente` = `d`.`idclie`))) join `fe_vend` `p` on((`p`.`idven` = `a`.`codv`))) join `fe_usua` `q` on((`q`.`idusua` = `c`.`idusua`))) left join (select `fe_rvendedor`.`vend_idau` AS `vend_idau`,`fe_rvendedor`.`vend_idrv` AS `vend_idrv` from `fe_rvendedor` where (`fe_rvendedor`.`vend_acti` = 'A')) `z` on((`z`.`vend_idau` = `c`.`idauto`))) where ((`c`.`tipom` = 'V') and (`c`.`acti` <> 'I') and (`a`.`acti` <> 'I')) */;
