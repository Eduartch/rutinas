...
MySQL - 8.0.39 : Database - syscom_bdfertiriego
*********************************************************************
*/
...
ncome float,ncomc float,nutil1 float,nutil2 float,nutil3 float,nidusua integer,nsmax float, nsmin float,ccodigo1 varchar(20),ndolar float) RETURNS int
BEGIN
declare nid integer default 0;
...
/*!50003 CREATE FUNCTION `FunIngresaDPedidos`(ncoda integer,ncant float,nprec float,nidauto integer,igv CHAR(1)) RETURNS int
BEGIN
INSERT INTO fe_ped(idart,cant,prec,idautop,incl)
VALUES(ncoda,ncant,nprec,nidauto,igv);
return 10;
END */$$
DELIMITER ;
...
open cursor1;
start transaction;
UPDATE fe_art SET uno=0,dos=0,tre=0,cua=0,cin=0,sei=0,sie=0;
repeat
...
if ctipoa='A' then
    ...
    UPDATE fe_ped SET idart=ncoda,cant=ncant,prec=nprec,incl=igv WHERE idped=nr;
end if;
END */$$
DELIMITER ;
...
ncomc float,nidus integer,ncoda integer,nsmax float, nsmin float,ccodigo1 varchar(20),ndolar float,ce char)
BEGIN
...
BEGIN
   ROLLBACK;
   SET estado:="No se ejecuto correctamente la transaccion";
END;
START TRANSACTION;
SET nid=0;
IF nidauto=0 THEN
