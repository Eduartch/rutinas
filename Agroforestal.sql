...
MySQL - 5.7.42 : Database - eduar_bdwalter
*********************************************************************
*/
...
if new.acti='I' then
    update fe_dseries set dser_acti='I' where dser_idka=old.idkar;
    call astock(old.idart,old.alma,old.cant,if(old.tipo="C","V","C"));
    insert into fe_akardex(logk_deta,logk_idar,logk_cant,logk_prec,
    logk_ida1,logk_idk1,logk_fech,logk_idco)values('Anulada',old.idart,old.cant,old.prec,old.idauto,
...
if new.acti='I' then
   ...
   update fe_rseries set rser_acti='I' where rser_idau=old.idauto;
...
/* Trigger structure for table `fe_rseries` */

DELIMITER $$

/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `AnulaRSeries` */$$

/*!50003 CREATE */ /*!50003 TRIGGER `AnulaRSeries` AFTER UPDATE ON `fe_rseries` FOR EACH ROW begin
if new.rser_acti='I' then
   update fe_dseries set dser_acti='I' where dser_idre=old.rser_seri;
end if;
end */$$
...
/*!50003 CREATE FUNCTION `FuncreaProductos`(`cdesc` VARCHAR(150), `cunid` VARCHAR(20), `nprec` FLOAT, `ncosto` FLOAT, `np1` FLOAT, `np2` FLOAT, `np3` FLOAT, `npeso` FLOAT, `ccat` INTEGER, `cmar` INTEGER, `ctipro` CHAR, `nflete` INTEGER, `cm` CHAR, `cidpc` VARCHAR(45), `ncome` FLOAT, `ncomc` FLOAT, `nutil1` FLOAT, `nutil2` FLOAT, `nutil3` FLOAT, `nidusua` INTEGER, `nsmin` FLOAT, `nsmax` FLOAT, `ccoda` VARCHAR(20), `ndolar` FLOAT) RETURNS int(11)
BEGIN
declare nid integer default 0;
...
cidpc,ncome,ncomc,nutil1,nutil2,nutil3,nidusua,nsmin,nsmax,ccoda,ndolar);
...
DECLARE sw INTEGER DEFAULT 0;
DECLARE CONTINUE HANDLER FOR NOT FOUND SET sw=0;
SELECT idauto INTO sw FROM fe_rcom WHERE ndoc=cdcto AND tdoc=ctdoc AND acti<>'I' AND idauto<>nidauto;
RETURN sw;
END */$$
DELIMITER ;
   ...
   select last_insert_id() into nid from fe_rcom group by last_insert_id();
    if n1>0 and n2>0 and n3>0 then
      ...
      if idce>0 and cform='E' then
        Call ProIngresaDatosLcajaE1(dfecha,"",concat("Compra al Contado No Dcto ",cndoc),
        idce,0,nt+npvta,cm,ndolar,nus,ccodp,nid);
      end if;
   end if;
  else
...
/*!50003 CREATE FUNCTION `FunIngresaCabeceraTraspasoN`(`ctdoc` VARCHAR(2), `cform` CHAR, `cndoc` VARCHAR(10), `dfecha` DATE, `dfechar` DATE, `cdetalle` VARCHAR(120), `nv` FLOAT, `nigv` FLOAT, `nt` FLOAT, `cndo2` VARCHAR(10), `cm` CHAR, `ndolar` FLOAT, `ni` FLOAT, `ctg` CHAR, `ccodp` INTEGER, `cmvto` CHAR, `nus` INTEGER, `opt` INTEGER, `nidcodt` INTEGER, `n1` INTEGER, `n2` INTEGER, `n3` INTEGER, `nitem` INTEGER, `npvta` FLOAT, `copt` CHAR) RETURNS int(11)
BEGIN
declare nid integer;
...
/* Function  structure for function  `FuningresaDocumentoElectronicoretencion` */

/*!50003 DROP FUNCTION IF EXISTS `FuningresaDocumentoElectronicoretencion` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `FuningresaDocumentoElectronicoretencion`(`ctdoc` VARCHAR(2), `cform` CHAR, 
`cndoc` VARCHAR(12), `dfecha` DATE, `cdetalle` VARCHAR(220), `nv` DECIMAL(12,2), `nigv` DECIMAL(12,2), `nt` DECIMAL(12,2), 
`cndo2` VARCHAR(10), `cm` CHAR, `ndolar` DECIMAL(6,4), `ni` DECIMAL(6,4), `ctg` CHAR, `ccodp` INTEGER, tgratuitas decimal(10,2), `nus` INTEGER, 
`nidcodt` INTEGER, `n1` INTEGER, `n2` INTEGER, `n3` INTEGER, `nvend` INTEGER, `idtr` DECIMAL(10,2), `nexon` DECIMAL(12,2), 
`ndscto` DECIMAL(12,2),nretencion decimal(8,2)) RETURNS int(11)
BEGIN
declare nid integer;
set nid=0;
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idcliente,
   tipom,fusua,idusua,codt,pimpo,rcom_exon,rcom_dsct,rcom_vend,rcom_otro,rcom_mret)
   VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,'V',localtime,nus,nidcodt,
   idtr,nexon,ndscto,nvend,tgratuitas,nretencion);
   select last_insert_id() into nid from fe_rcom group by last_insert_id();
    if n1>0 and n2>0 and n3>0 then
      call IngresaCuentasV(nv,nigv,nt,n1,n2,n3,"H","H","D",nid);
end if;
...
/*!50003 CREATE FUNCTION `FuningresaDocumentoElectronicoVtas`(`ctdoc` VARCHAR(2), `cform` CHAR, 
`cndoc` VARCHAR(12), `dfecha` DATE, `cdetalle` VARCHAR(220), `nv` DECIMAL(12,2), `nigv` DECIMAL(12,2), `nt` DECIMAL(12,2), 
`cndo2` VARCHAR(10), `cm` CHAR, `ndolar` DECIMAL(6,4), `ni` DECIMAL(6,4), `ctg` CHAR, `ccodp` INTEGER, tgratuitas decimal(10,2), `nus` INTEGER, 
`nidcodt` INTEGER, `n1` INTEGER, `n2` INTEGER, `n3` INTEGER, `nvend` INTEGER, `idtr` DECIMAL(10,2), `nexon` DECIMAL(12,2), 
`ndscto` DECIMAL(12,2)) RETURNS int(11)
BEGIN
declare nid integer;
set nid=0;
   INSERT INTO fe_rcom(tdoc,form,ndoc,fech,fecr,deta,valor,igv,impo,ndo2,mone,dolar,vigv,tcom,idcliente,
   tipom,fusua,idusua,codt,pimpo,rcom_exon,rcom_dsct,rcom_vend,rcom_otro)
   VALUES (ctdoc,cform,cndoc,dfecha,dfecha,cdetalle,nv,nigv,nt,cndo2,cm,ndolar,ni,ctg,ccodp,'V',localtime,nus,nidcodt,
   idtr,nexon,ndscto,nvend,tgratuitas);
   select last_insert_id() into nid from fe_rcom group by last_insert_id();
    if n1>0 and n2>0 and n3>0 then
      call IngresaCuentasV(nv,nigv,nt,n1,n2,n3,"H","H","D",nid);
end if;
...
dfechat DATETIME,nidus INTEGER,cdeta VARCHAR(150),nidtr INTEGER,cndoc VARCHAR(12),nidtda INTEGER,cdcto VARCHAR(20),df DATE,nidpr INTEGER,cubigeo VARCHAR(8)) RETURNS int(11)
BEGIN
DECLARE id INTEGER;
INSERT INTO fe_guias(guia_fech,guia_ptop,guia_ptoll,guia_idau,guia_fect,guia_idus,guia_fope,guia_deta,guia_idtr,guia_ndoc,guia_moti,
...
/*!50003 CREATE FUNCTION `FunIngresaKardex1`(`nid` INTEGER, `cc` INTEGER, `ct` CHAR, `npr` FLOAT, `nct` FLOAT, 
`cincl` CHAR, `tmvto` CHAR, `ccodv` INTEGER, `calma` INTEGER, `nidcosto1` decimal(10,2), `vcom` FLOAT) RETURNS int(11)
BEGIN
declare nidk integer default 0;
if ct='C' then
...
declare vdvto integer default 0;
DECLARE CONTINUE HANDLER FOR NOT FOUND SET vdvto=0;
SELECT idauto into vdvto FROM fe_rcom WHERE ndoc=cdcto and tdoc=ctdoc AND acti<>'I' and idauto<>id1 and idcliente>0 limit 1;
return vdvto;
END */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProActualizaCabeceraCV`(`ctdoc` VARCHAR(2), `cform` CHAR, `cndoc` VARCHAR(12), 
`dfecha` DATE, `dfechar` DATE, `cdetalle` VARCHAR(120), `nv` DECIMAL(12,2), `nigv` DECIMAL(12,2), `nt` DECIMAL(12,2), 
`cndo2` VARCHAR(10), `cm` CHAR, `ndolar` FLOAT, `ni` FLOAT, `ctg` CHAR, `ccodp` INTEGER, `cmvto` CHAR, `nus` INTEGER, 
`opt` INTEGER, `nidcodt` INTEGER, `n1` INTEGER, `n2` INTEGER, `n3` INTEGER, tgratuitas decimal(10,2), `npvta` FLOAT, `nidauto` INTEGER)
BEGIN
...
if opt=0 then
 ...
 call ProDesactivaLcajaE(nidauto);
  if n1>0 and n2>0 and n3>0 then
      ...
      if idce>0 and cform='E' then
      Call ProIngresaDatosLcajaE1(dfecha,"",concat("Compra ",cndoc),idce,0,nt+npvta,cm,ndolar,nus,ccodp,nidauto);
   end if;
   end if;
  else
   select gene_idve into idve from fe_gene where idgene=1;
   update fe_rcom set tdoc=ctdoc,form=cform,ndoc=cndoc,fech=dfecha,fecr=dfechar,deta=cdetalle,valor=nv,igv=nigv,impo=nt,ndo2=cndo2,
   mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom=cmvto,idusua1=nus,codt=nidcodt,rcom_otro=tgratuitas where idauto=nidauto;
   call ProDesactivaLcajaE(nidauto);
   if n1>0 and n2>0 and n3>0 then
     ...
     if idve>0 and cform='E' and ctdoc<>'20' then
      Call ProIngresaDatosLcajaE1(dfecha,"",concat("Venta ",cndoc),idve,nt,0,cm,ndolar,nus,ccodp,nidauto);
     end if;
   end if;
 end if;
END */$$
...
ndolar FLOAT,ni FLOAT,ctg CHAR,ccodp INTEGER,nretencion DECIMAL(10,2),nus INTEGER,nporcentaje DECIMAL(5,2),nidcodt INTEGER,
...
UPDATE fe_rcom SET tdoc=ctdoc,form=cform,ndoc=cndoc,fech=dfecha,fecr=dfecha,deta=cdetalle,valor=nv,igv=nigv,impo=nt,ndo2=cndo2,
 mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom='V',idusua1=nus,codt=nidcodt,
 rcom_vend=nidven,rcom_dsct=nporcentaje,rcom_exon=nexon,rcom_mret=nretencion WHERE idauto=nidauto;
 CALL ProDesactivaLcajaE(nidauto);
   IF n1>0 AND n2>0 AND n3>0 THEN
...
DECLARE idve,nidctaper INTEGER;
    UPDATE fe_rcom SET tdoc=ctdoc,form=cform,ndoc=cndoc,fech=dfecha,fecr=dfecha,deta=cdetalle,valor=nv,igv=nigv,impo=nt,ndo2=cndo2,
   mone=cm,dolar=ndolar,vigv=ni,tcom=ctg,idcliente=ccodp,tipom='V',idusua1=nus,codt=nidcodt,rcom_exon=nexon,rcom_mret=ndetraccion WHERE idauto=nidauto;
 CALL ProDesactivaLcajaE(nidauto);
  IF n1>0 AND n2>0 AND n3>0 THEN
      SELECT @i1:=SUM(xx.nid1),@i2:=SUM(xx.nid2),@i3:=SUM(xx.nid3)
      FROM (SELECT CASE a.nitem WHEN 1 THEN idectas ELSE 0 END AS nid1,
...
/*!50003 CREATE PROCEDURE `ProActualizaPreciosProducto`(`cc` INTEGER, `dfe` DATE, `npr` FLOAT, `cnd` INTEGER, `idp` INTEGER, `cmda` CHAR, `ni` FLOAT, `ndolar` FLOAT, `nidcosto` INTEGER)
BEGIN
...
/*!50003 CREATE PROCEDURE `ProActualizaProductos`(`cdesc` VARCHAR(150), `cunid` VARCHAR(20), `ncosto` FLOAT, `np1` FLOAT, `np2` FLOAT, `np3` FLOAT, `npeso` FLOAT, `ccat` INTEGER, `cmar` INTEGER, `ctipro` CHAR, `nflete` INTEGER, `cm` CHAR, `nprecio` FLOAT, `ccoda` VARCHAR(20), `nutil1` FLOAT, `nutil2` FLOAT, `nutil3` FLOAT, `ncome` FLOAT, `ncomc` FLOAT, `nidus` INTEGER, `ncoda` INTEGER, `nsmin` FLOAT, `nsmax` FLOAT, `nidcosto` INTEGER, `ndolar` FLOAT, `cestado` CHAR)
BEGIN
...
prod_smin=nsmin,prod_idco=nidcosto,prod_dola=ndolar,prod_acti=cestado  WHERE idart=ncoda;
END */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProActualizaRCompras`(`ctdoc` VARCHAR(2), `cform` CHAR, `cndoc` VARCHAR(10), `dfecha` DATE, `dfechar` DATE, `cdetalle` VARCHAR(120), `nv` DECIMAL(12,2), `nigv` DECIMAL(12,2), `nt` DECIMAL(12,2), `cndo2` VARCHAR(10), `cm` CHAR, `ndolar` FLOAT, `ni` FLOAT, `ctg` CHAR, `ccodp` INTEGER, `cmvto` CHAR, `nus` INTEGER, `opt` INTEGER, `nidcodt` INTEGER, `n1` INTEGER, `n2` INTEGER, `n3` INTEGER, `nitems` INTEGER, `npvta` FLOAT, `nidauto` INTEGER, `nexon` DECIMAL(12,2), `notros` DECIMAL(12,2))
BEGIN
...
BEGIN
select @nidauto:=guia_idau from fe_guias where guia_idgui=na and guia_moti='D' limit 1;
if @nidauto>0 then
   update fe_rcom set acti='I' where idauto=@nidauto;
end if;   
...
/*!50003 CREATE PROCEDURE `ProAnulaTransacciones1`(OUT `estado` VARCHAR(500), IN `ctdoc` VARCHAR(2), IN `cndoc` VARCHAR(12), IN `ctipo` CHAR, IN `nidauto` INTEGER, IN `nu` INTEGER, IN `sw` CHAR, `dfecha` DATE, IN `nu1` INTEGER, IN `nimporte` INTEGER)
BEGIN
declare nid integer;
DECLARE EXIT HANDLER FOR SQLEXCEPTION,SQLWARNING,NOT FOUND
begin
...
if nidauto=0 then
   ...
   set @cn1:=cndoc;
   set @df:=dfecha;
   select @idclave:=idauto,@nimp:=impo from fe_rcom where tdoc=ctdoc and ndoc=cndoc and tipom =ctipo limit 1;
   set nid=@idclave;
  else
   set nid=nidauto;
   select @ct:=tdoc,@cn1:=ndoc,@df:=fech,@nimp:=impo from fe_rcom where idauto=nid limit 1;
end if;
if nid>0 then
   update fe_rcom set acti='I',idusua1=nu,rcom_idus=nu1 where idauto=nid;
   if ctipo='V' then
...
BEGIN
SELECT nume,items,idserie,numebnotas FROM fe_serie WHERE serie=nserie AND tdoc=ctdoc;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `ProGeneraCorrelativootraserie` */

/*!50003 DROP PROCEDURE IF EXISTS  `ProGeneraCorrelativootraserie` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `ProGeneraCorrelativootraserie`(`nn` INTEGER, `ns` INTEGER)
BEGIN
UPDATE fe_serie SET numebnotas=nn WHERE idserie=ns;
END */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProIngresaDatosLcajaEefectivo`(`dfecha` DATE, `cndoc` VARCHAR(10), `cdeta` VARCHAR(120), `idcta` INTEGER, `sdeudor` DECIMAL(12,2), `sacreedor` DECIMAL(12,2), `cmone` CHAR, `ndolar` DECIMAL(5,3), `nidus` INTEGER, `nidcp` INTEGER, `nidauto` INTEGER, `cform` CHAR, `cdcto` CHAR(15), `ctdoc` VARCHAR(2))
begin
insert into fe_lcaja(lcaj_fech,lcaj_ndoc,lcaj_deta,lcaj_idct,lcaj_deud,lcaj_acre,lcaj_mone,lcaj_dola,
...
/*!50003 CREATE PROCEDURE `ProIngresaDetalleVta`(`cdesc` VARCHAR(80), `nitem` INTEGER, `nitem1` INTEGER, `nitem2` INTEGER, `nidauto` INTEGER, `nprecio` DECIMAL(12,5), `ncant` DECIMAL(12,4), `cunid` VARCHAR(10))
BEGIN
...
begin
 ...
 values(@nc,dfecha,dfecha,cndoc,ctdoc,'V','','','K','E','S','N',localtime,nidus,nimporte);
 SELECT @na:=LAST_INSERT_ID() FROM fe_rcom group by LAST_INSERT_ID();
...
BEGIN
SELECT nomb,idalma,dire,ciud,sucuidserie,ubigeo FROM fe_sucu where idalma in(1,2) ORDER BY idalma;
END */$$
DELIMITER ;
...
if opt=0 then
   ...
   ifnull(celu,'0') as celu,refe,clie_tipo,clie_codv,clie_lcre,clie_corr,clie_rpm,clie_idus,clie_actu,fechclie,clie_feac,
   ifnull(c.zona_nomb,'') as Zona,ifnull(a.clie_idzo,0) as clie_idzo,clie_rete
...
WHEN opt=0 THEN
    ...
    ROUND(IF(tmon='S',(a.prec*prod_tigv)+b.prec,(a.prec*prod_tigv*nd)+b.prec),2) AS costo,c.idgrupo,c.dcat,prod_dola,prod_cod1 AS cod1,
    ROUND(IF(tmon='S',(a.prec*prod_tigv),(a.prec*prod_tigv*nd)),2) AS costosf,b.prec AS flete,prod_cod1,
    IFNULL(d.cost_cost,0) AS costor,IFNULL(d.cost_prec,0) AS precr,IFNULL(d.cost_mone,'')  AS moner,
    CAST(IFNULL(d.cost_idco,0) AS UNSIGNED) AS cost_idco,IFNULL(d.cost_flet,0)  AS fleter,IFNULL(d.cost_dola,0) AS dolar,
    ...
    u.nomb AS usuacreo,us.nomb AS usuamodifico,prod_grat
    FROM fe_art  AS a
...
BEGIN
SELECT idserie,tdoc,serie,nume,codt,seri_idal,numebnotas FROM fe_serie ORDER BY idserie;
END */$$
DELIMITER ;
...
/*!50001 CREATE TABLE  `vguiasdevolucion`(
 ...
 `refe` varchar(12) ,
 `tdoc` varchar(2) ,
 `guia_mens` varchar(150) ,
 `guia_arch` varchar(200) ,
 `email` varchar(45) ,
 `guia_hash` varchar(30) ,
 `guia_feen` datetime ,
 `guia_codt` int(5) ,
...
/*!50001 CREATE TABLE  `vlistaprecios`(
 ...
 `tmon` varchar(1) ,
 `idflete` int(11) ,
 `prod_uti1` decimal(6,3) ,
 `prod_uti2` decimal(6,3) ,
 `prod_uti3` decimal(6,3) ,
 `prod_come` float ,
 `prod_comc` float ,
 `ulpc` int(11) ,
 `prod_idus` int(10) ,
...
/*!50001 CREATE TABLE  `vmuestraventas`(
 ...
 `rcom_tipo` char(1) ,
 `kar_cost` decimal(12,2) ,
 `rcom_mret` decimal(10,2) 
)*/;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vmuestraventas` AS select `c`.`rcom_exon` AS `rcom_exon`,`c`.`rcom_mens` AS `rcom_mens`,`c`.`idusua` AS `idusua`,`a`.`kar_comi` AS `kar_comi`,`a`.`codv` AS `codv`,`a`.`idauto` AS `idauto`,`a`.`alma` AS `alma`,`a`.`kar_idco` AS `idcosto`,`a`.`idkar` AS `idkar`,`a`.`idart` AS `Coda`,`a`.`cant` AS `cant`,`a`.`prec` AS `prec`,`c`.`valor` AS `valor`,`c`.`igv` AS `igv`,`c`.`impo` AS `impo`,`c`.`fech` AS `fech`,`c`.`fecr` AS `fecr`,`c`.`form` AS `form`,`c`.`deta` AS `deta`,`c`.`exon` AS `exon`,`c`.`ndo2` AS `ndo2`,`c`.`rcom_entr` AS `rcom_entr`,`c`.`idcliente` AS `idclie`,`d`.`razo` AS `razo`,`d`.`nruc` AS `nruc`,`d`.`dire` AS `dire`,`d`.`ciud` AS `ciud`,`d`.`ndni` AS `ndni`,`a`.`tipo` AS `tipo`,`c`.`tdoc` AS `tdoc`,`c`.`ndoc` AS `ndoc`,`c`.`dolar` AS `dolar`,`c`.`mone` AS `mone`,`b`.`descri` AS `descri`,cast(0 as decimal(10,0)) AS `idcaja`,`b`.`unid` AS `unid`,`b`.`pre1` AS `pre1`,`b`.`peso` AS `peso`,`b`.`pre2` AS `pre2`,ifnull(`z`.`vend_idrv`,0) AS `nidrv`,`c`.`vigv` AS `vigv`,`a`.`dsnc` AS `dsnc`,`a`.`dsnd` AS `dsnd`,`a`.`gast` AS `gast`,`c`.`idcliente` AS `idcliente`,`c`.`codt` AS `codt`,`b`.`pre3` AS `pre3`,`b`.`cost` AS `costo`,`b`.`uno` AS `uno`,`b`.`dos` AS `dos`,(`b`.`uno` + `b`.`dos`) AS `TAlma`,`c`.`fusua` AS `fusua`,`p`.`nomv` AS `Vendedor`,`q`.`nomb` AS `Usuario`,`c`.`rcom_idtr` AS `rcom_idtr`,`c`.`rcom_tipo` AS `rcom_tipo`,`a`.`kar_cost` AS `kar_cost`,`c`.`rcom_mret` AS `rcom_mret` from ((((((`eduar_bdwalter`.`fe_rcom` `c` join `eduar_bdwalter`.`fe_kar` `a` on((`a`.`idauto` = `c`.`idauto`))) join `eduar_bdwalter`.`vlistaprecios` `b` on((`b`.`idart` = `a`.`idart`))) join `eduar_bdwalter`.`fe_clie` `d` on((`d`.`idclie` = `c`.`idcliente`))) join `eduar_bdwalter`.`fe_vend` `p` on((`p`.`idven` = `a`.`codv`))) join `eduar_bdwalter`.`fe_usua` `q` on((`q`.`idusua` = `c`.`idusua`))) left join (select `eduar_bdwalter`.`fe_rvendedor`.`vend_idau` AS `vend_idau`,`eduar_bdwalter`.`fe_rvendedor`.`vend_idrv` AS `vend_idrv` from `eduar_bdwalter`.`fe_rvendedor` where (`eduar_bdwalter`.`fe_rvendedor`.`vend_acti` = 'A')) `z` on((`z`.`vend_idau` = `c`.`idauto`))) where ((`c`.`acti` <> 'I') and (`a`.`acti` <> 'I')) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vpdtesx` AS select sum(ifnull(`f`.`entr_cant`,0)) AS `entregado`,(`b`.`cant` - sum(ifnull(`f`.`entr_cant`,0))) AS `saldo`,`a`.`idauto` AS `idauto`,`b`.`idkar` AS `idkar`,`b`.`idart` AS `idart` from (((`eduar_bdwalter`.`fe_kar` `b` join `eduar_bdwalter`.`fe_rcom` `a` on((`a`.`idauto` = `b`.`idauto`))) left join (select `eduar_bdwalter`.`fe_ent`.`entr_cant` AS `entr_cant`,`eduar_bdwalter`.`fe_ent`.`entr_idkar` AS `entr_idkar`,`eduar_bdwalter`.`fe_ent`.`entr_idgu` AS `entr_idgu`,`eduar_bdwalter`.`fe_ent`.`entr_acti` AS `entr_acti` from `eduar_bdwalter`.`fe_ent` where (`eduar_bdwalter`.`fe_ent`.`entr_acti` = 'A')) `f` on((`f`.`entr_idkar` = `b`.`idkar`))) left join `eduar_bdwalter`.`fe_guias` `w` on((`w`.`guia_idgui` = `f`.`entr_idgu`))) where (((`a`.`acti` = 'A') and (`b`.`acti` = 'A') and (`a`.`idcliente` > 0)) or (`f`.`entr_acti` = 'A') or isnull(`f`.`entr_acti`)) group by `b`.`idkar`,`a`.`idauto`,`b`.`idart` */;
