...
MySQL - 8.0.35 : Database - syscom_bdmzatrujillo
*********************************************************************
*/
...
cdir VARCHAR(150),nruc VARCHAR(11),chofe VARCHAR(100),cbreve VARCHAR(15),
...
cdire VARCHAR(150),cruc VARCHAR(11),cchofer VARCHAR(100),cbreve VARCHAR(25),cmarca VARCHAR(50),ccons VARCHAR(30),
...
BEGIN
SELECT nomb,idalma,dire,ciud,sucuidserie FROM fe_sucu  where idalma in(1) order by nomb ;
END */$$
DELIMITER ;
...
/*!50003 CREATE PROCEDURE `ProMuestraProductosconstock`(abuscar VARCHAR(80), nd DECIMAL(5,3),constock CHAR)
BEGIN
DECLARE cbuscar VARCHAR(80);
SET cbuscar=CONCAT('%',TRIM(abuscar),+'%');
IF constock='S' THEN
	...
	CAST(IFNULL(d.cost_idco,0) AS UNSIGNED) AS cost_idco,IFNULL(d.cost_flet,0)  AS fleter,nd AS dolar,
	peso,a.prec,tipro,idmar,a.idcat,cost,tmon,a.idflete,prod_uti1,prod_uti2,prod_uti3,prod_idus,prod_equi1,prod_equi2,
	prod_come,prod_comc,ulpc,prod_idus,prod_uact,prod_fact,fechc,prod_smax,prod_smin,IFNULL(o.razo,'') AS proveedor,
	IFNULL(yy.ndoc,'') AS ndoc,IFNULL(yy.fech,'') AS fech,prod_icbper,prod_acti,prod_detr
	FROM fe_art  AS a 
	...
	LEFT JOIN fe_rcom AS yy ON (yy.idauto=a.prod_idau) 
	LEFT JOIN fe_prov AS o ON (o.idprov=yy.idprov)
	WHERE descri LIKE cbuscar AND prod_acti<>'I' AND uno>0 ORDER BY DESCRI;
 ELSE
 SELECT idart,descri,unid,prod_unid1,
	CAST(IF(uno>0,IF(MOD(uno,prod_equi1)=0,uno/prod_equi1,
	IF(MOD(uno,prod_equi1)=0,uno DIV prod_equi1,TRUNCATE(uno/prod_equi1,0))),0.00) AS DECIMAL(12,2)) AS prod_unim,
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vmuestractasventas` AS select left(`p`.`nomb`,3) AS `tdoc`,`b`.`ndoc` AS `ndoc`,`b`.`fech` AS `fech`,`a`.`ncta` AS `ncta`,`c`.`razo` AS `razo`,(case `x`.`tipo` when 'D' then if((`b`.`mone` = 'S'),`x`.`impo`,round((`x`.`impo` * `b`.`dolar`),2)) else 0 end) AS `Debe`,(case `x`.`tipo` when 'H' then if((`b`.`mone` = 'S'),`x`.`impo`,round((`x`.`impo` * `b`.`dolar`),2)) else 0 end) AS `Haber`,`x`.`tipo` AS `tipo`,`a`.`idcta` AS `idcta`,`a`.`nomb` AS `nomb`,`b`.`idauto` AS `idrven`,`b`.`mone` AS `mone`,`x`.`idectas` AS `idectas`,`c`.`idclie` AS `idclie` from ((((`fe_ectas` `x` join `fe_plan` `a` on((`a`.`idcta` = `x`.`idcta`))) join `fe_rcom` `b` on((`b`.`idauto` = `x`.`idrven`))) join `fe_clie` `c` on((`c`.`idclie` = `b`.`idcliente`))) join `fe_tdoc` `p` on((`p`.`tdoc` = `b`.`tdoc`))) where ((`x`.`impo` <> 0) and (`b`.`acti` <> 'I') and (`p`.`dcto_acti` = 'A') and (`x`.`acti` = 'A') and (`b`.`tdoc` in ('01','03','07','08'))) */;
